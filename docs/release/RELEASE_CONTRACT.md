...
La préversion corrigée est `v0.1.0-rc.3`. Le tag lisible utilise cette convention, tandis que les manifestes desktop portent l’identité MSI-compatible `0.1.0-3` et le paquet Python la forme PEP 440 équivalente `0.1.0rc3`. Le builder normalise uniquement cette forme Python vers `0.1.0-3`, puis refuse toute autre divergence. Cette dissociation est nécessaire car le bundle MSI exige un identifiant de préversion numérique ne dépassant pas `65535`. Aucun tag n’est créé automatiquement par CI.
