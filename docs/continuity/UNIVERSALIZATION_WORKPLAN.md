...
| Révision VERA-MMU | `4f9d1ed0c881d41b7e98a01e228f05903e65a408` sur `main`, chaînage runtime default/override jusqu’au reader M4-A3 publié et vérifié. | `OBSERVED` | Journal M4-A à consigner avec les commits `1ea116f`, `e3105b0`, `cdf65f7`, `8263d40`. |
...
| Capacité actuelle | M1, M2 et M3 sont publiés dans leurs périmètres contractuels. M4-A ajoute le resolver runtime read-only avec policy WAL/SHM fail-closed, le batch Core sur type existant, le ledger d’import 033, la conformité profonde `component` et une autorisation de page non fusionnelle qui délègue exclusivement au ledger. | `OBSERVED` : migrations 001→033, `301 passed, 14 subtests passed`, resolver baseline `DEFAULT_RUNTIME_LAYOUT`/`NO_WAL_SIDECARS` et chaîne override jusqu’au reader validée, import baseline de 17 composants avec 17 liens de ledger et replay sans écriture dans un store VERA temporaire, zéro evidence/proof link, roue isolée et frontières de pack validées. | Registre M4 ; journal M4-A à consigner. |
| Exclusions actuelles | Aucun shell arbitraire, réseau implicite, accès fichier externe, runner autre que `NOOP`/`EVIDENCE_HASH`/`EVIDENCE_FIELDS`, validator métier ou oracle externe, quorum pondéré, expiration/fenêtre temporelle ou révocation de gate, pause/reprise/réouverture ou orchestration de lifecycle, CLI/MCP de production, import ARET hors pages `component` explicitement autorisées M4-A, merge, proof/promotion, pack de compatibilité complet ou dashboard. Le sous-ensemble de paramètres ne devient pas JSON Schema général; `CONFIRM` ne devient pas une confirmation interactive; HMAC ne couvre ni rotation/révocation/expiration ni algorithme alternatif; aucun runner ou policy ne devient admission/proof automatique. | `OBSERVED` | Registre M4 ; journal M4-A à consigner. |
...
| Lot suivant | M4 est ouvert. M4-A a prouvé le resolver/WAL read-only, le ledger, l’import de page `component` et sa post-validation read-only, mais la conformance globale, `function_symbol`, `brick`, les données sémantiques, le pack capabilities et la parité restent à livrer suivant le [registre de clôture M4](M4_COMPLETION_REGISTER.md). Aucun verdict ne clôture C01–C16, `MEM-WALL-001` ou la parité ARET. | `M4_IN_PROGRESS` ; `MEM-WALL-001` préservée | Registre M4 ; journal M4-A à consigner. |
...
| `M4` | Pack ARET et primitive Core | M4.1–M4.15 livrent les surfaces déclaratives et le premier write-path. M4-A ajoute le resolver runtime read-only avec policy WAL/SHM fail-closed, le batch Core sur type existant, le ledger 033 append-only/idempotent, la conformité source read-only `component` et l’autorisation de page explicite avec série non fusionnelle. Toute écriture de page passe par le ledger, demeure auditée/rollbackée et retourne `IMPORTED_NO_PROMOTION`. Conformance globale, autres tables, capabilities, toolchain, MCP/hooks/bundles/VCS et parité restent dans le [registre de clôture M4](M4_COMPLETION_REGISTER.md). | M4-A : tests-first, suite Core `305 passed, 14 subtests passed`, scans Core/pack, roues isolées, rollback/idempotence, post-validation read-only et import réel de 17 composants/17 liens de ledger/replay sans écriture dans une cible temporaire ; zéro evidence/proof link. La parité ARET reste mesurée uniquement contre une baseline exécutable. | I002, I004, I006–I008, I015 | `IN_PROGRESS` — M4-A `PASS` borné ; C01–C06/C16 `SPLIT`, C07/C08 `BLOCKED`, parité `UNKNOWN` |
