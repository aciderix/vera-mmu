...
>
> **Dernière revue factuelle :** 26 août 2026 — M3.25 validé; M3.EXIT est la seule gate M3 restante.
>
...
| Révision VERA-MMU | `a67d0dd05102b1d14341bd299d9fc62ce6029e2e` sur `main`, commit fonctionnel M3.25 publié et vérifié. | `OBSERVED` | Journal `LOG-0120`. |
...
| Capacité actuelle | M1 et M2 sont publiés. M3.1–M3.25 ajoutent des contrats, policies et runners locaux fermés, executions/evidence hashées, validation locale, admission/preuve séparées, gates, lifecycle/readiness, diagnostics passifs, policy de complétion, binding admission-validation et catalogue runner-validator fermé. | `OBSERVED` : migrations 001→032, `186 passed, 14 subtests passed`, wheel isolée et frontières de sécurité validés. | Mémoire `MEM-STATE-065`; journal `LOG-0119`. |
| Exclusions actuelles | Aucun shell arbitraire, réseau implicite, accès fichier externe, runner autre que `NOOP`/`EVIDENCE_HASH`/`EVIDENCE_FIELDS`, validator métier ou oracle externe, quorum pondéré, expiration/fenêtre temporelle ou révocation de gate, pause/reprise/réouverture ou orchestration de lifecycle, CLI/MCP de production, pack/importeur ARET ou dashboard. Le sous-ensemble de paramètres ne devient pas JSON Schema général; `CONFIRM` ne devient pas une confirmation interactive; HMAC ne couvre ni rotation/révocation/expiration ni algorithme alternatif; aucun runner ou policy ne devient admission/proof automatique. | `OBSERVED` | Mémoire `MEM-DEC-045`; journal `LOG-0115`. |
...
| Lot suivant | Les lots M3.22–M3.25 sont `PASS`. M3.EXIT est la seule gate désormais autorisée : audit cumulatif, fresh/historical upgrades, chaîne Core complète, scans, wheel, documentation et publication; aucun nouveau lot M3 ne peut être introduit. | `M3_IN_PROGRESS` ; `MEM-WALL-001` préservée | Journal `LOG-0119`; mémoire `MEM-STATE-066`. |
...
| `M3` | Capability / Evidence / Gates | M3.1–M3.25 ont livré les contrats/policies/runners locaux fermés, evidence/admission/proof séparées, validation locale, gates, lifecycle/readiness, diagnostics passifs, complétion optionnelle, binding admission-validation et catalogue fermé. Reste exclusivement M3.EXIT selon le contrat approuvé. | Migrations 001→032, compatibilité fresh/historique, rollback, suite Core, scans de frontière et wheels isolées sont validés par lot. Aucun verdict ne ferme M3 global ni la parité ARET avant M3.EXIT. | I004–I008, I013, I014, I015 | `IN_PROGRESS` — M3.1–M3.25 `PASS`; M3.EXIT uniquement |
