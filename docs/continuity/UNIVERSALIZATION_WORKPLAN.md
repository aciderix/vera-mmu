...
| Révision VERA-MMU | Les tranches M4-D jusqu’à `f6f8be3` sont publiées. La tranche locale `8818100` ajoute la matrice contractuelle de transport des verdicts ARET; sa publication accompagne la correction documentaire de périmètre. | `OBSERVED` ; Core et Pack restent séparés. | Registre M4 et artefacts du 26 août 2026. |
...
| Capacité actuelle | M1, M2 et M3 sont publiés dans leurs périmètres contractuels. M4-D fournit le transport fermé Pack→Core : normalisation, execution, asset, evidence, validation, admission, proof et gate. | `OBSERVED` : migrations 001→038, suite `397 passed, 25 subtests passed`, roue isolée. Les payloads contractuels `272/272`, `271/272`, timeout, sortie inconnue, dépendance absente et hash non promouvable sont classés et ne permettent une promotion que pour `PASS` validé. Le score effectivement obtenu par ARET est hors de cette conformance VERA. | Registre M4 ; `artifacts/m4d_verdict_transport_scope_correction_2026-08-26.md`. |
...
| Lot suivant | M4.EXIT reste `NOT_ELIGIBLE` pour ses imports, playbook, bundles/VCS et compatibilités restants. Le prochain test de transport client est M5 : exposer exactement la même matrice de verdicts via un vrai MCP, sans laisser le client injecter commande, verdict ou artefact. Les divergences Wine sont des observations ARET distinctes, non une condition de réussite du transport VERA. | `M4_IN_PROGRESS` ; parité ARET hors conformance de transport | Registre M4 ; contrat MCP M5 du 26 août 2026. |
...
| `M5` | Compilateur MCP et adapters | Manifeste immuable, API stable, instructions/hook/config générés, plus façade MCP qui reproduit les scénarios `PASS`/`FAIL`/`SKIPPED`/`ERROR`/`UNKNOWN` déjà testés au niveau service. | Même entrée = même `mcp_build_hash`; vrai serveur/client MCP, snapshots et matrice client→execution→evidence→admission→gate passent sans injection client de commande/verdict/artefact. | I007–I009, I012–I014 | `PLANNED` |
