...
>
> **Dernière revue factuelle :** 26 août 2026 — M4-A importe la page baseline de `component` dans un store temporaire ; M4-B a ensuite importé et post-validé des pages bornées de `function_symbol` et `brick`, avec ledger, audits et replay sans écriture. M4 reste `IN_PROGRESS`, les couplages restent `SPLIT` et la parité `UNKNOWN`.
>
...
| Révision VERA-MMU | `2ab13fb4d8b6cb7558c824f6d405c4d7b27e95db` local sur `main`, correctif minimal du Domain Pack pour résoudre `brick.component_id` vers le parent VERA projeté `aret-component--<id>` ; aucune publication distante de ce commit n’a été faite. | `OBSERVED` | `LOG-0172`; commit local et test de régression. |
...
| Lot suivant | M4-B reste ouvert malgré l’intégration réelle bornée : il faut préserver la limite de page initiale, tracer la publication locale éventuelle et définir séparément toute extension de série, lifecycle/Front ou parité. Aucun verdict ne clôture C01–C16, `MEM-WALL-001` ou la parité ARET. | `M4_IN_PROGRESS` ; `MEM-WALL-001` préservée | Registre M4 ; `LOG-0172`. |
...
| `M4` | Pack ARET et primitive Core | M4.1–M4.15 livrent les surfaces déclaratives et le premier write-path. M4-A ajoute resolver/WAL read-only, ledger entity 033, conformité `component` et autorisation de page non fusionnelle. M4-B livre lecteurs/projections structurels, conformances/préflights, collision/autorisation, imports atomiques/post-validations et le Core 034 de batch `SYMBOL`/`WORK_ITEM`, audité/append-only/idempotent, sans import legacy implicite. Conformance globale, autres tables, capabilities, toolchain, MCP/hooks/bundles/VCS et parité restent dans le [registre de clôture M4](M4_COMPLETION_REGISTER.md). | Intégration temporaire réelle : 17 components/17 liens, 9 symbols et 13 work items, audits/ledgers et replay sans écriture ; Core status `PLANNED` préservé pour les bricks, zéro evidence/proof/admission/promotion. Tests-first du correctif de parent brick, suite `360 passed, 14 subtests passed`, scans et roue isolée passent. La parité ARET reste mesurée uniquement contre une baseline exécutable. | I002, I004, I006–I008, I015 | `IN_PROGRESS` — M4-A `PASS` borné ; M4-B intégration de première page observée, mais C01–C06/C16 `SPLIT`, C07/C08 `BLOCKED`, parité `UNKNOWN` |
