...
| Révision VERA-MMU | Les commits fonctionnels locaux `d2efe72` (séries structurelles M4-B) et `41594ba` (ledger knowledge M4-C.1) sont au-dessus de `e92ab70c2608bac4fa2e21e401ec9d1833f747bb`, seule révision publiée et vérifiée sur `origin/main` à ce stade. | `OBSERVED` | Publication antérieure vérifiée ; `LOG-0172` à `LOG-0174`. |
...
| Capacité actuelle | M1, M2 et M3 sont publiés dans leurs périmètres contractuels. M4-A apporte resolver runtime/WAL fail-closed, ledger entity 033, conformité `component` et import de page non fusionnel. M4-B ajoute lecteurs/projections, Core 034, conformances/préflights, collisions/autorisations, imports structurels atomiques, séries non fusionnelles et post-validations read-only. M4-C.1 ajoute le ledger générique knowledge Core 035 et le lecteur/projection knowledge read-only. | `OBSERVED` : migrations 001→035, suite `369 passed, 14 subtests passed`, et intégration structurelle réelle bornée dans une cible `/tmp` : 17 components, 9 symbols en 3 pages et 13 work items en 5 pages, replays sans écriture, 8 batches structurels, 22 liens et zéro evidence/proof/admission/promotion. M4-C.1 ne comporte encore aucun import knowledge réel. | Registre M4 ; `LOG-0172` à `LOG-0174`. |
...
| Lot suivant | M4-C est engagé par le ledger knowledge et la projection read-only; la prochaine progression autorisée est un contrat knowledge de type cible, préflight/collision/autorisation et intégration temporaire, sans déduire de preuve d’un import. Les limites M4-B lifecycle/Front, hiérarchie, conflits/courses et parité sont inchangées. Aucun verdict ne clôture C01–C16, `MEM-WALL-001` ou la parité ARET. | `M4_IN_PROGRESS` ; `MEM-WALL-001` préservée | Registre M4 ; `LOG-0173`, `LOG-0174`. |
...
| `M4` | Pack ARET et primitive Core | M4.1–M4.15 livrent les surfaces déclaratives et le premier write-path. M4-A ajoute resolver/WAL read-only, ledger entity 033, conformité `component` et autorisation de page non fusionnelle. M4-B livre lecteurs/projections structurels, conformances/préflights, collision/autorisation, imports atomiques/post-validations, séries non fusionnelles et le Core 034 de batch `SYMBOL`/`WORK_ITEM`, audité/append-only/idempotent, sans import legacy implicite. M4-C.1 ajoute le Core 035 de batch `knowledge` et son lecteur/projection source sans écriture. Conformance globale, autres tables, capabilities, toolchain, MCP/hooks/bundles/VCS et parité restent dans le [registre de clôture M4](M4_COMPLETION_REGISTER.md). | Intégration temporaire réelle : 17 components/17 liens, 9 symbols en 3 pages et 13 work items en 5 pages, 8 batches structurels et replay sans écriture ; Core status `PLANNED` préservé pour les bricks, zéro evidence/proof/admission/promotion. Pour M4-C.1, ledger et projection sont testés mais aucun import knowledge réel n’est encore observé. Suite `369 passed, 14 subtests passed`, scans et roue isolée passent. La parité ARET reste mesurée uniquement contre une baseline exécutable. | I002, I004, I006–I008, I015 | `IN_PROGRESS` — M4-A `PASS` borné ; M4-B séries réelles observées; M4-C.1 substrate `PASS` borné, mais C01–C06/C16 `SPLIT`, C07/C08 `BLOCKED`, parité `UNKNOWN` |
