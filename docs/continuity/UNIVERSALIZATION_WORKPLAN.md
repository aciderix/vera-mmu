...
| Révision VERA-MMU | Les tranches M4-D et la correction documentaire sont publiées jusqu’à `44c928b`. Les jalons fonctionnels locaux `5ffe182`, `5de260d`, `50cc79a`, `e073fa2` et `9010293` portent façade MCP, manifeste, registry, adapter/hôte de Pack et instructions attestées. | `OBSERVED` ; Core, Pack, façade, manifeste, registry et instructions restent séparés. | `LOG-0182` à `LOG-0186` et artefact M5 du 26 août 2026. |
...
| Capacité actuelle | M1, M2 et M3 fournissent le Core; M4-D transporte les verdicts Pack→Core; M5-A expose la chaîne par vrai serveur/client MCP stdio; M5-B la lie à un manifeste canonique vérifié; M5-C résout capability→adapter dans un registry fermé; M5-D assemble un adapter ARET Pack; M5-E attache des instructions compilées. | `OBSERVED` : migrations 001→038, suite `416 passed, 37 subtests passed`, roue isolée. Les payloads contractuels atteignent admission/gate seulement pour `PASS` validé; manifest/adapter/instruction étranger, périmé ou ambigu est refusé; l’adapter Pack refuse les résultats ou commandes du client. Le score effectivement obtenu par ARET est hors de cette conformance VERA. | `LOG-0182` à `LOG-0186`; artefacts M4-D et M5-A/B/C/D/E du 26 août 2026. |
| Exclusions actuelles | Aucun shell arbitraire, réseau implicite, accès fichier externe libre, validator métier, admission/proof/promotion automatique, quorum pondéré, expiration/fenêtre temporelle ou révocation de gate, pause/reprise/réouverture ou orchestration de lifecycle, hooks/config générés, CLI complète, merge, pack de compatibilité complet ou dashboard. La façade MCP M5-A/B/C/D/E vérifie manifeste, registry et instructions mais refuse toute execution sans hôte de Pack explicite; le seul runner externe Core est `OBSERVED_PROCESS`, qui n’exécute rien ; le pack ARET ne lance que son catalogue fermé sous namespace réseau isolé. Les imports M4-A/M4-B restent non fusionnels et temporaires. | `OBSERVED` | `LOG-0182` à `LOG-0186`; registre M4 et tests du runner fermé. |
...
| Lot suivant | M4.EXIT reste `NOT_ELIGIBLE` pour ses imports, playbook, bundles/VCS et compatibilités restants. M5-A/B/C/D/E ont validé transport, manifeste, registry, adapter Pack et instructions; le prochain sous-lot M5 compile hooks/config et une installation déclarative attestée, sans laisser le client injecter commande, verdict ou artefact. Les divergences Wine restent des observations ARET distinctes. | `M5_IN_PROGRESS` ; parité ARET hors conformance de transport | `LOG-0182` à `LOG-0186`; contrat MCP M5-A/B/C/D/E du 26 août 2026. |
...
| `M5` | Compilateur MCP et adapters | M5-A livre la façade MCP stdio et la matrice client→execution→evidence→admission→gate; M5-B livre le manifeste immuable, `mcp_build_hash` et sa vérification; M5-C livre le registry d’objets hôte; M5-D livre l’adapter et l’hôte ARET Pack; M5-E livre les instructions générées et vérifiées. Restent hooks/config générés, installation attestée et snapshots. | Même entrée = mêmes `mcp_build_hash` et `instructions_hash`; vrai serveur/client MCP, snapshots et matrice client→execution→evidence→admission→gate passent sans injection client de commande/verdict/artefact. | I007–I009, I012–I014 | `IN_PROGRESS` — M5-A `PASS` (`5ffe182`), M5-B `PASS` (`5de260d`), M5-C `PASS` (`50cc79a`), M5-D `PASS` (`e073fa2`), M5-E `PASS` (`9010293`), hooks/config/installation `PLANNED` |
