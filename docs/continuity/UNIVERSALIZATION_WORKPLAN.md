...
| Lot suivant | M2.EXIT est passé. Le prochain travail technique éventuel est `M3 — Capability / Evidence / Gates`, avec un nouveau plan et une hypothèse autonome ; aucune capacité M3 n’est engagée par cette gate. | `M2_COMPLETE` ; `MEM-WALL-001` préservée | Journal `LOG-0049` ; mémoire `MEM-STATE-024`. |
