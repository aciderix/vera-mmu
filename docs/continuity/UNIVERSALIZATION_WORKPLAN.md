...
| Révision VERA-MMU | `d2efe72` est le commit fonctionnel local de séries structurelles M4-B non fusionnelles, au-dessus de `e92ab70c2608bac4fa2e21e401ec9d1833f747bb` publié et vérifié sur `origin/main`; la documentation de continuité reste alors à committer séparément. | `OBSERVED` | Publication antérieure vérifiée ; `LOG-0172`, `LOG-0173`. |
...
| Capacité actuelle | M1, M2 et M3 sont publiés dans leurs périmètres contractuels. M4-A apporte resolver runtime/WAL fail-closed, ledger entity 033, conformité `component` et import de page non fusionnel. M4-B ajoute lecteurs/projections, Core 034, conformances/préflights, collisions/autorisations, imports structurels atomiques, séries non fusionnelles et post-validations read-only. | `OBSERVED` : migrations 001→034, suite `362 passed, 14 subtests passed`, et intégration réelle bornée dans une cible `/tmp` : 17 components, 9 symbols en 3 pages et 13 work items en 5 pages, replays sans écriture, 8 batches structurels, 22 liens et zéro evidence/proof/admission/promotion. | Registre M4 ; `LOG-0172`, `LOG-0173`. |
...
| Lot suivant | M4-B conserve des limites lifecycle/Front, hiérarchie, conflits/courses et parité, mais la prochaine progression autorisée est M4-C : mappings sémantiques table par table, sans déduire de preuve d’un import. Aucun verdict ne clôture C01–C16, `MEM-WALL-001` ou la parité ARET. | `M4_IN_PROGRESS` ; `MEM-WALL-001` préservée | Registre M4 ; `LOG-0173`. |
