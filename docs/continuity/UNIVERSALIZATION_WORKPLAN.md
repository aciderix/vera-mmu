...
>
> **Dernière revue factuelle :** 26 août 2026 — M4.2 validé; M4 est `IN_PROGRESS`, C01 et C02 restent `SPLIT`.
>
...
| Révision VERA-MMU | `e023a12ab506c5ce44a1566f2b221e2aba88b8bc` sur `main`, manifeste runtime ARET V1 M4.2 publié et vérifié. | `OBSERVED` | Journal `LOG-0126`. |
...
| Lot suivant | M4 est ouvert. M4.1–M4.2 sont `PASS`; le prochain lot doit cibler un couplage de la matrice sous un contrat séparé. Aucun verdict ne clôture C01–C16, `MEM-WALL-001` ou la parité ARET. | `M4_IN_PROGRESS` ; `MEM-WALL-001` préservée | Journal `LOG-0125`; mémoire `MEM-STATE-072`. |
...
| `M4` | Pack ARET | M4.1 livre le lecteur `ARET://` V1 strictement canonique et en lecture; M4.2 déclare le layout runtime V1 sans le résoudre. Import hors ligne, pipelines, playbook, toolchain et parité restent à contractualiser séparément. | M4.1–M4.2 : surfaces fermées, Core sans dépendance pack, refus non-canonique, aucune I/O runtime, suite Core et wheels. La parité ARET reste mesurée uniquement contre une baseline exécutable. | I002, I004, I006–I008, I015 | `IN_PROGRESS` — M4.1–M4.2 `PASS`; C01/C02 `SPLIT`, parité `UNKNOWN` |
