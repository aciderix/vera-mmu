...
| Capacité actuelle | Core d’identité, substrate SQLite, entités et relations : migrations checksumées, metadata/audit, transaction, types enregistrés, entités JSON canoniques et arêtes immuables à lecture exacte. | `OBSERVED` : 48 tests et 14 sous-tests passent ; wheel isolé et relation typée validée. | Journal `LOG-0015` ; mémoire `MEM-STATE-009`. |
| Exclusions actuelles | Pas encore de knowledge, traversal/lifecycle relationnel, symbol, evidence/bundle, serveur MCP de production, Capability Engine, importeur ARET ou dashboard. | `OBSERVED` | Mémoire `MEM-STATE-009` ; journal `LOG-0015`. |
...
| Lot suivant | Aucun sous-lot actif ; M2.4 ne peut être armé que par un rituel séparé après vérification du commit M2.3. | `READY_FOR_M2.4_RITUAL` ; `MEM-WALL-001` préservée | Journal `LOG-0015` ; mémoire section 7. |
...
| `M2` | Persistence universelle | M2.1 : substrate SQLite. M2.2 : types et entités. M2.3 : types relationnels et arêtes immuables entre entités. Sous-lots ultérieurs : knowledge et contraintes métier restantes. | M2.1/M2.2/M2.3 validés ; knowledge append-only, traversal/lifecycle relationnel, intégrité référentielle étendue et audit métier restent à compléter par sous-lots. | I001–I006, I010, I011, I014, I015 | `IN_PROGRESS` — M2.1/M2.2/M2.3 `PASS`; M2 complet `UNKNOWN` |
