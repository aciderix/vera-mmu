...
>
> **Dernière revue factuelle :** 26 août 2026 — M3.24 validé; M3 reste `IN_PROGRESS` jusqu’à `M3.EXIT`.
>
...
| Révision VERA-MMU | `14fda1f972745eaada9b8f30806dedd7ac58fe43` sur `main`, commit fonctionnel M3.24 publié et vérifié. | `OBSERVED` | Journal `LOG-0118`. |
...
| Capacité actuelle | M1 et M2 sont publiés. M3.1–M3.24 ajoutent des contrats, policies et runners locaux fermés, executions/evidence hashées, validation locale, admission/preuve séparées, gates, lifecycle/readiness, diagnostics passifs, policy de complétion et binding admission-validation strict. | `OBSERVED` : migrations 001→032, `184 passed, 14 subtests passed`, wheel isolée et frontières de sécurité validés. | Mémoire `MEM-STATE-063`; journal `LOG-0117`. |
...
| Lot suivant | M3.24 ajoute le binding admission-validation strict. Le seul lot désormais autorisé est M3.25, catalogue fermé runner-validator-schema, sous le contrat terminal approuvé; aucun autre lot M3 ne peut être introduit. | `M3_IN_PROGRESS` ; `MEM-WALL-001` préservée | Journal `LOG-0117`; mémoire `MEM-STATE-064`. |
...
| `M3` | Capability / Evidence / Gates | M3.1–M3.24 ont livré les contrats/policies/runners locaux fermés, evidence/admission/proof séparées, validation locale, gates, lifecycle/readiness, diagnostics passifs, complétion optionnelle et binding admission-validation strict. Restent exclusivement M3.25 et M3.EXIT selon le contrat approuvé. | Migrations 001→032, compatibilité fresh/historique, rollback, suite Core, scans de frontière et wheels isolées sont validés par lot. Aucun verdict ne ferme M3 global ni la parité ARET avant M3.EXIT. | I004–I008, I013, I014, I015 | `IN_PROGRESS` — M3.1–M3.24 `PASS`; M3.25 puis M3.EXIT uniquement |
