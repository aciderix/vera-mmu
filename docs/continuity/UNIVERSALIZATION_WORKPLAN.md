...
| Révision VERA-MMU | `8d6e4fc2ec674ac3d2be8297ad3b3f9868239eaa` sur `main`, conformance et préflights structurels M4-B publiés après le primitive Core 034. | `OBSERVED` | `LOG-0165`, `LOG-0167`, `LOG-0168`; commits Core/packs publiés et vérifiés. |
...
| Capacité actuelle | M1, M2 et M3 sont publiés dans leurs périmètres contractuels. M4-A apporte le resolver runtime read-only/WAL fail-closed, le ledger entity 033, la conformité `component` et l’import de page non fusionnel. M4-B apporte lecteurs/projections read-only/purs, le Core 034 et les conformances/préflights structurels qui lient les snapshots inspectés sans conférer d’autorisation. | `OBSERVED` : migrations 001→034, suite `340 passed, 14 subtests passed`, import baseline de 17 composants/17 liens/replay sans écriture dans une cible temporaire, et M4-B validé par conformances source, préflights zéro-write, Core 034, replay, rollback, migration 033→034, immutabilité et wheel isolée ; zéro evidence/proof link créée par ces paths. | Registre M4 ; `LOG-0165`. |
| Exclusions actuelles | Aucun shell arbitraire, réseau implicite, accès fichier externe, runner autre que `NOOP`/`EVIDENCE_HASH`/`EVIDENCE_FIELDS`, validator métier ou oracle externe, quorum pondéré, expiration/fenêtre temporelle ou révocation de gate, pause/reprise/réouverture ou orchestration de lifecycle, CLI/MCP de production, import ARET hors pages `component` explicitement autorisées M4-A, merge, proof/promotion, pack de compatibilité complet ou dashboard. Le Core 034 et les préflights ne constituent pas une autorisation de `function_symbol` ni de `brick`; collision/non-fusion, autorisation et post-validation restent requis. | `OBSERVED` | Registre M4 ; `LOG-0165`–`LOG-0166`. |
...
| Lot suivant | M4-B reste ouvert. Les prochains contrats sont collision/non-fusion puis autorisation explicite, séparés pour `function_symbol` et `brick` et liées aux préflights/au Core 034 ; l’import et la post-validation ne peuvent suivre qu’après ces contrats. Aucun verdict ne clôture C01–C16, `MEM-WALL-001` ou la parité ARET. | `M4_IN_PROGRESS` ; `MEM-WALL-001` préservée | Registre M4 ; `LOG-0169`. |
