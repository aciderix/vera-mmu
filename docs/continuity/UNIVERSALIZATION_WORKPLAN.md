...
| Capacité actuelle | M1 et M2 sont publiés. M3.1–M3.7 ajoutent : contrats de capability fermés, validation locale de paramètres typés, seul runner `NOOP` sous `DENY_NETWORK`, executions immuables, evidence JSON hashée, admission immutable, preuve dérivée `PROVEN` sans mutation de knowledge et gate de work item calculée à partir d’une admission existante. | `OBSERVED` : migrations 001→019, 141 tests et 14 sous-tests, wheel isolé et frontières de sécurité validés. | Mémoire `MEM-STATE-027`; journal `LOG-0051` à `LOG-0073`. |
| Exclusions actuelles | Aucun shell arbitraire, réseau implicite, accès fichier externe, runner additionnel, policy `ALLOW`/`DENY`/`CONFIRM`, validator framework, policy HMAC de projet, gate multi-evidence, lifecycle/traversal de work graph, CLI/MCP de production, pack/importeur ARET ou dashboard. Le sous-ensemble de paramètres ne devient pas JSON Schema général. | `OBSERVED` | Mémoire `MEM-DEC-027`, `MEM-STATE-028`; journal `LOG-0073`. |
...
| Lot suivant | M3.7 a fermé la validation de paramètres bornée. Le prochain lot M3 candidat est un catalogue de décisions explicites `ALLOW`/`DENY`/`CONFIRM`, sans runner additionnel, avec baseline/hypothèse/tests propres. | `M3_IN_PROGRESS` ; `MEM-WALL-001` préservée | Journal `LOG-0073`; mémoire `MEM-STATE-028`. |
...
| `M3` | Capability / Evidence / Gates | `M3.S1` livré et M3.7 ajoute la validation locale des paramètres de capability avant execution. Restent policies, validators, runners additionnels, HMAC de projet, gates/graph avancés et surfaces CLI/MCP. | `M3.S1.EXIT` a validé migrations 001→019, tests ciblé/complet, wheel isolé, absence de shell arbitraire/réseau implicite, chaîne execution→evidence `PASS`→admission→preuve dérivée et gate d’admission. M3.7 ajoute un sous-ensemble de paramètres fermé, sans généraliser JSON Schema. Aucun verdict ne ferme M3 global ni la parité ARET. | I004–I008, I013, I014, I015 | `IN_PROGRESS` — `M3.S1.EXIT` et M3.7 `PASS`; lots restants séparés |
