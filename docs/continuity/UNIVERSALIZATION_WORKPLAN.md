...
| `M9` | Release contrôlée | Packaging, migration, documentation dérivée, hashes/signatures et garanties de compatibilité. | Definition of Done globale satisfaite et rapport d’écarts nul ou accepté explicitement. | Tous | `IN_PROGRESS` — M9-A `PASS` : candidats CLI + desktop sous CI native ; M9-B `PASS` : workflow non signant, assembleur final, README, Apache-2.0, DCO et politique de marque, suite `510 passed, 43 subtests passed` localement et run `33070861267` vert Linux/Windows. Run de tag, signatures, GitHub Release, viewer versionné, installation utilisateur et hôtes réels `NOT_RUN`/`PENDING`. |
