...
>
> **Dernière revue factuelle :** 26 août 2026 — M4.1 validé; M4 est `IN_PROGRESS` et C01 reste `SPLIT`.
>
...
| Révision VERA-MMU | `fc91a1c202cc507666c38535c11d3d40a0045aae` sur `main`, lecteur ARET V1 en lecture M4.1 publié et vérifié. | `OBSERVED` | Journal `LOG-0124`. |
...
| Capacité actuelle | M1, M2 et M3 sont publiés dans leurs périmètres contractuels. M4.1 ajoute un lecteur `ARET://` V1 canonique et fermé dans le Domain Pack, sans résolution ni import. | `OBSERVED` : migrations 001→032, `192 passed, 14 subtests passed`, wheel M4.1 et frontières de pack validées. | Mémoire `MEM-STATE-069`; journal `LOG-0123`. |
| Exclusions actuelles | Aucun shell arbitraire, réseau implicite, accès fichier externe, runner autre que `NOOP`/`EVIDENCE_HASH`/`EVIDENCE_FIELDS`, validator métier ou oracle externe, quorum pondéré, expiration/fenêtre temporelle ou révocation de gate, pause/reprise/réouverture ou orchestration de lifecycle, CLI/MCP de production, importeur ARET, pack ARET fonctionnel au-delà du lecteur d’adresses M4.1, ou dashboard. Le sous-ensemble de paramètres ne devient pas JSON Schema général; `CONFIRM` ne devient pas une confirmation interactive; HMAC ne couvre ni rotation/révocation/expiration ni algorithme alternatif; aucun runner ou policy ne devient admission/proof automatique. | `OBSERVED` | Mémoire `MEM-DEC-045`; journal `LOG-0115`. |
...
| Lot suivant | M4 est ouvert. M4.1 est `PASS`; le prochain lot doit cibler un couplage de la matrice sous un contrat séparé. Aucun verdict M4.1 ne réactive ou ne clôture C02–C16, `MEM-WALL-001` ou la parité ARET. | `M4_IN_PROGRESS` ; `MEM-WALL-001` préservée | Journal `LOG-0123`; mémoire `MEM-STATE-070`. |
...
| `M4` | Pack ARET | M4.1 livre le lecteur `ARET://` V1 strictement canonique et en lecture dans le Domain Pack. Import hors ligne, pipelines, playbook, toolchain et parité restent à contractualiser séparément. | M4.1 : parser fermé, Core sans dépendance pack, refus non-canonique, suite Core et wheel. La parité ARET reste mesurée uniquement contre une baseline exécutable. | I002, I004, I006–I008, I015 | `IN_PROGRESS` — M4.1 `PASS`; C01 `SPLIT`, parité `UNKNOWN` |
