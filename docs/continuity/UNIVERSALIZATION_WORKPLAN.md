...
| Lot suivant | M3.S2 est `PASS` pour M3.7–M3.12. Le prochain lot M3 candidat est un validator de contenu explicitement borné ou un runner sûr additionnel, sans oracle ARET, avec baseline/hypothèse/tests propres. Toute nouvelle tranche devra avoir une sortie distincte. | `M3_IN_PROGRESS` ; `MEM-WALL-001` préservée | Journal `LOG-0090`; mémoire `MEM-STATE-040`. |
...
| `M3` | Capability / Evidence / Gates | `M3.S1` livré; `M3.S2` valide l’intégration de M3.7–M3.12 : paramètres fermés, policy `ALLOW`, HMAC de projet, validator local, conjonction de gates et lifecycle dérivé. Restent validators de contenu/oracles, runners additionnels, lifecycle/graph avancés et surfaces CLI/MCP. | `M3.S1.EXIT` a validé migrations 001→019; `M3.S2.EXIT` valide migrations 001→024, suite complète, wheel isolé et chaîne intégrée sans généraliser JSON Schema, confirmation interactive, rotation, quorum, orchestration ou oracle externe. Aucun verdict ne ferme M3 global ni la parité ARET. | I004–I008, I013, I014, I015 | `IN_PROGRESS` — `M3.S1.EXIT` et `M3.S2.EXIT` `PASS`; lots restants séparés |
