...
| Capacité actuelle | Core d’identité, substrate SQLite et registre d’entités : migrations checksumées, metadata/audit, transaction, types enregistrés, entités JSON canoniques et lecture exacte. | `OBSERVED` : 40 tests et 14 sous-tests passent ; wheel isolé et création/lecture d’entité validées. | Journal `LOG-0013` ; mémoire `MEM-STATE-008`. |
| Exclusions actuelles | Pas encore de services knowledge/relation/symbol/evidence/bundle, serveur MCP de production, Capability Engine, importeur ARET ou dashboard. | `OBSERVED` | Mémoire `MEM-STATE-008` ; journal `LOG-0013`. |
...
| Lot suivant | Aucun sous-lot actif ; M2.3 ne peut être armé que par un rituel séparé après vérification du commit M2.2. | `READY_FOR_M2.3_RITUAL` ; `MEM-WALL-001` préservée | Journal `LOG-0013` ; mémoire section 7. |
...
| `M2` | Persistence universelle | M2.1 : substrate SQLite. M2.2 : registre de types et entités génériques auditables. Sous-lots ultérieurs : relations, knowledge et autres contraintes métier. | M2.1/M2.2 validés ; relations, knowledge append-only, intégrité référentielle étendue et audit métier restent à compléter par sous-lots. | I001–I006, I010, I011, I014, I015 | `IN_PROGRESS` — M2.1/M2.2 `PASS`; M2 complet `UNKNOWN` |
