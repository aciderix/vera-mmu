...
| `M9` | Release contrôlée | Packaging, migration, documentation dérivée, hashes/signatures et garanties de compatibilité. | Definition of Done globale satisfaite et rapport d’écarts nul ou accepté explicitement. | Tous | `IN_PROGRESS` — M9-A/M9-B `PASS` : candidats CLI + desktop, workflow/assembleur, README, Apache-2.0, DCO et policy de marque ; run native vert. `v0.1.0-rc.1` est non publiable (MSI) ; `v0.1.0-rc.2` est non publiable (checksum final auto-référent) ; `v0.1.0-rc.3` corrige avec identité binaire `0.1.0-3` (Python `0.1.0rc3`), suite `511 passed, 43 subtests passed`, tag/run exact `PENDING`. Exception non signée limitée à rc.3 ; stable/diffusion large exigent signature. GitHub Release, viewer versionné, installation utilisateur et hôtes réels `NOT_RUN`/`PENDING`. |
