...
| Révision VERA-MMU | Les tranches M4-D et la correction documentaire sont publiées jusqu’à `44c928b`. Les jalons fonctionnels locaux `5ffe182` et `5de260d` portent respectivement la façade MCP stdio et son manifeste/vérificateur déterministe. | `OBSERVED` ; Core, Pack, façade et manifeste restent séparés. | `LOG-0182`, `LOG-0183` et artefact M5 du 26 août 2026. |
...
| Capacité actuelle | M1, M2 et M3 fournissent le Core; M4-D transporte les verdicts Pack→Core; M5-A expose la chaîne par vrai serveur/client MCP stdio; M5-B la lie à un manifeste canonique et vérifié. | `OBSERVED` : migrations 001→038, suite `404 passed, 32 subtests passed`, roue isolée. Les payloads contractuels `272/272`, `271/272`, timeout, sortie inconnue, dépendance absente, hash non promouvable et asset altéré atteignent admission/gate seulement pour `PASS` validé; configuration MCP étrangère/périmée est refusée. Le score effectivement obtenu par ARET est hors de cette conformance VERA. | `LOG-0182`, `LOG-0183`; artefacts M4-D et M5-A/B du 26 août 2026. |
| Exclusions actuelles | Aucun shell arbitraire, réseau implicite, accès fichier externe libre, validator métier, admission/proof/promotion automatique, quorum pondéré, expiration/fenêtre temporelle ou révocation de gate, pause/reprise/réouverture ou orchestration de lifecycle, registry/adapters de production configurés, instructions/hooks/config générés, CLI complète, merge, pack de compatibilité complet ou dashboard. La façade MCP M5-A/B existe et vérifie son manifeste, mais refuse toute execution sans adapter explicite; le seul runner externe est `OBSERVED_PROCESS`, qui n’exécute rien dans le Core ; le pack ARET ne lance que son catalogue fermé sous namespace réseau isolé. Les imports M4-A/M4-B restent non fusionnels et temporaires. | `OBSERVED` | `LOG-0182`, `LOG-0183`; registre M4 et tests du runner fermé. |
...
| Lot suivant | M4.EXIT reste `NOT_ELIGIBLE` pour ses imports, playbook, bundles/VCS et compatibilités restants. M5-A/B ont validé transport et manifeste; le prochain sous-lot M5 déclare un registry d’adapters de production et compile instructions/hooks/config sans laisser le client injecter commande, verdict ou artefact. Les divergences Wine restent des observations ARET distinctes. | `M5_IN_PROGRESS` ; parité ARET hors conformance de transport | `LOG-0182`, `LOG-0183`; contrat MCP M5-A/B du 26 août 2026. |
...
| `M5` | Compilateur MCP et adapters | M5-A livre la façade MCP stdio et la matrice client→execution→evidence→admission→gate; M5-B livre le manifeste immuable, `mcp_build_hash` et sa vérification par la façade. Restent registry/adapters de production, instructions/hooks/config générés et snapshots. | Même entrée = même `mcp_build_hash`; vrai serveur/client MCP, snapshots et matrice client→execution→evidence→admission→gate passent sans injection client de commande/verdict/artefact. | I007–I009, I012–I014 | `IN_PROGRESS` — M5-A `PASS` (`5ffe182`), M5-B `PASS` (`5de260d`), adapters/instructions/config de production `PLANNED` |
