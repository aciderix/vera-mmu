...
| Capacité actuelle | M1 et M2 sont publiés. M3.1–M3.10 ajoutent : contrats de capability fermés, validation locale de paramètres typés, policy immutable `ALLOW`/`DENY`/`CONFIRM`, seul runner `NOOP` sous `DENY_NETWORK`, executions immuables, evidence JSON hashée, admission immutable, policy HMAC de projet sans secret persistant, preuve dérivée `PROVEN` sans mutation de knowledge, gate de work item calculée à partir d’une admission existante et validator local `EVIDENCE_HASH`. | `OBSERVED` : migrations 001→022, 148 tests et 14 sous-tests, wheel isolé et frontières de sécurité validés. | Mémoire `MEM-STATE-033`; journal `LOG-0051` à `LOG-0082`. |
| Exclusions actuelles | Aucun shell arbitraire, réseau implicite, accès fichier externe, runner additionnel, validator métier ou oracle externe, gate multi-evidence, lifecycle/traversal de work graph, CLI/MCP de production, pack/importeur ARET ou dashboard. Le sous-ensemble de paramètres ne devient pas JSON Schema général; `CONFIRM` ne devient pas une confirmation interactive; HMAC ne couvre ni rotation/révocation/expiration ni algorithme alternatif; `EVIDENCE_HASH` ne devient pas admission ou preuve. | `OBSERVED` | Mémoire `MEM-DEC-027` à `MEM-DEC-030`, `MEM-STATE-034`; journal `LOG-0082`. |
...
| Lot suivant | M3.10 a fermé le validator local `EVIDENCE_HASH`. Le prochain lot M3 candidat est un validator de contenu explicitement borné ou une gate multi-evidence, sans runner additionnel ni oracle ARET, avec baseline/hypothèse/tests propres. | `M3_IN_PROGRESS` ; `MEM-WALL-001` préservée | Journal `LOG-0082`; mémoire `MEM-STATE-034`. |
...
| `M3` | Capability / Evidence / Gates | `M3.S1` livré; M3.7 ajoute paramètres fermés, M3.8 une policy immutable `ALLOW`/`DENY`/`CONFIRM` exigée par `NOOP`, M3.9 une policy HMAC singleton de projet sans secret persistant et M3.10 le validator local `EVIDENCE_HASH`. Restent validators de contenu/oracles, runners additionnels, gates/graph avancés et surfaces CLI/MCP. | `M3.S1.EXIT` a validé migrations 001→019; M3.7–M3.10 ajoutent paramètres fermés, policy `ALLOW`, HMAC projet et validation d’intégrité locale sans généraliser JSON Schema, confirmation interactive, rotation ou oracle externe. Aucun verdict ne ferme M3 global ni la parité ARET. | I004–I008, I013, I014, I015 | `IN_PROGRESS` — `M3.S1.EXIT`, M3.7–M3.10 `PASS`; lots restants séparés |
