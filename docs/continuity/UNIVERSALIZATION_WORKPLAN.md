...
| `M11-D-B2-contract` — Journalisation et exécution bornée des mouvements de racines | `PASS` dans le périmètre borné | Le preview/journal expose `workspace_moves`; l’exécuteur et la reprise déplacent/rollbackent les racines isolées same-filesystem après vérifications exactes. Les sources englobant projet/Profile/runtime et les migrations inter-filesystems restent refusées. |
| `M11-D-B2-strategy` — Classification inter-filesystems fail-closed | `PASS` préparatoire | Le preview/journal classe `RENAME_ATOMIC` ou `COPY_VERIFY_SWITCH` à partir des devices réels. L’exécuteur refuse encore `COPY_VERIFY_SWITCH` tant que la copie vérifiée et sa reprise ne sont pas livrées. |
| `M11-D-B2-copy` — Primitive de copie vérifiée | `PASS` préparatoire | `_copy_tree_verified` copie les fichiers réguliers, vérifie hash/taille, refuse les symlinks et supprime la cible partielle en erreur. La primitive reste non branchée tant que le journal de progression et la reprise inter-filesystems ne sont pas complets. |
| `M11-D-B2-copy-progress` — États de copie par fichier | `PASS` préparatoire | La primitive émet `COPYING` puis `VERIFIED` pour chaque fichier après contrôle hash/taille via callback interne. La persistance durable est désormais couverte par `M11-D-B2-copy-journal`; la reprise de ces états reste ouverte. |
| `M11-D-B2-copy-journal` — Persistance atomique de progression | `PASS` préparatoire | Le journal persiste `copy_progress`; `record_copy_progress()` accepte uniquement `COPYING → VERIFIED`, effectue un append atomique et refuse les transitions invalides. La reprise inter-filesystems complète reste ouverte. |
| `M11-D-B2-copy-wiring` — Branchement de la copie sur le journal | `PASS` préparatoire | `_copy_tree_verified()` persiste désormais `COPYING` avant la copie et `VERIFIED` après hash/taille. Un échec après copie laisse le journal `EXECUTING` avec progression partielle et supprime la cible partielle ; l’exécuteur principal refuse encore `COPY_VERIFY_SWITCH`. Tests ciblés : `10 passed`; régression : `622 passed, 49 subtests passed`. |
| `M11-D-B2-state-machine` — Transitions globales du journal | `PASS préparatoire` | `transition_profile_migration_state()` applique une allowlist atomique et refuse les sauts `PLANNED → SWITCHING/COMMITTED`, les états inconnus et toute transition après `COMMITTED`. Les états `PLANNED`, `COPYING`, `VERIFIED`, `SWITCHING`, `COMMITTED`, `DIVERGED`, `RECOVERY_REQUIRED` et `ROLLED_BACK` sont déclarés ; `EXECUTING` reste compatible avec la reprise same-filesystem. Tests ciblés : `12 passed`; régression : `624 passed, 49 subtests passed`. La validation d’inventaire, la bascule et la reprise inter-filesystems restent ouvertes. |
| `M11-D-B2-inventory-validation` — Validation canonique avant bascule | `PASS préparatoire` | `validate_profile_migration_inventory()` vérifie read-only les sources, cibles, hashes, tailles, séquence `COPYING → VERIFIED`, Profile cible, symlinks et fichiers inattendus. Il retourne `READY_FOR_SWITCH` seulement avec preuve complète, `RECOVERY_REQUIRED` pour les absences/non-vérifications et `DIVERGED` pour les divergences de cible. Les racines workspace restent bloquantes tant qu’elles ne sont pas validées. Tests ciblés : `14 passed`; régression : `626 passed, 49 subtests passed`. |
| `M11-D-B2-sqlite-validation` — Validation SQLite/WAL/SHM et schéma | `PASS préparatoire` | `validate_sqlite_migration_target()` compare SQLite et les sidecars `-wal`/`-shm`, vérifie hashes/tailles, `PRAGMA integrity_check`, schéma `sqlite_master` et refuse les symlinks. Les bases corrompues, schémas divergents et artefacts divergents donnent `DIVERGED`. Tests ciblés : `16 passed`; régression : `628 passed, 49 subtests passed`. Le checkpoint opérationnel et le branchement exécuteur restent ouverts. |
| `M11-D-B2-sqlite-gate` — Intégration SQLite dans la validation d’inventaire | `PASS préparatoire` | Les entrées `kind=sqlite` de `validate_profile_migration_inventory()` sont désormais contrôlées par `validate_sqlite_migration_target()`. Une corruption, une divergence de schéma ou un artefact ambigu empêche `READY_FOR_SWITCH`. Tests ciblés : `16 passed`; régression : `628 passed, 49 subtests passed`. Le checkpoint WAL et la copie opérationnelle restent ouverts. |
| `M11-D-B2-sqlite-preparation` — Checkpoint et copie journalisée SQLite/WAL/SHM | `PASS préparatoire` | `prepare_sqlite_migration_artifacts()` effectue le checkpoint WAL, copie SQLite et les sidecars présents via fichiers temporaires, vérifie hash/taille, journalise `COPYING/VERIFIED`, valide la cible puis supprime toute cible partielle en cas d’interruption. Tests ciblés : `18 passed`; régression : `630 passed, 49 subtests passed`. Le branchement global `SWITCHING`, les racines workspace et la reprise complète restent ouverts. |
| `M11-D-B2-executor-gate` — Exécuteur same-filesystem et preuve SQLite | `PASS dans le périmètre borné` | L’exécuteur journalise `EXECUTING`, passe par `SWITCHING` après checkpoint, déplace le runtime/racines autorisées, vérifie SQLite après déplacement et journalise `COMMITTED` seulement après preuve. Les erreurs tentent un rollback et marquent `ROLLED_BACK`; `COPY_VERIFY_SWITCH` reste refusé. Tests ciblés : `18 passed`; régression : `630 passed, 49 subtests passed`. |
| `M11-D-B2-recovery-proof` — Reprise prouvée des phases EXECUTING/SWITCHING | `PASS dans le périmètre borné` | La reprise finalise seulement après preuve du runtime cible, Profile cible, SQLite et sidecars, ou restaure vers `PLANNED` après preuve du runtime source et du backup. Runtimes simultanés, Profile divergent, SQLite invalide et racines ambiguës donnent `RECOVERY_REQUIRED`. Tests ciblés : `20 passed`; régression : `632 passed, 49 subtests passed`. |
| `M11-D-B2-workspace-inventory` — Inventaire des fichiers de racines workspace | `PASS préparatoire` | Le preview/journal persiste `workspace_inventory` avec source, cible, kind, taille et hash par fichier des racines déplacées. La validation contrôle source/cible, symlinks et entrées inattendues ; absences → `RECOVERY_REQUIRED`, divergences/ambiguïtés → `DIVERGED`. Tests ciblés : `20 passed`; régression : `632 passed, 49 subtests passed`. La copie workspace inter-filesystems et sa progression dédiée restent ouvertes. |
| `M11-D-B2-copy-verify-switch` — Copie complète inter-filesystems et bascule contrôlée | `PASS dans le périmètre implémenté` | Lorsque le preview sélectionne `COPY_VERIFY_SWITCH`, le runtime et les racines workspace sont copiés fichier par fichier avec progression `COPYING/VERIFIED`, hash/taille, validation complète avant `SWITCHING`, suppression des sources après bascule et rollback fail-closed. Avant `SWITCHING`, les cibles partielles sont nettoyées et le journal devient `ROLLED_BACK`; pendant `SWITCHING`, il devient `RECOVERY_REQUIRED`. Tests ciblés : `21 passed`; régression : `633 passed, 49 subtests passed`. |
| `SPEC-UNIVERSAL-DEV-MMU-v1` — Spécification finale versionnée | `RÉFÉRENCE` | Le document fourni `UNIVERSAL_DEV_MMU_SPECIFICATION_FINALE.md` est intégré dans `docs/` comme référence canonique d’architecture, périmètre et Definition of Done. SHA-256 : `d8e5d01b673e243e0104a30fb62328bc2a7fc650373ab91b1a103652a1737d75`. La Definition of Done globale reste distincte des sous-lots effectivement prouvés. |
...
| `M11-D-D4` — Statut migration read-only dans Dashboard | `PASS` dans le périmètre borné | `migration.status` est relayé par bridge/Tauri/React; sans journal, le résultat canonique est `NO_PENDING_MIGRATION`; avec journal, la classification Core est retournée sans réparation ni exécution. 21 tests ciblés et build React/TypeScript passants. |
...
| `M11-F-C1` — Import project-local sans symlink intermédiaire | `PASS` | Le préflight d’import refuse les fichiers symlinkés et tout chemin traversant un répertoire symlinké; la provenance reste hashée, project-bound et `OBSERVED`. |
