...
| Révision VERA-MMU | Les commits locaux `d2efe72` (séries structurelles M4-B), `41594ba` (ledger knowledge M4-C.1), `88e56d5` (import knowledge M4-C.2), `f0aad88` (ledger Core 036) et `873fad9` (projection `knowledge_source`) sont au-dessus de `e92ab70c2608bac4fa2e21e401ec9d1833f747bb`, dernière révision publiée et vérifiée avant cette nouvelle tranche. | `OBSERVED` | Registre M4 et artefacts d’audit du 26 août 2026. |
...
| Capacité actuelle | M1, M2 et M3 sont publiés dans leurs périmètres contractuels. M4-A apporte resolver runtime/WAL fail-closed, ledger entity 033, conformité `component` et import de page non fusionnel. M4-B ajoute lecteurs/projections, Core 034, conformances/préflights, collisions/autorisations, imports structurels atomiques, séries non fusionnelles et post-validations read-only. M4-C.1/C.2 ajoutent Core 035 et l’import knowledge autorisé; M4-C.3 ajoute Core 036 et la lecture/projection `knowledge_source` non écrivable. | `OBSERVED` : migrations 001→036, suite `378 passed, 14 subtests passed`, intégrations structurelles/knowledge temporaires sans écriture ARET, et wheel isolée. L’attache source réelle, les autres tables, les oracles et la parité restent hors preuve. | Registre M4 ; artefacts d’audit du 26 août 2026. |
| Exclusions actuelles | Aucun shell arbitraire, réseau implicite, accès fichier externe, runner autre que `NOOP`/`EVIDENCE_HASH`/`EVIDENCE_FIELDS`, validator métier ou oracle externe, quorum pondéré, expiration/fenêtre temporelle ou révocation de gate, pause/reprise/réouverture ou orchestration de lifecycle, CLI/MCP de production, merge, proof/promotion, pack de compatibilité complet ou dashboard. Les imports M4-A/M4-B restent non fusionnels et temporaires ; lifecycle/Front, hiérarchie complète, import sémantique, courses externes et parité restent hors contrat. | `OBSERVED` | Registre M4 ; `LOG-0173`. |
...
| Précondition toolchain ARET | La branche toolkit verrouillée fournit les sources, scripts et corpus ; le binaire a été reconstruit hors checkout et huit scripts fermés ont été exécutés. Les résultats restent des observations externes ; `winediff` échoue à `255/264`. | `OBSERVED_RESTORED` ; C07/C08 restent `IN_PROGRESS` | `artifacts/aret_toolkit_oracle_execution_2026-08-26.md`. |
| Registre M0.2 | Seize couplages documentés avec sources, frontière Core/pack, stratégie et tests de parité ; la précondition C07/C08 est restaurée en observation, mais le runner/doctor ARET et la chaîne de preuve restent absents. | `PASS` pour la cartographie ; parité `UNKNOWN` | [Matrice](../DECOUPLING_MATRIX.md) ; artefact d’exécution du 26 août 2026. |
| Lot suivant | M4.EXIT a été audité `NOT_ELIGIBLE`. La référence ARET attestée est désormais disponible ; la progression légitime consiste à définir test-first la capability fermée, la déclaration de dépendances, le normaliseur et la chaîne d’admission VERA, puis à résoudre ou qualifier contractuellement les neuf divergences Wine. Les limites M4-B lifecycle/Front, hiérarchie, conflits/courses et les dépendances M5/M6 sont inchangées. | `M4_IN_PROGRESS` ; parité `UNKNOWN` | Registre M4 ; artefacts d’audit et d’exécution du 26 août 2026. |
...
| `M4` | Pack ARET et primitive Core | M4.1–M4.15 livrent les surfaces déclaratives et le premier write-path. M4-A ajoute resolver/WAL read-only, ledger entity 033, conformité `component` et autorisation de page non fusionnelle. M4-B livre lecteurs/projections structurels, conformances/préflights, collision/autorisation, imports atomiques/post-validations, séries non fusionnelles et le Core 034 de batch `SYMBOL`/`WORK_ITEM`, audité/append-only/idempotent, sans import legacy implicite. M4-C.1/C.2 ajoutent le Core 035 de batch `knowledge`, son lecteur/projection et l’import source autorisé sans promotion. Conformance globale, autres tables, capabilities, toolchain, MCP/hooks/bundles/VCS et parité restent dans le [registre de clôture M4](M4_COMPLETION_REGISTER.md). | Intégration temporaire réelle : 17 components/17 liens, 9 symbols en 3 pages, 13 work items en 5 pages et 532 knowledge en 6 pages, avec replays sans écriture. Core status `PLANNED` préservé pour les bricks; knowledge `SUPERSEDED` conservée `OBSERVED` avec métadonnées legacy; zéro evidence/proof/admission/promotion. Suite `372 passed, 14 subtests passed`, scans et roue isolée passent. La parité ARET reste mesurée uniquement contre une baseline exécutable. | I002, I004, I006–I008, I015 | `IN_PROGRESS` — M4-A `PASS` borné ; M4-B séries réelles observées; M4-C knowledge réel observé sans lineage, mais C01–C06/C16 `SPLIT`, C07/C08 `IN_PROGRESS` après restauration observée de la toolchain, runner/doctor/normalisation VERA absents, parité `UNKNOWN` et `M4.EXIT` `NOT_ELIGIBLE` |
