...
| Capacité actuelle | Core d’identité, substrate SQLite, entités, relations et knowledge append-only : migrations checksumées, metadata/audit, transaction, types déclaratifs, entités JSON, arêtes immuables et assertions hashées à lecture exacte. | `OBSERVED` : 57 tests et 14 sous-tests passent ; wheel isolé et knowledge observée validée. | Journal `LOG-0017` ; mémoire `MEM-STATE-010`. |
| Exclusions actuelles | Pas encore de `PROVEN`/evidence, FTS/FIND, supersession knowledge, traversal/lifecycle relationnel, symbol, bundle, serveur MCP de production, Capability Engine, importeur ARET ou dashboard. | `OBSERVED` | Mémoire `MEM-STATE-010` ; journal `LOG-0017`. |
...
| Lot suivant | Aucun sous-lot actif ; M2.5 ne peut être armé que par un rituel séparé après vérification du commit M2.4. | `READY_FOR_M2.5_RITUAL` ; `MEM-WALL-001` préservée | Journal `LOG-0017` ; mémoire section 7. |
...
| `M2` | Persistence universelle | M2.1 : substrate SQLite. M2.2 : types et entités. M2.3 : types relationnels et arêtes. M2.4 : types knowledge et assertions append-only hashées. Sous-lots ultérieurs : Evidence Store et contraintes métier restantes. | M2.1/M2.2/M2.3/M2.4 validés ; `PROVEN`/evidence, FTS/FIND, supersession, traversal/lifecycle, intégrité référentielle étendue et audit métier restent à compléter par sous-lots. | I001–I006, I010, I011, I014, I015 | `IN_PROGRESS` — M2.1/M2.2/M2.3/M2.4 `PASS`; M2 complet `UNKNOWN` |
