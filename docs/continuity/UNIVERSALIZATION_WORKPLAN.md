...
| Capacité actuelle | Core d’identité et substrate SQLite : Profile/identités hashés, URI strictes, roots/runtime confinés, migrations checksumées, metadata/audit techniques, transaction et `vmmu init`. | `OBSERVED` : 31 tests et 14 sous-tests passent ; wheel isolé et `vmmu init` validés. | Journal `LOG-0011` ; mémoire `MEM-STATE-007`. |
| Exclusions actuelles | Pas encore de services knowledge/entity/relation/evidence/bundle, serveur MCP de production, Capability Engine, importeur ARET ou dashboard. | `OBSERVED` | Mémoire `MEM-STATE-007` ; journal `LOG-0011`. |
...
| Lot suivant | Aucun sous-lot actif ; M2.2 ne peut être armé que par un rituel séparé après vérification du commit M2.1. | `READY_FOR_M2.2_RITUAL` ; `MEM-WALL-001` préservée | Journal `LOG-0011` ; mémoire section 7. |
...
| `M2` | Persistence universelle | M2.1 : substrate SQLite (migrations checksumées, identity binding, audit technique, transaction). Sous-lots ultérieurs : services génériques et contraintes métier. | M2.1 validé ; migrations checksumées, intégrité référentielle et audit métier restent à compléter par sous-lots. | I001–I006, I010, I011, I014, I015 | `IN_PROGRESS` — M2.1 `PASS`; M2 complet `UNKNOWN` |
