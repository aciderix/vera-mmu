...
| Capacité actuelle | M1 et M2 sont publiés. M3.S1 et M3.S2 sont des tranches validées. M3.1–M3.17 ajoutent : contrats de capability fermés, validation locale de paramètres typés, policy immutable `ALLOW`/`DENY`/`CONFIRM`, runners locaux `NOOP` et `EVIDENCE_HASH` sous `DENY_NETWORK`, executions immuables, evidence JSON hashée, admission immutable avec policy optionnelle stricte, policy HMAC de projet sans secret persistant, preuve dérivée `PROVEN` sans mutation de knowledge, gates de work item à une ou plusieurs evidences admises avec policy optionnelle `ALL`/`ANY`/`AT_LEAST`, validator local `EVIDENCE_HASH`, lifecycle de work item dérivé par événements et readiness dérivée avec policy de démarrage optionnelle `OPEN`/`REQUIRE_READY` et validator local `EVIDENCE_FIELDS`. | `OBSERVED` : migrations 001→029, 170 tests et 14 sous-tests, wheel isolé et frontières de sécurité validés. | Mémoire `MEM-STATE-047`; journal `LOG-0051` à `LOG-0100`. |
...
| Lot suivant | M3.17 ajoute un validator local de présence de clés, sans oracle métier. Le prochain lot M3 candidat est un validator de contenu/oracle explicitement borné, un runner sûr additionnel ou un lifecycle/graph avancé sous contrat distinct, sans oracle ARET implicite, avec baseline/hypothèse/tests propres. Toute nouvelle tranche devra avoir une sortie distincte. | `M3_IN_PROGRESS` ; `MEM-WALL-001` préservée | Journal `LOG-0102`; mémoire `MEM-STATE-050`. |
...
| `M3` | Capability / Evidence / Gates | `M3.S1` livré; `M3.S2` valide l’intégration de M3.7–M3.12; M3.13 ajoute la policy d’admission `PASS_EVIDENCE`/`VALIDATED_PASS_EVIDENCE` sans validation implicite; M3.14 ajoute le runner local fermé `EVIDENCE_HASH`; M3.15 ajoute les policies de gate `ALL`/`ANY`/`AT_LEAST` lues sur admissions existantes; M3.16 ajoute une readiness dérivée et un démarrage `REQUIRE_READY` optionnel; M3.17 ajoute `EVIDENCE_FIELDS`. Restent validators de contenu/oracles, runners additionnels, gates pondérées/temporelles, lifecycle/graph avancés et surfaces CLI/MCP. | `M3.S1.EXIT` a validé migrations 001→019; `M3.S2.EXIT` valide migrations 001→024; M3.13 valide migration 025; M3.14 valide migration 026; M3.15 valide migration 027; M3.16 valide migration 028; M3.17 valide migration 029, suite complète et wheel isolé `EVIDENCE_FIELDS`, sans généraliser JSON Schema, confirmation interactive, rotation, pondération/expiration, orchestration ou oracle externe. Aucun verdict ne ferme M3 global ni la parité ARET. | I004–I008, I013, I014, I015 | `IN_PROGRESS` — `M3.S1.EXIT`, `M3.S2.EXIT`, M3.13 à M3.17 `PASS`; lots restants séparés |
