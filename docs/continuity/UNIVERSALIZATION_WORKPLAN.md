...
| Révision VERA-MMU | Les tranches M4-D et la correction documentaire sont publiées jusqu’à `44c928b`. Le jalon fonctionnel local `5ffe182` porte une façade MCP stdio VERA, son entry point et une matrice client réelle. | `OBSERVED` ; Core, Pack et façade restent séparés. | Registre M4, `LOG-0182` et artefact M5-A du 26 août 2026. |
...
| Capacité actuelle | M1, M2 et M3 fournissent le Core; M4-D transporte les verdicts Pack→Core; M5-A expose la même chaîne via un vrai serveur/client MCP stdio. | `OBSERVED` : migrations 001→038, suite `399 passed, 32 subtests passed`, roue isolée. Les payloads contractuels `272/272`, `271/272`, timeout, sortie inconnue, dépendance absente, hash non promouvable et asset altéré atteignent admission/gate seulement pour `PASS` validé. Le score effectivement obtenu par ARET est hors de cette conformance VERA. | `LOG-0182`; artefacts M4-D et M5-A du 26 août 2026. |
| Exclusions actuelles | Aucun shell arbitraire, réseau implicite, accès fichier externe libre, validator métier, admission/proof/promotion automatique, quorum pondéré, expiration/fenêtre temporelle ou révocation de gate, pause/reprise/réouverture ou orchestration de lifecycle, manifest/compiler MCP, adapter de production configuré, CLI complète, merge, pack de compatibilité complet ou dashboard. La façade MCP M5-A existe mais refuse toute execution sans adapter explicite; le seul runner externe est `OBSERVED_PROCESS`, qui n’exécute rien dans le Core ; le pack ARET ne lance que son catalogue fermé sous namespace réseau isolé. Les imports M4-A/M4-B restent non fusionnels et temporaires. | `OBSERVED` | `LOG-0182`; registre M4 et tests du runner fermé. |
...
| Lot suivant | M4.EXIT reste `NOT_ELIGIBLE` pour ses imports, playbook, bundles/VCS et compatibilités restants. M5-A a validé le transport client MCP; le prochain sous-lot M5 compile un manifeste immuable et configure des adapters/instructions sans laisser le client injecter commande, verdict ou artefact. Les divergences Wine restent des observations ARET distinctes. | `M5_IN_PROGRESS` ; parité ARET hors conformance de transport | `LOG-0182`; contrat MCP M5-A du 26 août 2026. |
...
| `M5` | Compilateur MCP et adapters | M5-A livre la façade MCP stdio, l’API minimale stable et la matrice client→execution→evidence→admission→gate. Restent manifeste immuable, `mcp_build_hash`, compilation d’adapters/instructions/hooks/config et snapshots. | Même entrée = même `mcp_build_hash`; vrai serveur/client MCP, snapshots et matrice client→execution→evidence→admission→gate passent sans injection client de commande/verdict/artefact. | I007–I009, I012–I014 | `IN_PROGRESS` — M5-A `PASS` (`5ffe182`), compilateur/adapters de production `PLANNED` |
