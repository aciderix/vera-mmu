...
>
> **Dernière revue factuelle :** 26 août 2026 — M4-A étend M4.15 par un ledger d’import générique, une conformité profonde read-only de `component` et un import de pages ARET explicitement autorisées. M4 reste `IN_PROGRESS`, les couplages restent `SPLIT` et la parité `UNKNOWN`.
>
...
| Révision VERA-MMU | `3f21200cc0ca31119e752b5a785dc54170fa15ce` sur `main`, checks de collision non-fusionnelle et autorisations structurelles zéro-write M4-B publiés après les préflights et le Core 034. | `OBSERVED` | `LOG-0165`, `LOG-0167`, `LOG-0168`, `LOG-0170`; commits Core/packs publiés et vérifiés. |
...
| Capacité actuelle | M1, M2 et M3 sont publiés dans leurs périmètres contractuels. M4-A apporte le resolver runtime read-only/WAL fail-closed, le ledger entity 033, la conformité `component` et l’import de page non fusionnel. M4-B apporte lecteurs/projections read-only/purs, le Core 034, les conformances/préflights et les checks/permissions explicites qui lient la cible sans exécuter d’import. | `OBSERVED` : migrations 001→034, suite `349 passed, 14 subtests passed`, import baseline de 17 composants/17 liens/replay sans écriture dans une cible temporaire, et M4-B validé par conformances source, préflights zéro-write, collision/autorisation zéro-write, Core 034, replay, rollback, migration 033→034, immutabilité et wheel isolée ; zéro evidence/proof link créée par ces paths. | Registre M4 ; `LOG-0165`. |
| Exclusions actuelles | Aucun shell arbitraire, réseau implicite, accès fichier externe, runner autre que `NOOP`/`EVIDENCE_HASH`/`EVIDENCE_FIELDS`, validator métier ou oracle externe, quorum pondéré, expiration/fenêtre temporelle ou révocation de gate, pause/reprise/réouverture ou orchestration de lifecycle, CLI/MCP de production, import ARET hors pages `component` explicitement autorisées M4-A, merge, proof/promotion, pack de compatibilité complet ou dashboard. Le Core 034, les préflights et les autorisations zéro-write ne constituent pas un import de `function_symbol` ni de `brick`; les write-paths et post-validations restent requis. | `OBSERVED` | Registre M4 ; `LOG-0165`–`LOG-0166`. |
...
| Lot suivant | M4-B reste ouvert. Les prochains contrats sont les write-paths atomiques et les post-validations read-only, séparés pour `function_symbol` et `brick`, qui doivent consommer les autorisations exactes et le Core 034 sans SQL d’écriture de pack. Aucun verdict ne clôture C01–C16, `MEM-WALL-001` ou la parité ARET. | `M4_IN_PROGRESS` ; `MEM-WALL-001` préservée | Registre M4 ; `LOG-0171`. |
...
| `M4` | Pack ARET et primitive Core | M4.1–M4.15 livrent les surfaces déclaratives et le premier write-path. M4-A ajoute resolver/WAL read-only, ledger entity 033, conformité `component` et autorisation de page non fusionnelle. M4-B livre lecteurs/projections structurels, conformances/préflights, collision/autorisation zéro-write et le Core 034 de batch `SYMBOL`/`WORK_ITEM`, audité/append-only/idempotent, sans import legacy implicite. Conformance globale, contrats propres d’import structurel, autres tables, capabilities, toolchain, MCP/hooks/bundles/VCS et parité restent dans le [registre de clôture M4](M4_COMPLETION_REGISTER.md). | M4-A : import réel de 17 composants/17 liens/replay sans écriture; Core 034 + prérequis M4-B : tests-first, conformance/préflight/collision/autorisation, replay/rollback/fingerprint, upgrade 033→034, immutabilité, savepoints, suite `349 passed, 14 subtests passed`, scans et wheel isolée. Zéro evidence/proof/admission/promotion dans le nouveau path. La parité ARET reste mesurée uniquement contre une baseline exécutable. | I002, I004, I006–I008, I015 | `IN_PROGRESS` — M4-A `PASS` borné ; M4-B prérequis d’écriture seulement ; C01–C06/C16 `SPLIT`, C07/C08 `BLOCKED`, parité `UNKNOWN` |
