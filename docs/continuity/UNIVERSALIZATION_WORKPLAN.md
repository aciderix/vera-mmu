...
| Wall active | Toolchain des oracles ARET indisponible : exécutables, binaire, scripts/corpus et build ARET nécessaires sont absents du checkout et des références publiques accessibles; ne pas confondre `SKIPPED` et validation. | `BLOCKED — MEM-WALL-001` | `artifacts/m4_exit_toolchain_restoration_check_2026-08-26.md`. |
...
| Lot suivant | M4.EXIT a été audité `NOT_ELIGIBLE`. La prochaine progression légitime exige d’abord un bundle/révision ARET attesté contenant corpus, scripts et build/binaire de référence, puis des contrats M4-C table par table et les dépendances M5/M6. Les limites M4-B lifecycle/Front, hiérarchie, conflits/courses et parité sont inchangées. | `M4_IN_PROGRESS` ; `MEM-WALL-001` préservée | Registre M4 ; artefacts d’audit du 26 août 2026. |
