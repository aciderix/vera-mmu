...
| Sous-lot | Statut | Preuve et limite |
|---|---|---|
| `M11-D-B2` — Préflight/preview, journal préparatoire et inspection de reprise Profile/runtime | `PASS` dans le périmètre borné | `src/vera_mmu/profile_migration.py` valide les profils source/cible, chemins confinés, symlinks, collisions workspace/runtime, inventaire et hashes avant/après, persiste atomiquement un journal `PLANNED` hors runtime après confirmation (`mutation: JOURNAL_ONLY`) et classe read-only les états `READY_FOR_EXECUTOR`/`DIVERGED` (`mutation: NONE`). Aucun déplacement physique, rollback ou reprise n’est encore implémenté. |
