...
| Révision VERA-MMU | `e568cd5fe8bda80b4d9434836a9173ad0195d9f0` sur `main`, commit M2.10 publié et vérifié ; le record de publication est consigné dans le commit documentaire suivant. | `OBSERVED` | Journal `LOG-0033`, `LOG-0034`. |
...
| Capacité actuelle | Core d’identité, substrate SQLite, entités, relations, knowledge append-only, provenance déclarative, supersession directe, assets binaires hashés, associations knowledge–asset, index direct borné de ces associations et provenance documentaire déclarative des assets : migrations checksumées, metadata/audit, transactions, objets typés immuables, assertions hashées, sources bornées, bytes revérifiés avant lecture et références source d’asset sans contenu. | `OBSERVED` : 96 tests et 14 sous-tests passent ; wheel isolé et registre de sources d’asset validés. | Journal `LOG-0033` ; mémoire `MEM-STATE-016`. |
| Exclusions actuelles | Pas encore de chemin/fichier externe, fetch, réseau, import/export, comparaison source↔asset, bundle, mutation de statut ou `SUPERSEDED`, version counter, `PROVEN`/evidence, execution/validator, gate, FTS/FIND, recherche libre, traversal multi-sauts, lecture d’asset par liaison ou source, intégration à `RelationService`, symbol, serveur MCP de production, Capability Engine, importeur ARET ou dashboard. | `OBSERVED` | Mémoire `MEM-STATE-016` ; journal `LOG-0033`. |
...
| Lot suivant | Aucun sous-lot actif : M2.10 est publié. M2.11 exige un rituel distinct et une hypothèse explicitement bornée. | `READY_FOR_M2.11_RITUAL` ; `MEM-WALL-001` préservée | Journal `LOG-0033`, `LOG-0034` ; mémoire section 7. |
