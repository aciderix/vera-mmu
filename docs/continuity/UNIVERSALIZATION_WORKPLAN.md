...
| `M6` | CLI et Dashboard | **M6-A livré :** façade `vmmu adapter` avec matrix, doctor observationnel, validate, stage et configure project-local. Le dashboard, generate/install automatisés et éditeur visuel restent des tranches séparées. | M6-A route les contrôles des adapters sans pollution du Core, conserve confirmations et refuse toute voie user-scope générale ; suite, roue et scans passent. | I002, I011, I013, I014 | `IN_PROGRESS` — M6-A `PASS` (`17a2bba`, `LOG-0203`); dashboard et opérations visuelles `NOT_DELIVERED` |
