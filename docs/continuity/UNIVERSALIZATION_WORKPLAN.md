...
| `M11-D-B2-copy-progress` — États de copie par fichier | `PASS` préparatoire | La primitive émet `COPYING` puis `VERIFIED` pour chaque fichier après contrôle hash/taille via callback interne. La persistance durable et la reprise de ces états restent ouvertes. |
