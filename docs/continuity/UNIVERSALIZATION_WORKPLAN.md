...
| `M9` | Release contrôlée | Packaging, migration, documentation dérivée, hashes/signatures et garanties de compatibilité. | Definition of Done globale satisfaite et rapport d’écarts nul ou accepté explicitement. | Tous | `IN_PROGRESS` — M9-A/M9-B `PASS` : candidats CLI + desktop, workflow/assembleur, README, Apache-2.0, DCO et policy de marque ; suite `510 passed, 43 subtests passed` et run `33070861267` vert Linux/Windows. Version `0.1.0-rc.1` alignée (Python PEP 440 `0.1.0rc1`), frontend et Rust build/check `PASS`. Exception autorisée : préversion gratuite non signée après run de tag exact ; signature reste requise pour stable/diffusion large. GitHub Release, viewer versionné, installation utilisateur et hôtes réels `NOT_RUN`/`PENDING`. |
