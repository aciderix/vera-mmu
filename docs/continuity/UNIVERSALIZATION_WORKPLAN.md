...
| Capacité actuelle | Core d’identité, substrate SQLite, entités, relations, knowledge append-only, provenance déclarative, supersession directe, assets binaires hashés, associations knowledge–asset, index direct borné de ces associations, provenance documentaire déclarative des assets, index exact borné de sources knowledge par hash et symboles déclaratifs immuables rattachés à une entity : migrations checksumées, metadata/audit, transactions et objets typés immuables. | `OBSERVED` : 109 tests et 14 sous-tests passent ; wheel isolé et Symbol Registry validés. Le candidat d’index d’assets par hash reste rejeté comme redondant. | Journal `LOG-0036`, `LOG-0038`, `LOG-0042` ; mémoire `MEM-STATE-019`. |
| Exclusions actuelles | Pas encore de chemin/fichier externe, fetch, réseau, import/export, comparaison source↔asset, bundle, mutation de statut ou `SUPERSEDED`, version counter, `PROVEN`/evidence, execution/validator, gate, FTS/FIND, recherche libre, traversal multi-sauts, lecture de knowledge/document par index, lecture d’asset par liaison ou source, intégration à `RelationService`, scan/résolution de symboles, work item, serveur MCP de production, Capability Engine, importeur ARET ou dashboard. | `OBSERVED` | Mémoire `MEM-STATE-019`, `MEM-DEC-022` ; journal `LOG-0042`. |
...
| Lot suivant | M2.12 est validé techniquement. Après son commit et sa publication, le prochain travail technique sera `M2.13 — Work-Item Backbone`, avec baseline et hypothèse autonomes. | `READY_FOR_M2.13_RITUAL` ; cadence en macro-lots ; `MEM-WALL-001` préservée | Journal `LOG-0042` ; mémoire `MEM-STATE-020`. |
...
| `M2` | **Universal Schema** fini | Livré : M2.1–M2.12 (substrate, entités/relations, knowledge/provenance/supersession, assets/associations et Symbol Registry). Reste : `M2.13 Work-Item Backbone`, `M2.14 Capability Declaration & Execution Schema`, puis `M2.EXIT`. Chaque lot regroupe schéma, service, migrations et tests de sa ressource ; aucun index isolé n’est une fin de lot. | **M2.EXIT** : toutes les six ressources de la spécification M2 existent avec migrations vérifiées, services de persistance exacts et bornés, FKs/immuabilité/audit/rollback testés ; fresh install et upgrade 001→courant, tests complets, wheel isolé, scans anti-ARET/no-shell/no-network/no-`PROVEN` passent. La gate ne revendique ni parité ARET ni preuve métier. | I001–I006, I010, I011, I014, I015 ; frontière I004, I007–I008, I013 | `IN_PROGRESS` — M2.1–M2.12 `PASS`; M2.13–M2.14 et M2.EXIT `PLANNED`; parité ARET `UNKNOWN` |
