...
>
> **Dernière revue factuelle :** 26 août 2026 — M4.3 validé; M4 est `IN_PROGRESS`, les couplages restent `SPLIT` et la parité `UNKNOWN`.
>
...
| Révision VERA-MMU | `b98058c0365131ff4070a4d3c9c248a00a48d475` sur `main`, manifeste du schéma ARET V1 M4.3 publié et vérifié. | `OBSERVED` | Journal `LOG-0128`. |
...
| Lot suivant | M4 est ouvert. M4.1–M4.3 sont `PASS`; le prochain lot doit cibler un couplage de la matrice sous un contrat séparé. Aucun verdict ne clôture C01–C16, `MEM-WALL-001` ou la parité ARET. | `M4_IN_PROGRESS` ; `MEM-WALL-001` préservée | Journal `LOG-0127`; mémoire `MEM-STATE-074`. |
...
| `M4` | Pack ARET | M4.1 livre le lecteur `ARET://` V1 strictement canonique; M4.2 déclare le layout runtime; M4.3 inventorie le schéma applicatif V1 observé. Import hors ligne, mappings, pipelines, playbook, toolchain et parité restent à contractualiser séparément. | M4.1–M4.3 : surfaces fermées et déclaratives, Core sans dépendance pack, refus non-canonique, aucune I/O runtime ou lecture SQLite, suite Core et wheels. La parité ARET reste mesurée uniquement contre une baseline exécutable. | I002, I004, I006–I008, I015 | `IN_PROGRESS` — M4.1–M4.3 `PASS`; C01–C06/C16 `SPLIT`, parité `UNKNOWN` |
