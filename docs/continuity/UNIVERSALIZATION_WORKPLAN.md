...
| Capacité actuelle | M1 et M2 sont publiés. La tranche M3.1–M3.6 ajoute : contrats de capability fermés, seul runner `NOOP` sous `DENY_NETWORK`, executions immuables, evidence JSON hashée, admission immutable, preuve dérivée `PROVEN` sans mutation de knowledge et gate de work item calculée à partir d’une admission existante. | `OBSERVED` : migrations 001→019, 139 tests et 14 sous-tests, wheel isolé et frontières de sécurité validés. | Mémoire `MEM-STATE-025`; journal `LOG-0051` à `LOG-0069`, `LOG-0070`. |
| Exclusions actuelles | Aucun shell arbitraire, réseau implicite, accès fichier externe, runner additionnel, validation typée complète de paramètres, policy `ALLOW`/`DENY`/`CONFIRM`, validator framework, policy HMAC de projet, gate multi-evidence, lifecycle/traversal de work graph, CLI/MCP de production, pack/importeur ARET ou dashboard. | `OBSERVED` | Mémoire `MEM-STATE-026`; journal `LOG-0070`. |
...
| Lot suivant | `M3.S1.EXIT` contrôle la tranche livrée sans fermer M3. Ensuite, ouvrir un lot M3 unique parmi les exclusions documentées, avec baseline/hypothèse/tests propres. | `M3_IN_PROGRESS` ; `MEM-WALL-001` préservée | Journal `LOG-0070`; mémoire `MEM-DEC-026`, `MEM-STATE-026`. |
...
| `M3` | Capability / Evidence / Gates | `M3.S1` livré : contrats fermés, runner `NOOP` sous `DENY_NETWORK`, executions, evidence hashée, admission, preuve dérivée `PROVEN`, dépendances directes et gates mono-evidence. Restent validation typée, policies, validators, runners additionnels, HMAC de projet, gates/graph avancés et surfaces CLI/MCP. | `M3.S1.EXIT` a validé migrations 001→019, tests ciblé/complet, wheel isolé, absence de shell arbitraire/réseau implicite, chaîne execution→evidence `PASS`→admission→preuve dérivée et gate d’admission. Ce verdict ne ferme pas M3 global ni la parité ARET. | I004–I008, I013, I014, I015 | `IN_PROGRESS` — `M3.S1.EXIT` `PASS`; lots restants séparés |
