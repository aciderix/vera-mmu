...
| `M9` | Release contrôlée | Packaging, migration, documentation dérivée, hashes/signatures et garanties de compatibilité. | Definition of Done globale satisfaite et rapport d’écarts nul ou accepté explicitement. | Tous | `IN_PROGRESS` — M9-A `PASS` : contrat, modèle de notes, builder CLI natif, candidat Linux extrait et run `33067150688` vert (Linux/Windows : `508 passed, 43 subtests passed`, CLI + desktop bundles). Licence, signatures, tag/release, viewer versionné, installation utilisateur et hôtes réels `NOT_RUN`/`PENDING`. |
