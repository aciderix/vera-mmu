...
| Capacité actuelle | M1 et M2 sont publiés. M3.S1 et M3.S2 sont des tranches validées. M3.1–M3.15 ajoutent : contrats de capability fermés, validation locale de paramètres typés, policy immutable `ALLOW`/`DENY`/`CONFIRM`, runners locaux `NOOP` et `EVIDENCE_HASH` sous `DENY_NETWORK`, executions immuables, evidence JSON hashée, admission immutable avec policy optionnelle stricte, policy HMAC de projet sans secret persistant, preuve dérivée `PROVEN` sans mutation de knowledge, gates de work item à une ou plusieurs evidences admises avec policy optionnelle `ALL`/`ANY`/`AT_LEAST`, validator local `EVIDENCE_HASH` et lifecycle de work item dérivé par événements. | `OBSERVED` : migrations 001→027, 164 tests et 14 sous-tests, upgrade 026→027, wheel isolé et frontières de sécurité validés. | Mémoire `MEM-STATE-045`; journal `LOG-0051` à `LOG-0098`. |
...
| Lot suivant | M3.15 ajoute une policy de gate locale, immutable et lue purement : `ALL`, `ANY` ou `AT_LEAST` sur admissions déjà persistées. Le prochain lot M3 candidat est un validator de contenu/oracle explicitement borné, un runner sûr additionnel ou une extension temporelle/pondérée de gate sous contrat distinct, sans oracle ARET implicite, avec baseline/hypothèse/tests propres. Toute nouvelle tranche devra avoir une sortie distincte. | `M3_IN_PROGRESS` ; `MEM-WALL-001` préservée | Journal `LOG-0098`; mémoire `MEM-STATE-046`. |
...
| `M3` | Capability / Evidence / Gates | `M3.S1` livré; `M3.S2` valide l’intégration de M3.7–M3.12; M3.13 ajoute la policy d’admission `PASS_EVIDENCE`/`VALIDATED_PASS_EVIDENCE` sans validation implicite; M3.14 ajoute le runner local fermé `EVIDENCE_HASH`; M3.15 ajoute les policies de gate `ALL`/`ANY`/`AT_LEAST` lues sur admissions existantes. Restent validators de contenu/oracles, runners additionnels, gates pondérées/temporelles, lifecycle/graph avancés et surfaces CLI/MCP. | `M3.S1.EXIT` a validé migrations 001→019; `M3.S2.EXIT` valide migrations 001→024; M3.13 valide migration 025; M3.14 valide migration 026; M3.15 valide migration 027, upgrade 026→027, suite complète et wheel isolé `ALL`/`ANY`/`AT_LEAST`, sans généraliser JSON Schema, confirmation interactive, rotation, pondération/expiration, orchestration ou oracle externe. Aucun verdict ne ferme M3 global ni la parité ARET. | I004–I008, I013, I014, I015 | `IN_PROGRESS` — `M3.S1.EXIT`, `M3.S2.EXIT`, M3.13 à M3.15 `PASS`; lots restants séparés |
