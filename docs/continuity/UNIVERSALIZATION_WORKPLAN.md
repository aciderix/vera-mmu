...
| `M7` | Dashboard, bridge desktop et distribution | Dashboard React statique M7-A ; bridge desktop stdio, catalogue d’adapters partagé et installation project-local prévisualisée/confirmée M7-B ; application Tauri et binaires Windows/Linux à compléter. | Aucun frontend ne contrôle chemin, shell, adapter brut, verdict ou contenu à écrire ; le sidecar stdio réutilise le Core ; Git transporte la mémoire SQLite project-local sans fusion binaire implicite. | I001, I004, I007–I011, I013–I015 | `IN_PROGRESS` — dashboard M7-A `OBSERVED` (checkpoint WebDev `f28ac0fa`), bridge M7-B `PASS` pour transport/générique (`57279e1`, `LOG-0206`), desktop packaging et hôtes réels `NOT_RUN` |
