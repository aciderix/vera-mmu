...
| Révision VERA-MMU | Les commits fonctionnels locaux `d2efe72` (séries structurelles M4-B), `41594ba` (ledger knowledge M4-C.1) et `88e56d5` (import knowledge M4-C.2) sont au-dessus de `e92ab70c2608bac4fa2e21e401ec9d1833f747bb`, seule révision publiée et vérifiée sur `origin/main` à ce stade. | `OBSERVED` | Publication antérieure vérifiée ; `LOG-0172` à `LOG-0175`. |
...
| Capacité actuelle | M1, M2 et M3 sont publiés dans leurs périmètres contractuels. M4-A apporte resolver runtime/WAL fail-closed, ledger entity 033, conformité `component` et import de page non fusionnel. M4-B ajoute lecteurs/projections, Core 034, conformances/préflights, collisions/autorisations, imports structurels atomiques, séries non fusionnelles et post-validations read-only. M4-C.1/C.2 ajoutent le ledger générique knowledge Core 035 et une chaîne knowledge autorisée sans promotion. | `OBSERVED` : migrations 001→035, suite `372 passed, 14 subtests passed`, intégration structurelle réelle bornée de 17 components, 9 symbols en 3 pages et 13 work items en 5 pages, et intégration knowledge réelle de 532 records en 6 pages; replays sans écriture, 6 batches knowledge, zéro `knowledge_source`/supersession/evidence/proof/admission. | Registre M4 ; `LOG-0172` à `LOG-0175`. |
...
| Lot suivant | M4-C a importé knowledge sous contrat; la prochaine progression autorisée est `knowledge_source` et les liens sémantiques, table par table, sans inférer de preuve ni de lineage d’un import knowledge. Les limites M4-B lifecycle/Front, hiérarchie, conflits/courses et parité sont inchangées. Aucun verdict ne clôture C01–C16, `MEM-WALL-001` ou la parité ARET. | `M4_IN_PROGRESS` ; `MEM-WALL-001` préservée | Registre M4 ; `LOG-0173` à `LOG-0175`. |
