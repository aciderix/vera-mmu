...
| Révision VERA-MMU | Les tranches M4-D et la correction documentaire sont publiées jusqu’à `44c928b`. Les jalons fonctionnels locaux `5ffe182`, `5de260d`, `50cc79a`, `e073fa2`, `9010293`, `5dab574`, `ea7235a`, `8b38b1b` et `674929c` portent façade, manifeste, registry, adapter/hôte Pack, instructions, config, hooks, plan hôte et installateur. | `OBSERVED` ; Core, Pack, façade, manifeste, registry, instructions, config, hooks, plan hôte et installateur restent séparés. | `LOG-0182` à `LOG-0190` et artefact M5 du 26 août 2026. |
...
| Lot suivant | M4.EXIT reste `NOT_ELIGIBLE` pour ses imports, playbook, bundles/VCS et compatibilités restants. M5-A/B/C/D/E/F/G/H/I ont validé transport, manifeste, registry, adapter Pack, instructions, config, hook plan, plan hôte et installateur MCP. Un adapter exécutable de hook reste séparé et ne doit pas être induit de l’installation M5-I. Les divergences Wine restent des observations ARET distinctes. | `M5_IN_PROGRESS` ; parité ARET hors conformance de transport | `LOG-0182` à `LOG-0190`; contrat MCP M5-A/B/C/D/E/F/G/H/I du 26 août 2026. |
...
| `M5` | Compilateur MCP et adapters | M5-A livre la façade MCP stdio et la matrice client→execution→evidence→admission→gate; M5-B livre le manifeste immuable, `mcp_build_hash` et sa vérification; M5-C livre le registry d’objets hôte; M5-D livre l’adapter et l’hôte ARET Pack; M5-E livre les instructions générées et vérifiées; M5-F la config prévisualisée; M5-G le plan SessionStart; M5-H l’adapter de revue Claude Code; M5-I l’installateur MCP opt-in. Adapter de hook exécutable à cadrer séparément. | Même entrée = mêmes `mcp_build_hash`, `instructions_hash`, `config_hash`, `hook_plan_hash` et `plan_hash`; vrai serveur/client MCP, snapshots et matrice client→execution→evidence→admission→gate passent sans injection client de commande/verdict/artefact. | I007–I009, I012–I014 | `IN_PROGRESS` — M5-A `PASS` (`5ffe182`), M5-B `PASS` (`5de260d`), M5-C `PASS` (`50cc79a`), M5-D `PASS` (`e073fa2`), M5-E `PASS` (`9010293`), M5-F `PASS` (`5dab574`), M5-G `PASS` (`ea7235a`), M5-H `PASS` (`8b38b1b`), M5-I `PASS` (`674929c`), hook exécutable `PLANNED` |
