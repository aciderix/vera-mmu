...
| Révision VERA-MMU | Les commits antérieurs M4-B/M4-C et le commit documentaire `2b98c41` ont été publiés. La tranche locale suivante ajoute `124670e` (contrat d’oracles Pack), `d204d28` (runner Core générique et migration 037) et `e5e7ca1` (runner ARET fermé). | `OBSERVED` ; publication de cette nouvelle tranche non demandée. | Registre M4 et artefacts d’intégration du 26 août 2026. |
...
| Capacité actuelle | M1, M2 et M3 sont publiés dans leurs périmètres contractuels. M4-A/B/C restent inchangés. M4-D ajoute le validator Core `EVIDENCE_ASSET`, qui lie génériquement evidence, asset et execution; le Pack conserve catalogue, préflight et runner ARET fermé. | `OBSERVED` : migrations 001→038, suite `391 passed, 21 subtests passed`, roue isolée. Un `difftest 272/272` réel a parcouru normalization→asset→validation→admission stricte→proof HMAC→gate dans un runtime temporaire; les non-`PASS` et assets altérés sont refusés. Doctor, corpus Wine global et parité restent hors preuve. | Registre M4 ; `artifacts/m4d_universal_verdict_chain_2026-08-26.md`. |
| Exclusions actuelles | Aucun shell arbitraire, réseau implicite, accès fichier externe libre, validator métier, admission/proof/promotion automatique, quorum pondéré, expiration/fenêtre temporelle ou révocation de gate, pause/reprise/réouverture ou orchestration de lifecycle, CLI/MCP de production, merge, pack de compatibilité complet ou dashboard. Le seul runner externe est `OBSERVED_PROCESS`, qui n’exécute rien dans le Core ; le pack ARET ne lance que son catalogue fermé sous namespace réseau isolé. Les imports M4-A/M4-B restent non fusionnels et temporaires. | `OBSERVED` | Registre M4 ; tests du runner fermé et `LOG-0173`. |
...
| Précondition toolchain ARET | La branche toolkit verrouillée fournit sources/scripts/corpus ; le binaire externe SHA-256 est épinglé. Le préflight Pack vérifie commit, propreté, scripts, dépendances et sandbox sans installer. `difftest 272/272 PASS` a parcouru validator asset/execution, admission stricte, proof HMAC et gate dans un runtime temporaire; la fixture Wine reste `PASS 1/1`. Le corpus Wine historique reste `FAIL 255/264` et le corpus sandboxé complet bloque sur `win32_winsock`. | `OBSERVED_RESTORED` ; C07/C08 restent `IN_PROGRESS` | `artifacts/m4d_universal_verdict_chain_2026-08-26.md`. |
...
| Lot suivant | M4.EXIT reste `NOT_ELIGIBLE`. Le mécanisme validator→admission/proof/gate est maintenant démontré sur un `PASS` réel et les non-`PASS` sont refusés. La progression légitime est de versionner doctor/image/corpus, de produire des evidence réelles `FAIL`/`ERROR`/`SKIPPED` contrôlées, puis de diagnostiquer `win32_winsock` et les neuf divergences Wine sans filtrer de fixture ni transformer un run interrompu en résultat. Les limites M4-B/C et les dépendances M5/M6 sont inchangées. | `M4_IN_PROGRESS` ; parité `UNKNOWN` | Registre M4 ; artefact de chaîne universelle du 26 août 2026. |
...
| `M4` | Pack ARET et primitive Core | M4.1–M4.15 livrent les surfaces déclaratives et le premier write-path. M4-A/B/C restent décrits au registre. M4-D ajoute catalogue fermé, préflight, `OBSERVED_PROCESS` générique, runner ARET commit/binaire/sandbox-bound, validator Core asset/execution et chaîne d’admission/proof/gate strictement liée. Conformance globale, autres tables, doctor, MCP/hooks/bundles/VCS et parité restent dans le [registre de clôture M4](M4_COMPLETION_REGISTER.md). | Intégrations temporaires d’import inchangées. Pipeline réel M4-D : `difftest 272/272` a produit asset/execution/evidence, validation `PASS`, admission `ADMITTED`, proof `PROVEN` HMAC et gate `PASS` dans un runtime temporaire; `FAIL`/`ERROR`/`SKIPPED`/`UNKNOWN` et asset altéré restent refusés. Suite `391 passed, 21 subtests passed`, scans et roue isolée passent. Le corpus Wine historique reste `255/264 FAIL`; le run sandboxé global est non terminé. | I002, I004, I006–I008, I015 | `IN_PROGRESS` — M4-A `PASS` borné ; M4-B/C observés, C01–C06/C16 `SPLIT`, C07/C08 `IN_PROGRESS` avec chaîne universelle bornée, parité `UNKNOWN` et `M4.EXIT` `NOT_ELIGIBLE` |
