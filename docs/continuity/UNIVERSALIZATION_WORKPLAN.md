...
| `M9` | Release contrôlée | Packaging, migration, documentation dérivée, hashes/signatures et garanties de compatibilité. | Definition of Done globale satisfaite et rapport d’écarts nul ou accepté explicitement. | Tous | `IN_PROGRESS` — M9-A : contrat de release, modèle de notes et builder CLI natif versionné ; `4 passed`, suite `508 passed, 43 subtests passed`; candidat Linux local (archive/hash/manifest/scan extrait) `PASS`. CI Windows/Linux, licence, signatures, tag/release, viewer versionné et hôtes réels `NOT_RUN`/`PENDING`. |
