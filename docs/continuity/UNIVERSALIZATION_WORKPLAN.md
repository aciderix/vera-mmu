...
| Capacité actuelle | M1 et M2 sont publiés. M3.1–M3.8 ajoutent : contrats de capability fermés, validation locale de paramètres typés, policy immutable `ALLOW`/`DENY`/`CONFIRM`, seul runner `NOOP` sous `DENY_NETWORK`, executions immuables, evidence JSON hashée, admission immutable, preuve dérivée `PROVEN` sans mutation de knowledge et gate de work item calculée à partir d’une admission existante. | `OBSERVED` : migrations 001→020, 143 tests et 14 sous-tests, wheel isolé et frontières de sécurité validés. | Mémoire `MEM-STATE-029`; journal `LOG-0051` à `LOG-0076`. |
| Exclusions actuelles | Aucun shell arbitraire, réseau implicite, accès fichier externe, runner additionnel, validator framework, policy HMAC de projet, gate multi-evidence, lifecycle/traversal de work graph, CLI/MCP de production, pack/importeur ARET ou dashboard. Le sous-ensemble de paramètres ne devient pas JSON Schema général, et `CONFIRM` ne devient pas une confirmation interactive. | `OBSERVED` | Mémoire `MEM-DEC-027`, `MEM-DEC-028`, `MEM-STATE-030`; journal `LOG-0076`. |
...
| Lot suivant | M3.8 a fermé le catalogue immutable de décisions `ALLOW`/`DENY`/`CONFIRM`. Le prochain lot M3 candidat est un framework de validators fermé, sans runner additionnel ni oracle ARET, avec baseline/hypothèse/tests propres. | `M3_IN_PROGRESS` ; `MEM-WALL-001` préservée | Journal `LOG-0076`; mémoire `MEM-STATE-030`. |
...
| `M3` | Capability / Evidence / Gates | `M3.S1` livré; M3.7 ajoute la validation locale des paramètres et M3.8 une policy immutable `ALLOW`/`DENY`/`CONFIRM` exigée par `NOOP`. Restent validators, runners additionnels, HMAC de projet, gates/graph avancés et surfaces CLI/MCP. | `M3.S1.EXIT` a validé migrations 001→019; M3.7 et M3.8 ajoutent respectivement paramètres fermés et policy `ALLOW` obligatoire sans généraliser JSON Schema ni introduire confirmation interactive. Aucun verdict ne ferme M3 global ni la parité ARET. | I004–I008, I013, I014, I015 | `IN_PROGRESS` — `M3.S1.EXIT`, M3.7 et M3.8 `PASS`; lots restants séparés |
