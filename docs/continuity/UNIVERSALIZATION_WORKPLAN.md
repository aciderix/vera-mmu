...
| `M11-D-B2-contract` — Journalisation des mouvements de racines | `PASS` préparatoire | Le preview/journal expose `workspace_moves` avec source, cible et type; les racines ambiguës, symlinkées, non-régulières ou de cardinalité différente sont refusées. Aucun déplacement physique de racine n’est encore autorisé. |
