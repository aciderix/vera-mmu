...
| Capacité actuelle | M1 et M2 sont publiés. M3.S1 et M3.S2 sont des tranches validées. M3.1–M3.13 ajoutent : contrats de capability fermés, validation locale de paramètres typés, policy immutable `ALLOW`/`DENY`/`CONFIRM`, seul runner `NOOP` sous `DENY_NETWORK`, executions immuables, evidence JSON hashée, admission immutable avec policy optionnelle stricte, policy HMAC de projet sans secret persistant, preuve dérivée `PROVEN` sans mutation de knowledge, gates de work item à une ou plusieurs evidences admises, validator local `EVIDENCE_HASH` et lifecycle de work item dérivé par événements. | `OBSERVED` : migrations 001→025, 154 tests et 14 sous-tests, wheel isolé, intégration M3.S2 et frontières de sécurité validés. | Mémoire `MEM-STATE-041`; journal `LOG-0051` à `LOG-0093`. |
| Exclusions actuelles | Aucun shell arbitraire, réseau implicite, accès fichier externe, runner additionnel, validator métier ou oracle externe, quorum/disjonction/pondération de gate, pause/reprise/réouverture ou orchestration de lifecycle, traversal de work graph, CLI/MCP de production, pack/importeur ARET ou dashboard. Le sous-ensemble de paramètres ne devient pas JSON Schema général; `CONFIRM` ne devient pas une confirmation interactive; HMAC ne couvre ni rotation/révocation/expiration ni algorithme alternatif; `EVIDENCE_HASH` ne devient pas admission ou preuve, bien qu’un résultat `PASS` puisse être requis par la policy d’admission stricte. | `OBSERVED` | Mémoire `MEM-DEC-027` à `MEM-DEC-034`, `MEM-STATE-042`; journal `LOG-0093`. |
...
| Lot suivant | M3.13 lie optionnellement admission et validation persistée. Le prochain lot M3 candidat est un validator de contenu explicitement borné ou un runner sûr additionnel, sans oracle ARET, avec baseline/hypothèse/tests propres. Toute nouvelle tranche devra avoir une sortie distincte. | `M3_IN_PROGRESS` ; `MEM-WALL-001` préservée | Journal `LOG-0093`; mémoire `MEM-STATE-042`. |
...
| `M3` | Capability / Evidence / Gates | `M3.S1` livré; `M3.S2` valide l’intégration de M3.7–M3.12; M3.13 ajoute la policy d’admission `PASS_EVIDENCE`/`VALIDATED_PASS_EVIDENCE` sans validation implicite. Restent validators de contenu/oracles, runners additionnels, lifecycle/graph avancés et surfaces CLI/MCP. | `M3.S1.EXIT` a validé migrations 001→019; `M3.S2.EXIT` valide migrations 001→024; M3.13 valide migration 025, policy strictement fermée, suite complète et wheel isolé sans généraliser JSON Schema, confirmation interactive, rotation, quorum, orchestration ou oracle externe. Aucun verdict ne ferme M3 global ni la parité ARET. | I004–I008, I013, I014, I015 | `IN_PROGRESS` — `M3.S1.EXIT`, `M3.S2.EXIT` et M3.13 `PASS`; lots restants séparés |
