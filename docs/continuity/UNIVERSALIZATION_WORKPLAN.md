...
| Sous-lot | Statut | Preuve et limite |
|---|---|---|
| `M11-D-B2` — Préflight/preview, journal, reprise et exécution bornée Profile/runtime | `PASS` dans le périmètre borné | `src/vera_mmu/profile_migration.py` persiste un journal `PLANNED`, classe read-only `READY_FOR_EXECUTOR`/`DIVERGED`/`RECOVERY_REQUIRED`, puis déplace atomiquement un runtime sur le même filesystem avec checkpoint WAL, backup du Profile et rollback de base après confirmation (`mutation: RUNTIME_AND_PROFILE`). Les racines workspace, inter-filesystems et reprise automatique complète restent hors périmètre. |
