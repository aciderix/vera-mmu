...
>
> **Dernière revue factuelle :** 26 août 2026 — M4.15 validé et publié comme premier import atomique borné `component→entity`; M4 reste `IN_PROGRESS`, les couplages restent `SPLIT` et la parité `UNKNOWN`.
>
...
| Révision VERA-MMU | `034efaf9f6d845742d2209c89099d10dd5fc4ad0` sur `main`, premier import autorisé et atomique M4.15 publié et vérifié. | `OBSERVED` | Journal `LOG-0152`. |
...
| Capacité actuelle | M1, M2 et M3 sont publiés dans leurs périmètres contractuels. M4.1–M4.14 préparent adresse, runtime, schéma, source, lecture, préflight, projection, collision et batch Core; M4.15 lie explicitement ces états pour importer une page `component→entity` avec rollback/audit et zéro promotion. | `OBSERVED` : migrations 001→032, `261 passed, 14 subtests passed`, intégration ponctuelle de 17 composants dans un store VERA temporaire, wheel isolée et frontières de pack validées. | Mémoire `MEM-STATE-097`; journal `LOG-0151`. |
| Exclusions actuelles | Aucun shell arbitraire, réseau implicite, accès fichier externe, runner autre que `NOOP`/`EVIDENCE_HASH`/`EVIDENCE_FIELDS`, validator métier ou oracle externe, quorum pondéré, expiration/fenêtre temporelle ou révocation de gate, pause/reprise/réouverture ou orchestration de lifecycle, CLI/MCP de production, import ARET hors page `component` explicitement autorisée M4.15, merge, proof/promotion, pack de compatibilité complet ou dashboard. Le sous-ensemble de paramètres ne devient pas JSON Schema général; `CONFIRM` ne devient pas une confirmation interactive; HMAC ne couvre ni rotation/révocation/expiration ni algorithme alternatif; aucun runner ou policy ne devient admission/proof automatique. | `OBSERVED` | Mémoire `MEM-DEC-063`; journal `LOG-0151`. |
...
| Lot suivant | M4 est ouvert. M4.1–M4.15 sont `PASS` dans des contrats bornés; tout nouveau lot doit fermer une gate du [registre de clôture M4](M4_COMPLETION_REGISTER.md) sous contrat séparé. Aucun verdict ne clôture C01–C16, `MEM-WALL-001` ou la parité ARET. | `M4_IN_PROGRESS` ; `MEM-WALL-001` préservée | Journal `LOG-0151`; mémoire `MEM-STATE-098`. |
...
| `M4` | Pack ARET et primitive Core | M4.1–M4.14 livrent les surfaces déclaratives, de source, de lecture, de préflight, de projection, de collision et le batch Core atomique. M4.15 ajoute un contrat d’autorisation explicite qui relie strictement M4.11–M4.13, recontrôle la collision au write-path et appelle seulement le batch Core : une page `component→entity` peut être créée atomiquement avec audits, rollback sur conflit et état `IMPORTED_NO_PROMOTION`. Les imports complets, autres tables, provenance de lot persistée, pipelines, playbook, toolchain, MCP/hooks/bundles/VCS et parité restent séparés dans le [registre de clôture M4](M4_COMPLETION_REGISTER.md). | M4.15 : tests-first, ciblés `5 passed`, suite Core `261 passed, 14 subtests passed`, contrôle de frontières, import réel de 17 composants dans une cible VERA temporaire, rollback de conflit et wheel isolée. La parité ARET reste mesurée uniquement contre une baseline exécutable. | I002, I004, I006–I008, I015 | `IN_PROGRESS` — M4.1–M4.15 `PASS` bornés; C01–C06/C16 `SPLIT`, C07/C08 `BLOCKED`, parité `UNKNOWN` |
