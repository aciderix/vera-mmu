...
| Révision VERA-MMU | `8982b7855e09db8ed009ca2081021b9210bc8088` sur `main`, commit M2.8 publié et vérifié ; le record de publication est consigné dans le commit documentaire suivant. | `OBSERVED` | Journal `LOG-0027`, `LOG-0028`. |
...
| Capacité actuelle | Core d’identité, substrate SQLite, entités, relations, knowledge append-only, provenance déclarative, supersession directe, assets binaires hashés et associations knowledge–asset déclaratives : migrations checksumées, metadata/audit, transactions, objets typés immuables, assertions hashées, sources bornées, bytes revérifiés avant lecture et paires knowledge–asset exactes. | `OBSERVED` : 85 tests et 14 sous-tests passent ; wheel isolé et registre de liaisons validés. | Journal `LOG-0027` ; mémoire `MEM-STATE-014`. |
| Exclusions actuelles | Pas encore de chemin/fichier externe, fetch, réseau, import/export, bundle, mutation de statut ou `SUPERSEDED`, version counter, `PROVEN`/evidence, execution/validator, gate, FTS/FIND, traversal/listing de lignée, lecture d’asset par liaison, intégration à `RelationService`, symbol, serveur MCP de production, Capability Engine, importeur ARET ou dashboard. | `OBSERVED` | Mémoire `MEM-STATE-014` ; journal `LOG-0027`. |
...
| Lot suivant | Aucun sous-lot actif : M2.8 est publié. M2.9 exige un rituel distinct et une hypothèse explicitement bornée. | `READY_FOR_M2.9_RITUAL` ; `MEM-WALL-001` préservée | Journal `LOG-0027`, `LOG-0028` ; mémoire section 7. |
