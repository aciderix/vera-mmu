...
>
> **Dernière revue factuelle :** 26 août 2026 — M3.EXIT validé; M3 est `PASS` dans son périmètre Core contractuel.
>
...
| Révision VERA-MMU | `1d7b2efb6fdd914e58b8de7d3ff232de848c59a2` sur `main`, preuve exécutable M3.EXIT publiée et vérifiée. | `OBSERVED` | Journal `LOG-0122`. |
...
| Capacité actuelle | M1, M2 et M3 sont publiés dans leurs périmètres contractuels. M3 fournit le Core local fermé de capabilities, evidence, validation, admission, proof, gates, work graph et lifecycle dérivé. | `OBSERVED` : migrations 001→032, `188 passed, 14 subtests passed`, fresh/upgrade 001→032, chaîne terminale et wheel isolée validés. | Mémoire `MEM-STATE-067`; journal `LOG-0121`. |
...
| Lot suivant | Aucun lot M3 ne reste ni ne peut être ajouté. Les travaux ultérieurs relèvent de M4+ : pack ARET, adapters MCP, CLI/dashboard ou conformance, chacun sous contrat distinct; ils ne sont pas initiés par ce verdict. | `M3_PASS` ; `MEM-WALL-001` préservée | Journal `LOG-0121`; mémoire `MEM-STATE-068`. |
