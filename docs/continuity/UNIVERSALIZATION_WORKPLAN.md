...
| Révision VERA-MMU | `8e0d56692c3f1a5b19d9e2ac1d40678f10c7c7fc` sur `main`, primitive Core 034 de batches structurels `SYMBOL`/`WORK_ITEM` publiée et durcie contre toute coercition implicite de payload. | `OBSERVED` | `LOG-0165`, `LOG-0167`; commits Core 034 publiés après les sous-lots M4-A et M4-B read-only/purs. |
...
| Capacité actuelle | M1, M2 et M3 sont publiés dans leurs périmètres contractuels. M4-A apporte le resolver runtime read-only/WAL fail-closed, le ledger entity 033, la conformité `component` et l’import de page non fusionnel. M4-B apporte lecteurs/projections read-only/purs et le Core 034, qui enregistre les seuls batches génériques `SYMBOL`/`WORK_ITEM` par les services Core, sans sémantique source. | `OBSERVED` : migrations 001→034, suite `326 passed, 14 subtests passed`, import baseline de 17 composants/17 liens/replay sans écriture dans une cible temporaire, et Core 034 validé par replay, rollback, migration 033→034, immutabilité, refus de coercition implicite et wheel isolée ; zéro evidence/proof link créée par ces paths. | Registre M4 ; `LOG-0165`. |
| Exclusions actuelles | Aucun shell arbitraire, réseau implicite, accès fichier externe, runner autre que `NOOP`/`EVIDENCE_HASH`/`EVIDENCE_FIELDS`, validator métier ou oracle externe, quorum pondéré, expiration/fenêtre temporelle ou révocation de gate, pause/reprise/réouverture ou orchestration de lifecycle, CLI/MCP de production, import ARET hors pages `component` explicitement autorisées M4-A, merge, proof/promotion, pack de compatibilité complet ou dashboard. Le Core 034 ne constitue pas une autorisation de `function_symbol` ni de `brick`; leurs conformance/préflight/collision/autorisation/post-validation restent requis. | `OBSERVED` | Registre M4 ; `LOG-0165`–`LOG-0166`. |
...
| Lot suivant | M4-B reste ouvert. Les prochains contrats sont la conformance de schéma source et le préflight read-only séparés de `function_symbol` et `brick`; seulement ensuite pourront être examinés collision de série, autorisation, import et post-validation propres à chaque mapping. Aucun verdict ne clôture C01–C16, `MEM-WALL-001` ou la parité ARET. | `M4_IN_PROGRESS` ; `MEM-WALL-001` préservée | Registre M4 ; `LOG-0166`. |
