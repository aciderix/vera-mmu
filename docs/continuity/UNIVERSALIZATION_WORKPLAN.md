...
| Capacité actuelle | M1, M2 et M3 sont publiés dans leurs périmètres contractuels. M4-A/B/C restent inchangés. M4-D ajoute un catalogue Pack fermé de neuf oracles, le préflight sans installation, le runner Core `OBSERVED_PROCESS` générique, le runner ARET Git/binaire/sandbox-bound et la chaîne capability→asset→execution→evidence. | `OBSERVED` : migrations 001→037, suite `390 passed, 17 subtests passed`, roue isolée, `difftest 272/272` et `winediff win32_username 1/1` persistés comme evidence `PENDING`. Admission/proof/gate, doctor, corpus Wine global et parité restent hors preuve. | Registre M4 ; `artifacts/m4d_closed_oracle_pipeline_integration_2026-08-26.md`. |
...
| Précondition toolchain ARET | La branche toolkit verrouillée fournit sources/scripts/corpus ; le binaire externe SHA-256 est épinglé. Le préflight Pack vérifie commit, propreté, scripts, dépendances et sandbox sans installer. `difftest` complet est `PASS 272/272` et une fixture Wine est `PASS 1/1`, tous deux evidence `PENDING`; le corpus Wine historique reste `FAIL 255/264` et le corpus sandboxé complet bloque sur `win32_winsock`. | `OBSERVED_RESTORED` ; C07/C08 restent `IN_PROGRESS` | `artifacts/m4d_closed_oracle_pipeline_integration_2026-08-26.md`. |
| Registre M0.2 | Seize couplages documentés avec sources, frontière Core/pack, stratégie et tests de parité ; C07/C08 disposent maintenant d’un runner/préflight Pack bornés, mais validator, admission/proof/gate et doctor restent absents. | `PASS` pour la cartographie ; parité `UNKNOWN` | [Matrice](../DECOUPLING_MATRIX.md) ; artefact M4-D du 26 août 2026. |
| Lot suivant | M4.EXIT reste `NOT_ELIGIBLE`. La progression légitime est désormais de définir test-first le validator, l’admission/proof/gate reliés à l’evidence d’oracle, puis un doctor/image de référence versionné. Il faut ensuite diagnostiquer `win32_winsock` et les neuf divergences Wine sans filtrer de fixture ni transformer un run interrompu en résultat. Les limites M4-B/C et les dépendances M5/M6 sont inchangées. | `M4_IN_PROGRESS` ; parité `UNKNOWN` | Registre M4 ; artefact M4-D du 26 août 2026. |
...
| `M4` | Pack ARET et primitive Core | M4.1–M4.15 livrent les surfaces déclaratives et le premier write-path. M4-A/B/C restent décrits au registre. M4-D ajoute catalogue fermé, préflight, `OBSERVED_PROCESS` générique, runner ARET commit/binaire/sandbox-bound et evidence VERA non admise. Conformance globale, autres tables, validators, doctor, MCP/hooks/bundles/VCS et parité restent dans le [registre de clôture M4](M4_COMPLETION_REGISTER.md). | Intégrations temporaires d’import inchangées. Pipeline réel M4-D : `difftest 272/272` et `winediff win32_username 1/1` avec assets hashés, executions append-only et evidence `PENDING`; aucun admission/proof/gate/promotion. Suite `390 passed, 17 subtests passed`, scans et roue isolée passent. Le corpus Wine historique reste `255/264 FAIL`; le run sandboxé global est non terminé. | I002, I004, I006–I008, I015 | `IN_PROGRESS` — M4-A `PASS` borné ; M4-B/C observés, C01–C06/C16 `SPLIT`, C07/C08 `IN_PROGRESS` avec pipeline limité, parité `UNKNOWN` et `M4.EXIT` `NOT_ELIGIBLE` |
