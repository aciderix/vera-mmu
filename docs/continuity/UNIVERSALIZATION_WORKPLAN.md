...
>
> **Dernière revue factuelle :** 25 août 2026 — cadrage terminal M2 consigné dans `LOG-0040`.
>
...
| Révision VERA-MMU | `24c87989531b4df76929355c3b4b6be857153db0` sur `main`, record de publication M2.11 ; le commit fonctionnel est `34d9c2595ab93c1e041c88fb213451b2b1794929`. | `OBSERVED` | Journal `LOG-0038`, `LOG-0039`. |
...
| Capacité actuelle | Core d’identité, substrate SQLite, entités, relations, knowledge append-only, provenance déclarative, supersession directe, assets binaires hashés, associations knowledge–asset, index direct borné de ces associations, provenance documentaire déclarative des assets et index exact borné de sources knowledge par hash : migrations checksumées, metadata/audit, transactions, objets typés immuables, assertions hashées et sources retournées sans contenu knowledge. | `OBSERVED` : 100 tests et 14 sous-tests passent ; wheel isolé et index de sources knowledge validés. Le candidat d’index d’assets par hash a été rejeté comme redondant. | Journal `LOG-0036`, `LOG-0038` ; mémoire `MEM-STATE-017`. |
| Exclusions actuelles | Pas encore de chemin/fichier externe, fetch, réseau, import/export, comparaison source↔asset, bundle, mutation de statut ou `SUPERSEDED`, version counter, `PROVEN`/evidence, execution/validator, gate, FTS/FIND, recherche libre, traversal multi-sauts, lecture de knowledge/document par index, lecture d’asset par liaison ou source, intégration à `RelationService`, symbol, serveur MCP de production, Capability Engine, importeur ARET ou dashboard. | `OBSERVED` | Mémoire `MEM-STATE-017` ; journal `LOG-0038`. |
...
| Lot suivant | Aucun patch M2.12 n’est actif. Le prochain travail technique sera `M2.12 — Symbol Registry`, seulement après publication de ce cadrage et ouverture de son hypothèse autonome. | `READY_FOR_M2.12_RITUAL` ; cadence en macro-lots ; `MEM-WALL-001` préservée | Journal `LOG-0040` ; mémoire `MEM-STATE-018`. |
...
| `M2` | **Universal Schema** fini | Livré : M2.1–M2.11 (substrate, entités/relations, knowledge/provenance/supersession, assets et associations). Reste : `M2.12 Symbol Registry`, `M2.13 Work-Item Backbone`, `M2.14 Capability Declaration & Execution Schema`, puis `M2.EXIT`. Chaque lot regroupe schéma, service, migrations et tests de sa ressource ; aucun index isolé n’est une fin de lot. | **M2.EXIT** : toutes les six ressources de la spécification M2 existent avec migrations vérifiées, services de persistance exacts et bornés, FKs/immuabilité/audit/rollback testés ; fresh install et upgrade 001→courant, tests complets, wheel isolé, scans anti-ARET/no-shell/no-network/no-`PROVEN` passent. La gate ne revendique ni parité ARET ni preuve métier. | I001–I006, I010, I011, I014, I015 ; frontière I004, I007–I008, I013 | `IN_PROGRESS` — M2.1–M2.11 `PASS`; M2.12–M2.14 et M2.EXIT `PLANNED`; parité ARET `UNKNOWN` |
| `M3` | Capability / Evidence / Gates | Catalogue fermé, runners bornés, écriture/lecture opérationnelle des executions, validators, Evidence Store, admission, HMAC, `PROVEN`, gates et work graph. | Aucun shell arbitraire ; une gate ne passe que sur une exécution réelle et une preuve admissible. Une execution n’est jamais une preuve par elle-même. | I004–I008, I013, I014 | `PLANNED` |
