...
**M5-A à M5-M.3a restent `PASS`; M5-M.3b est `PASS` pour son mécanisme sous home simulé et M5-N est `PASS` pour sa chaîne Codex contrôlée. M5 demeure `IN_PROGRESS`.** M5-J a livré le mécanisme local, M5-K son attestation/transport MCP, M5-L le premier hôte local opt-in, M5-M.1 le plan/doctor cloud préinstallé, M5-M.2 l’adapter cloud staged, M5-M.3a la configuration project-local attestée, M5-M.3b la préparation user-scope à deux gates, et M5-N l’adapter Codex à couverture `PARTIAL_LOCAL_TOOLS` et M5-O l’adapter Gemini à garde `TOOL_GUARD_NO_POST_COMPACTION`. L’écriture user-scope réelle et les preuves hôte réelles restent `NOT_RUN`; les voies M5-P→M5-Q ajoutent ensuite les autres adapters à des niveaux de preuve explicites. Aucun résultat d’oracle ARET n’est requis pour cette conformance : les suites démontrent le transport et la gouvernance de verdicts, non l’environnement local Wine.
