...
> **Statut :** M5-J/K sont livrés comme Core/liaison MCP transport-neutres, M5-L/M comme adapters Claude et M5-N comme adapter Codex à couverture `PARTIAL_LOCAL_TOOLS`. Ce document ne revendique aucune preuve d’exécution Codex/Claude Web réelle, ni aucune garantie au-delà des surfaces hôte testées. Il remplace le cadrage trop étroit « un hook SessionStart Claude » par l’extraction du mécanisme de reprise fonctionnel ARET vers un Core VERA indépendant de l’hôte.
...
| **M5-N** | Adapter Codex `codex-v1` : runtime staged, hooks, MCP deny-by-default et configuration `.codex/` confirmée. | **`PASS` pour chaîne contrôlée** (`588c886`) : staging, conflits/symlink, conservation tiers, MCP stdio réel, acquittement et réarmement PostCompact. Niveau annoncé `PARTIAL_LOCAL_TOOLS`; client Codex/trust hôte `NOT_RUN`. | Prétention de garde totale sur hosted tools ou chemins non interceptés, écriture `~/.codex/`, réseau/bootstrap/secrets/OAuth ou auto-trust. |
| **M5-O** | Adapter Gemini CLI `gemini-cli-v1` : runtime staged, hook JSON, MCP deny-by-default et `.gemini/settings.json` confirmée. | **`PASS` pour chaîne contrôlée** (`7ca437e`) : staging, config/conflit/symlink, MCP stdio et garde `BeforeTool`; `PreCompress` est seulement signalé. Niveau `TOOL_GUARD_NO_POST_COMPACTION`; client Gemini/trust hôte `NOT_RUN`. | Promesse `PostCompact`, réarmement synthétique, support durable sans doctor hôte, home/trust, réseau/bootstrap/secrets/OAuth. |
| **M5-P** | Adapter Antigravity `antigravity-v1` : runtime staged, hook JSON, MCP deny-by-default et `.antigravity/settings.json` confirmée. | **`PASS` pour chaîne contrôlée** (`df03100`) : staging, config/conflit/symlink, MCP stdio, ouverture `PreInvocation`, garde `PreToolUse`, acquittement et `Stop`. Niveau `TURN_GUARD_HARD`; host/trust réels `NOT_RUN`. | Émulation fictive de session durable, compaction/restauration, home/trust, réseau/bootstrap/secrets/OAuth. |
| **M5-Q** | Fallback MCP générique : runtime staged, serveur stdio deny-by-default et `.mcp.json` project-local confirmée, sans hook. | **`PASS` pour fallback contrôlé** (`00f6cee`) : catalogue, refus capability/ack sans contexte, conflit/symlink et vrai client MCP stdio. Niveau `MCP_ONLY`; aucun host spécifique requis. | Toute automation de reprise, session, compaction, trust/home, réseau, secret ou exécution de capability. |
...
**M5-A à M5-M.3a restent `PASS`; M5-M.3b est `PASS` pour son mécanisme sous home simulé et M5-N est `PASS` pour sa chaîne Codex contrôlée. M5 demeure `IN_PROGRESS`.** M5-J a livré le mécanisme local, M5-K son attestation/transport MCP, M5-L le premier hôte local opt-in, M5-M.1 le plan/doctor cloud préinstallé, M5-M.2 l’adapter cloud staged, M5-M.3a la configuration project-local attestée, M5-M.3b la préparation user-scope à deux gates, et M5-N l’adapter Codex à couverture `PARTIAL_LOCAL_TOOLS` et M5-O l’adapter Gemini à garde `TOOL_GUARD_NO_POST_COMPACTION` et M5-P l’adapter Antigravity à `TURN_GUARD_HARD`. L’écriture user-scope réelle et les preuves hôte réelles restent `NOT_RUN`; M5-Q livre le fallback MCP-only à un niveau de preuve explicite. M6 devient le prochain lot de consolidation CLI/doctor sans déplacer la logique des adapters. Aucun résultat d’oracle ARET n’est requis pour cette conformance : les suites démontrent le transport et la gouvernance de verdicts, non l’environnement local Wine.
