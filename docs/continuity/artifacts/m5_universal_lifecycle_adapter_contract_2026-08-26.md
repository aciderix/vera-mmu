...
| **M5-M.3b** | Preview/fusion/apply user-scope sous double confirmation transactionnelle, puis preuve Claude Code web fraîche. | Aucune gate clôturable sans deux confirmations explicites au moment de l’écriture et observations host admissibles du protocole live. | Approbation automatique cachée, écriture home sans preview/double confirmation, push automatique. |
...
M5-M.3a est livré pour la configuration host **project-local**. Le prochain changement fonctionnel autorisé est **M5-M.3b seulement** : produire le preview/merge du trust user-scope, exiger deux confirmations transactionnelles explicites au moment de cette écriture, puis exécuter le protocole de preuve live dans une session cloud fraîche. Il ne doit ni réutiliser implicitement les fichiers locaux, ni effectuer réseau/bootstrap/push sans confirmation. Cela évite de recréer un script ARET sous un nouveau nom avant de prouver le premier hôte complet.
...
**M5-A à M5-M.3a restent `PASS`. M5 demeure `IN_PROGRESS`.** M5-J a livré le mécanisme local, M5-K son attestation/transport MCP, M5-L le premier hôte local opt-in, M5-M.1 le plan/doctor cloud préinstallé, M5-M.2 l’adapter cloud staged et M5-M.3a la configuration project-local attestée. M5-M.3b doit encore traiter le trust user-scope sous double confirmation puis prouver un hôte cloud réel ; la voie M5-N→M5-Q ajoute ensuite les autres adapters à des niveaux de preuve explicites. Aucun résultat d’oracle ARET n’est requis pour cette conformance : les suites démontrent le transport et la gouvernance de verdicts, non l’environnement local Wine.
