...
| `M4-EXIT-09` | `IN_PROGRESS` | `EVIDENCE_ASSET` valide la liaison evidence→asset→execution; `difftest PASS`, `winehash UNKNOWN`, `difftest SKIPPED` et `winediff user32_paint FAIL` sont observés par runs réels. Seul le `PASS` validé est admis; les trois non-`PASS` ont zéro proof/gate. | Corpus Wine complet comparable, couverture de tous les oracles et harnais de parité restent absents. `ERROR` est contractuellement testé mais non forcé en runtime; le corpus sandboxé global a bloqué sur `win32_winsock` et l’historique externe reste `255/264 FAIL`. |
