...
| `M4-EXIT-09` | `IN_PROGRESS` | `EVIDENCE_ASSET` lie evidence→asset→execution. La matrice interne couvre les payloads `272/272 PASS`, `271/272 FAIL`, timeout/sortie inconnue `ERROR`, dépendance absente `SKIPPED`, format non promouvable `UNKNOWN` et asset altéré; seul `PASS` validé est admis. | La couverture complète de la matrice et son exposition par une vraie surface MCP restent à livrer. Les scores Wine et `win32_winsock` sont des observations de Pack, non des critères du transport VERA. |
...
| `M4-EXIT-14` | `UNKNOWN` | Le harnais de transport VERA couvre les scénarios principaux d’un Pack ARET sans exiger qu’ARET réussisse localement. | Conformance VERA des imports et de l’ensemble de la matrice Pack, plus surface MCP M5 et compatibilités M5/M6, restent absentes. La parité fonctionnelle intrinsèque ARET est une piste distincte. |
