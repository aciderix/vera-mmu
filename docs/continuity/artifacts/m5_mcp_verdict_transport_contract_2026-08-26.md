...
> **Statut :** `M5-A/B/C/D/E/F/G/H/I/J/K/L/M.1/M.2/M.3a/M.3b PASS` pour leurs mécanismes testés — façade `5ffe182`, manifeste `5de260d`, registry `50cc79a`, adapter Pack `e073fa2`, instructions `9010293`, config `5dab574`, hooks `ea7235a`, plan hôte `8b38b1b`, installateur `674929c`, Lifecycle Core `e576b1a`, registry/acquittement `df73425`, adapter Claude local `45fe9af`, plan/doctor cloud `940fb7e`, adapter cloud staged `f79415b`, configuration project-local cloud `ed9f2e8`, trust user-scope préparé `3f26dad`.
> **Portée :** transport fermé, manifeste, registries runtime/lifecycle, runtime de Pack, doctrine, config, cycle SessionStart, revue hôte, installation MCP opt-in, acquittement contextualisé, plan cloud préinstallé, adapter cloud runtime-confined, configuration project-local attestée et mécanisme user-scope doublement confirmé. Toute écriture user-scope réelle, le trust host constaté, la preuve Claude Code web live et les adapters d’hôtes autres que Claude restent hors des jalons réalisés.
...
| Configuration cloud project-local | `claude_code_cloud.py` prévisualise, fusionne et applique après confirmation les hooks cloud fixes et le serveur MCP attesté. | Ajouté en M5-M.3a ; seulement `.claude/settings.json`, `.mcp.json` et état runtime sous projet ; aucun home/trust, réseau ou preuve web live. |
| Trust cloud user-scope | `claude_code_cloud.py` prévisualise/fusionne seulement l’approbation de l’identifiant MCP VERA attesté et sépare deux confirmations nominatives pour l’écriture. | Ajouté en M5-M.3b ; tests sous home temporaire, sans cible configurable, sans secret ni réseau ; trust réel et preuve web live non exécutés. |
| Codex project-local | `codex_adapter.py` stage un runtime, expose le MCP d’acquittement et génère/fusionne `.codex/hooks.json` et `.codex/config.toml` après confirmation. | Ajouté en M5-N (`588c886`) ; chaîne hook JSON→MCP stdio→ack→réarmement testée, mais `PARTIAL_LOCAL_TOOLS` seulement. Aucun `~/.codex/`, auto-trust, réseau, secret ou preuve host réelle. |
...
| Configuration cloud M5-M.3a | Preview et fusion attestés de `.claude/settings.json` et `.mcp.json`, application uniquement project-local après confirmation, état `vera-claude-code-cloud-host-config/v1`. | `3 failed` rouge puis preview stable, préservation tiers, refus conflit/symlink, application confirmée ; `7 passed`, suite `462 passed, 37 subtests passed`, roue isolée avec quatre entry points cloud. Aucun trust home ni preuve web live. |
| Trust user-scope M5-M.3b | Preview `vera-claude-code-cloud-user-trust/v1` lié au reçu M5-M.3a, fusion de la seule approbation MCP VERA et application seulement avec `confirm_preview` **et** `confirm_user_scope`. | `3 failed` rouge puis préservation tiers, précondition project, conflit disabled, symlink, double confirmation et CLI no-write ; `11 passed`, suite `465 passed, 37 subtests passed`, roue isolée `PASS`. Home réel, trust host et preuve web `NOT_RUN`. |
| Codex M5-N | Runtime `vera-codex-runtime/v1`, hooks project-local, MCP stdio `DenyRuntimeAdapter`, configuration `.codex/` et reçu hashé. | `3 failed` rouge, puis `4 passed` pour staging/conflit/symlink/configuration, chaîne hook JSON→MCP stdio→ack→PostCompact ; suite `470 passed, 37 subtests passed`, roue isolée avec quatre entry points. Couverture `PARTIAL_LOCAL_TOOLS`, client Codex/trust host `NOT_RUN`. |
