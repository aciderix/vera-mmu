...
> **Statut :** `M5-A/B/C/D/E/F/G/H/I/J/K/L/M.1/M.2/M.3a PASS` — façade `5ffe182`, manifeste `5de260d`, registry `50cc79a`, adapter Pack `e073fa2`, instructions `9010293`, config `5dab574`, hooks `ea7235a`, plan hôte `8b38b1b`, installateur `674929c`, Lifecycle Core `e576b1a`, registry/acquittement `df73425`, adapter Claude local `45fe9af`, plan/doctor cloud `940fb7e`, adapter cloud staged `f79415b`, configuration project-local cloud `ed9f2e8`.
> **Portée :** transport fermé, manifeste, registries runtime/lifecycle, runtime de Pack, doctrine, config, cycle SessionStart, revue hôte, installation MCP opt-in, acquittement contextualisé, plan cloud préinstallé, adapter cloud runtime-confined et configuration project-local attestée. Le trust user-scope cloud, la preuve Claude Code web live et les adapters d’hôtes autres que Claude restent hors des jalons réalisés.
