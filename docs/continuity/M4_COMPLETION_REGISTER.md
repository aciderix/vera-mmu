...
| Wall / précondition restaurée | La branche toolkit verrouillée a fourni source, corpus, scripts et build ; le binaire a été reconstruit et les oracles ont été exécutés comme observations externes hashées. | Aucun `SKIPPED`, `UNKNOWN`, résultat externe ni test synthétique ne permet d’affirmer C07/C08 ou M4.EXIT ; `winediff` est strictement en échec à `255/264`. |
...
| Parité ARET | `UNKNOWN`. |
| C07 / C08 | `IN_PROGRESS` : précondition toolkit restaurée et exécutions externes hashées disponibles ; runner/normalisation/admission/doctor VERA absents. |
...
| Prochain lot autorisé | Définir test-first la capability fermée, les dépendances, la normalisation et la chaîne d’admission VERA ; investiguer les neuf divergences Wine sans filtrer aucune fixture ; M3 ne doit pas être rouvert. |
