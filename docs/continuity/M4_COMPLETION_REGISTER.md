...
| `M4-EXIT-04` Import `function_symbol → symbol` | C04, C16 | Lecteur V1 borné, projection vers `symbol`, liaison exacte aux entities importées, unicité et provenance source. | Tests source/cible de colonnes, parent manquant, unicité, pagination, conflit, rollback et reprise ; comparaison de cardinalité et liens. | Lecteur read-only paginé/hashé, projection pure, conformité source, préflight, collision non-fusionnelle, autorisation explicite et Core 034 sont publiés. Une page temporaire de 9 symbols a été importée/post-validée/rejouée sans écriture ; pagination, conflits réels, séries et compatibilité **restent requis**. |
| `M4-EXIT-05` Import `brick → work_item` | C05, C16 | Lecteur V1 borné, projection de statut/type/priorité/milestone/platforme/parent et liens, avec provenance et politique de statut explicite. | Tests de tous statuts, hiérarchie, ordre roadmap, liens component, cycles/parents impossibles, rollback/reprise et comparaison de cardinalité. | Lecteur read-only, projection pure, conformité source, préflight, collision non-fusionnelle, autorisation explicite et Core 034 sont publiés. Une page temporaire de 13 work items a été importée/post-validée/rejouée sans écriture après résolution de `brick.component_id` vers l’entité VERA projetée ; sémantique d’état cible, pagination, conflits réels et reprise **restent requis**. L’état legacy reste une métadonnée tant qu’un contrat lifecycle/Front distinct n’est pas livré. |
| `M4-EXIT-06` Migration des données et invariants associés | C03–C05, C16 | Contrats isolés pour `knowledge`, `knowledge_source`, tags, relations, proofs/proof links, assets/associations, audit, front, séquences, métadonnées et ledger de migration. | Pour chaque table : mapping versionné, provenance `source_type/repository/revision/import_batch_id`, règles de conflit/non-merge, FK/cycles, cardinalité, no-loss, append-only, refus cross-project et rollback. | Les seuls imports observés sont `component`, `function_symbol` et `brick` dans leurs contrats de première page ; données sémantiques, audit/front et autres tables **non commencés**. |
...
| `M4-B` — objets structurels liés | `M4-EXIT-04` et `M4-EXIT-05` | `function_symbol` et `brick` sont les deux mappings structurels restants ; ils dépendent des components de `M4-A` et des mêmes politiques de batch/provenance. | Imports `symbol` et `work_item` sans perte, avec FKs/hiérarchie/statuts/liens, rollback et reprise. | `IN_PROGRESS` : lecteurs, projections, conformités, préflights, collisions, autorisations, write-paths/post-validations et Core 034 sont publiés. L’intégration temporaire d’une page initiale a observé 9 symbols et 13 work items, avec replay sans écriture, zéro promotion et états legacy conservés ; les séries, conflits/courses réels, lifecycle/Front et parité restent requis. |
...
| M4-B intégration | `OBSERVED` : snapshot attesté, import/post-validation temporaire de 9 symbols et 13 work items, replay sans écriture, 22 liens structurels et zéro evidence/proof/admission/promotion. Le correctif local de résolution des parents brick est couvert par tests. |
