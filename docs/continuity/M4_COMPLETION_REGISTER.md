...
| M4-A extension | `PASS borné` pour le resolver runtime read-only, policy WAL/SHM et chaînage override jusqu’au reader, le ledger Core 033, la conformité read-only `component` et l’import de page explicite/réexécutable ; baseline 17 records, 17 liens de ledger, replay sans écriture, zéro evidence et zéro proof link dans un store temporaire. |
