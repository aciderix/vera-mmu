...
>
> **État factuel au 26 août 2026 :** M4.1–M4.15 sont `PASS` dans leurs contrats bornés. M4-A a importé la page baseline de 17 components dans un store VERA temporaire avec 17 liens de ledger et replay sans écriture. M4-B a attesté la même source, puis importé/post-validé des pages initiales de 9 `function_symbol→symbol` et 13 `brick→work_item`, avec ledger, audits, replay sans écriture et zéro evidence/proof/admission/promotion. La branche toolkit fournie a restauré la précondition source/toolchain. M4-D possède désormais un runner Pack fermé et sandboxé : `difftest` a produit `PASS` (`272/272`) comme evidence VERA `PENDING`, et la fixture Wine `win32_username` a produit `PASS` (`1/1`) dans le même sandbox. Le corpus Wine global historique reste `FAIL` à `255/264` ; son exécution complète dans le sandbox a bloqué sur `win32_winsock` et n’a produit aucun verdict. Ces faits **ne valent ni compatibilité ARET complète, ni parité, ni M4.EXIT**.
>
...
| Wall / précondition restaurée | La branche toolkit verrouillée a fourni source, corpus, scripts et build ; le binaire externe hashé est désormais consommé par un runner Pack fermé, sandboxé, et relié à un asset/execution/evidence VERA. | Le résultat `difftest` `PASS` et la fixture Wine ciblée `PASS` restent `PENDING`; aucun `SKIPPED`, `UNKNOWN`, résultat externe ni fixture isolée ne permet d’affirmer C07/C08 ou M4.EXIT. Le corpus Wine historique reste en échec à `255/264` et son run sandboxé complet reste non terminé. |
