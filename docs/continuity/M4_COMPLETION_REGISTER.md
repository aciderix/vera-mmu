...
| `M4-EXIT-04` Import `function_symbol → symbol` | C04, C16 | Lecteur V1 borné, projection vers `symbol`, liaison exacte aux entities importées, unicité et provenance source. | Tests source/cible de colonnes, parent manquant, unicité, pagination, conflit, rollback et reprise ; comparaison de cardinalité et liens. | Lecteur read-only paginé/hashé, projection déterministe sans écriture et primitive Core 034 de batch `SYMBOL` publiés ; conformance profonde, préflight, collision de série, autorisation explicite, import ARET et reprise **restent requis**. |
| `M4-EXIT-05` Import `brick → work_item` | C05, C16 | Lecteur V1 borné, projection de statut/type/priorité/milestone/platforme/parent et liens, avec provenance et politique de statut explicite. | Tests de tous statuts, hiérarchie, ordre roadmap, liens component, cycles/parents impossibles, rollback/reprise et comparaison de cardinalité. | Lecteur read-only, projection déterministe sans écriture et primitive Core 034 de batch `WORK_ITEM` publiés ; sémantique d’état cible, préflight, collision de série, autorisation explicite, import ARET et reprise **restent requis**. |
...
| Prochain lot autorisé | Conformance source et préflight read-only séparés pour `function_symbol` et `brick`, avant tout contrat de collision, autorisation ou import ARET ; le Core 034 ne les autorise pas implicitement et M3 ne doit pas être rouvert. |
