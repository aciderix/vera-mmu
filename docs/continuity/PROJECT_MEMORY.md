...
## 61. Addendum — intégration réelle temporaire M4-B et correction de parent brick

| ID | Catégorie | Énoncé | Statut | Provenance | Journal |
|---|---|---|---|---|---|
| `MEM-STATE-120` | Intégration structurelle M4-B | `SUPERSEDES: MEM-STATE-119` pour l’exécution temporaire réelle. Le snapshot ARET V1 a été résolu sous `aret-memory/`, attesté au hash `85bdf19a5683591a8e3d42571bd4f28285a72f1a96627f392aa0dd0bfdb01cf5`, taille `11 280 384` bytes, sans `-wal`/`-shm`, et lié au commit propre `7f7b4df6d4f3bb493dfa26868fcec5f5b95a7ac4`. Dans un store VERA exclusivement sous `/tmp`, la page complète de 17 components a été importée puis post-validée, suivie de 9 `function_symbol→symbol` et de 13 `brick→work_item`; les trois replays exacts ont été sans écriture. | `OBSERVED` | `/tmp/vera-m4b-real-integration-_k5hvo49/integration_result.json`; vérifications Git et runtime de session. | `LOG-0172` |
| `MEM-DEC-067` | Parent structurel brick | `brick.component_id` est une identité source legacy. Avant le contrôle de collision, le pack la résout vers l’entité VERA déterministe créée par M4-A : `aret-component--<component_id>`. Cette transformation reste dans le Domain Pack et ne modifie ni le Core ni la source ARET. Le correctif local `2ab13fb4d8b6cb7558c824f6d405c4d7b27e95db` est couvert par un test de régression read-only. | `DECISION` | `structural_target_collision.py`; `test_aret_structural_target_collision.py`; cycle rouge→vert de session. | `LOG-0172` |
| `MEM-STATE-121` | Limites préservées après intégration | Les batches structurels observés restent `IMPORTED_NO_PROMOTION` et les post-validations `POST_VALIDATED_NO_PROMOTION`. La cible temporaire contient 17 entities, 9 symbols, 13 work items, 1 ledger component, 2 resource ledgers et 22 liens structurels. `evidence`, `evidence_admission` et `knowledge_proof` sont à zéro ; aucune preuve, admission, promotion ni mutation ARET n’a été créée. Les states legacy des bricks (`ACTIVE`, `DONE`, `PLANNED`) restent dans les métadonnées ; tous les work items Core restent `PLANNED`. | `OBSERVED` | Résultat d’intégration temporaire M4-B ; post-validations read-only et replays exacts. | `LOG-0172` |
| `MEM-STATE-122` | Reprise active | M4 et M4-B restent `IN_PROGRESS`. C01–C06/C16 restent `SPLIT`; C07/C08 restent `BLOCKED — MEM-WALL-001`; parité ARET reste `UNKNOWN`; `M4.EXIT` reste `NOT_ELIGIBLE`. Les contraintes de pagination/série et de lifecycle/Front restent hors de ce contrat. Le prochain travail non bloqué est la revue, la publication éventuelle des commits localement validés et la documentation de continuité ; aucun push ne doit être effectué sans validation explicite du propriétaire. | `OBSERVED` | Registre M4, résultat temporaire M4-B et instruction de publication du propriétaire. | `LOG-0172` |

### MEM-STATE-119 — M4-B : séries structurelles observées sur source ARET réelle

| Champ | Valeur |
|---|---|
| Baseline | VERA part de `e92ab70c2608bac4fa2e21e401ec9d1833f747bb`, avec une suite initiale verte ; ARET-MMU demeure propre au commit `7f7b4df6d4f3bb493dfa26868fcec5f5b95a7ac4`, sans sidecar WAL/SHM. |
| Hypothèse et patch | Une deuxième page structurelle peut être admise sans fusion seulement si **toutes** les ressources cibles existantes sont reliées par `resource_import_batch_record` à la même série `aret-v1` — même snapshot, mapping et kind. Le contrôle read-only classe alors la cible `MATCHING_PRIOR_SERIES_REQUIRED`; tout objet manuel, autre mapping, autre snapshot, lien manquant ou dérive de série reste refusé. L’import recontrôle cet état avant écriture, puis exige une cible vide uniquement pour la première page. |
| Tests | Deux régressions rouges→vert couvrent une seconde page `function_symbol→symbol` et `brick→work_item`, avec post-validation read-only. Contrats structurels ciblés : `17 passed`; suite complète : `362 passed, 14 subtests passed`; `git diff --check` et roue setuptools installée dans un environnement isolé : `PASS`. Commit fonctionnel local : `d2efe72` — `feat(aret-pack): support non-merge structural import series`. |
| Intégration réelle | L’artefact `docs/continuity/artifacts/m4b_multi_page_integration_2026-08-26.json` (SHA-256 `9e04c92ed404f36bcbffff6f2dc666fcfabdaabf6b4c7ee28ffddab80cd52c96`) enregistre un store VERA sous `/tmp` alimenté exclusivement en lecture depuis le snapshot ARET attesté `85bdf19a5683591a8e3d42571bd4f28285a72f1a96627f392aa0dd0bfdb01cf5`. Avec une taille de page de 3, les 9 symbols ont été importés/post-validés/rejoués en 3 pages et les 13 work items en 5 pages, après l’import des 17 components. Chaque page suivante porte `MATCHING_PRIOR_SERIES_REQUIRED`; les replays n’écrivent rien. |
| Invariants confirmés | ARET ne reçoit aucune écriture; la cible est temporaire; le ledger produit 8 batches structurels et 22 liens; les cardinalités finales sont 17 entities, 9 symbols et 13 work items. Les tables `evidence`, `evidence_admission` et `knowledge_proof` restent à zéro; les work items Core restent `PLANNED` et les états legacy restent des métadonnées. |
| Limites et verdict | `OBSERVED_MULTI_PAGE_INTEGRATION_SUCCESS_NO_PROMOTION` uniquement. L’intégration ne prouve pas la politique de lifecycle/Front, la réconciliation exhaustive des données sémantiques, la compatibilité runtime complète, C07/C08, la parité ARET ou `M4.EXIT`. M4-B demeure `IN_PROGRESS`; les C04/C05/C16 restent `SPLIT`. |

### LOG-0173 — Séries structurelles ARET non fusionnelles
...
### MEM-STATE-121 — M4-C.2 : import knowledge réel, paginé et sans promotion

| Champ | Valeur |
|---|---|
| Commit fonctionnel | `88e56d5` — `feat(aret-pack): import knowledge through verified batches` — prolonge le ledger 035 par une chaîne Pack explicite : type cible, préflight, collision, autorisation, write-path Core et post-validation. Aucune publication distante n’est effectuée. |
| Politique sémantique | Le type cible explicite et unique est `aret-legacy-knowledge`. Les 8 types legacy sont préservés dans `metadata.source.legacy_type`. `ACTIVE→ACTIVE`; `OBSERVED→OBSERVED`; l’unique `SUPERSEDED→OBSERVED` avec `legacy_status` et `supersedes_id` préservés, sans appel au service de supersession ni promotion. |
| Intégration réelle | L’artefact `docs/continuity/artifacts/m4c_multi_page_knowledge_integration_2026-08-26.json` (SHA-256 `567cdf8ccd06ee714c2220548e9677c0ad25d384a73286a57117be482f47ac1c`) montre l’import dans un store VERA neuf sous `/tmp` de 532 knowledge ARET attestées en 6 pages de 100/100/100/100/100/32. Chaque page est préflightée, autorisée, importée, post-validée et rejouée sans écriture; les pages 2→6 exigent `MATCHING_PRIOR_SERIES_REQUIRED`. |
| Invariants observés | ARET reste au commit propre `7f7b4df6d4f3bb493dfa26868fcec5f5b95a7ac4`, snapshot `85bdf19a5683591a8e3d42571bd4f28285a72f1a96627f392aa0dd0bfdb01cf5`, runtime `NO_WAL_SIDECARS`. La cible contient 532 knowledge, 6 batches et 532 liens; statuts Core `ACTIVE=50` et `OBSERVED=482`; `knowledge_supersession=0`, `knowledge_source=0`, `knowledge_proof=0`, `evidence=0`, `evidence_admission=0`. |
| Gates | Tests ciblés : `10 passed`; suite complète : `372 passed, 14 subtests passed`; `git diff --check`, scan anti-ARET Core et roue isolée : `PASS`. |
| Limites et verdict | `OBSERVED_MULTI_PAGE_KNOWLEDGE_IMPORT_NO_PROMOTION` uniquement. La migration source `knowledge_source`, tags, relations, supersession, proof/proof links, audit et Front demeure ouverte; le rabat `SUPERSEDED→OBSERVED` est une frontière sûre, pas une preuve de lineage. M4-C et M4.EXIT restent `IN_PROGRESS` / `NOT_ELIGIBLE`. |

### MEM-STATE-122 — Audit M4.EXIT et wall de toolchain confirmée

| Champ | Valeur |
|---|---|
| État sémantique livré | `f0aad88` ajoute Core 036 (`knowledge_source_import_batch`) et `873fad9` ajoute la lecture/projection `knowledge_source` read-only. Ces primitives sont validées localement mais n’autorisent ni attache source réelle, ni migration de tags/relations/lineage/proofs/audit/Front. |
| Audit de sortie | L’artefact `docs/continuity/artifacts/m4_exit_precondition_audit_2026-08-26.md` qualifie les 15 gates. Aucune n’est artificiellement promue : plusieurs restent `IN_PROGRESS`, C07/C08 restent `BLOCKED` et la parité est `UNKNOWN`. |
| Tentative de restauration | L’artefact `docs/continuity/artifacts/m4_exit_toolchain_restoration_check_2026-08-26.md` constate l’absence des binaires `gcc`/Cargo/Wine/MinGW/Clang/LLVM/zstd, du binaire ARET, des scripts `bench/*`, de `Cargo.toml` et de `src/cpudiff.rs`. Les quatre branches publiques disponibles et les références locales ne contiennent pas ces artefacts. Aucun package ni source non attestée n’a été installé/exécuté. |
| Vérification locale finale | Suite VERA : `378 passed, 14 subtests passed`. VERA est à `873fad9` avec seulement les deux artefacts d’audit en attente de commit; ARET reste propre au commit `7f7b4df6d4f3bb493dfa26868fcec5f5b95a7ac4`. Les prérequis `target/release/aret`, `bench/difftest.sh` et `Cargo.toml` sont explicitement absents. |
| Décision | `M4.EXIT = NOT_ELIGIBLE`. La condition de sortie exige toutes les gates `M4-EXIT-01..15 = PASS`, C01–C16 sans `SPLIT/BLOCKED/UNKNOWN`, toolchain/oracles de référence exécutables, parité comportementale et dépendances M5/M6 réalisées. Ces conditions ne sont pas réunies. |
| Reprise autorisée | Fournir un bundle ou une révision ARET attestée contenant les scripts/corpus/binaire ou le build reproductible de la toolchain, puis poursuivre les contrats M4-C par table et réaliser M5/M6. Ne pas installer des substituts ni déclarer une parité sur l’état actuel. |
### MEM-STATE-123 — Toolchain toolkit restaurée, oracles externes observés et M4.EXIT maintenu
| Champ | Valeur |
|---|---|
| Supersession documentaire | `SUPERSEDES: MEM-STATE-122` pour le seul constat d’indisponibilité de la toolchain ; le verdict M4.EXIT et les limites de migration restent applicables. |
| Référence attestée | La branche fournie `aciderix/Automatic-reverse-engineering-toolkit:claude/aret-mcp-startup-check-5a13sx` a été clonée séparément et verrouillée à `7a0429790bb04d1ad3c1819449e906140ebf4513`; elle contient `Cargo.toml`, `src/cpudiff.rs`, les scripts `bench/*` et le corpus attendu. Le clone reste propre. |
| Build externe | Rust/Cargo 1.85.0 construit avec `--locked --release` le binaire SHA-256 `6ca52f0955266aeda31d235caacf0844e2516f41d67468632f2ddb1bb1e16a19` hors du clone et hors de la baseline ARET-MMU. Les tentatives 1.75/1.79 ont échoué respectivement sur lockfile v4 et édition 2024, sans modification de source. |
| Observations d’oracle | `difftest 272/272`, transpile `4/4`, `stdcall PASS`, EH MSVC `6/6`, EH GNU `7/7`, et `funcdiff` (avec Rust 1.85 explicite) : 22 672 liftées, 11 602 optimisées, zéro divergence. `wine_hashes` : 155 `OK`, 14 `BUILD-FAIL`, 90 `SKIP`; `winediff` : `255/264`, exit 1, neuf divergences. |
| Frontière de certitude | Les scripts ont été exécutés directement depuis le clone de référence dans un environnement isolé. Logs, versions, commandes et hashes sont archivés, mais aucune capability VERA autorisée, evidence, admission, proof, gate `PASS` ou promotion `PROVEN` n’a été créée. |
...
## Addendum — M11 Génération documentaire project-bound
