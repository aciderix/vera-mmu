...
|---|---|---|---|---|
| `C05` | Migration `019` ajoute des dépendances directes append-only de `work_item` avec refus de cycle et des gates immuables reliées à une evidence. M3.11 ajoute des exigences supplémentaires append-only; M3.12 ajoute des événements de lifecycle `START`/`COMPLETE`/`CANCEL` qui dérivent un état sans réécrire le work item. L’évaluation de gate et le lifecycle ne modifient aucun knowledge. | `tests/test_work_graph_gates.py`, `tests/test_work_lifecycle.py` : dépendance, refus de cycle, gate simple/multi-evidence et lifecycle `PLANNED`→`ACTIVE`→`COMPLETED` avec work item historique inchangé; suite de 151 tests et 14 sous-tests, wheel isolé et migration 001→024; `LOG-0068`, `LOG-0070`, `LOG-0085`, `LOG-0088`. | Front, ordre roadmap, traversal, pause/réouverture/orchestration, propagation parent/enfant, quorum/disjonction/pondération/expiration de gate, import V1 `brick`, exécution réelle ARET et parité fonctionnelle. | `SPLIT` |
| `C06` | Migration `015` apporte un contrat fermé, immutable et versionné sur une capability existante : runner profile, policy réseau, timeout, schéma JSON et `yields_proof`. M3.7 valide localement un sous-ensemble fermé de paramètres (`object`, propriétés scalaires, `required`, `additionalProperties`). M3.8 ajoute une policy append-only `ALLOW`/`DENY`/`CONFIRM`; `NOOP` / `DENY_NETWORK` exige `ALLOW` avant validation ou insertion. Aucune commande ni accès réseau n’est construit. | `tests/test_capability_contracts.py`, `tests/test_noop_execution_runner.py`, `tests/test_capability_policies.py`; publications `LOG-0051`, `LOG-0055`; contrôles M3.S1 `LOG-0070`, M3.7 `LOG-0073` et M3.8 `LOG-0076`. | JSON Schema général, confirmation interactive/révision de policy, dry-run, artefacts, runners additionnels, catalogue/adaptateur ARET et parité de `PIPELINES`. | `SPLIT` |
| `C07` | Migrations `016`–`018` fournissent evidence JSON hashée liée à une execution, admission immutable ne permettant `ADMITTED` que pour `PASS`, et preuve dérivée immutable reliée à knowledge/evidence/admission. M3.9 ajoute une policy de projet singleton `HMAC_SHA256` avec exigence HMAC optionnelle; le secret ne quitte jamais la mémoire et seul le digest est persisté. M3.10 ajoute le validator local `EVIDENCE_HASH`, qui écrit un résultat `PASS`/`FAIL` séparé sans admission. | `tests/test_evidence_store.py`, `tests/test_evidence_admission.py`, `tests/test_knowledge_proof.py`, `tests/test_proof_policies.py`, `tests/test_validators.py`; publications `LOG-0058`, `LOG-0061`, `LOG-0067`; contrôles M3.S1 `LOG-0070`, M3.9 `LOG-0079` et M3.10 `LOG-0082`. | Validators de contenu/oracles, rotation/révocation de secret, capture d’artefacts, runner ARET, toolchain/oracles réels et validation de parité. `MEM-WALL-001` bloque toujours ces dimensions. | `BLOCKED` — `MEM-WALL-001` |
| `C16` | Les couches execution, evidence, admission, preuve dérivée et gate sont séparées et append-only : execution≠evidence, evidence `PASS`≠admission, admission≠preuve, et la preuve ne réécrit pas knowledge. | Migrations 014–019, tests M3.S1, triggers d’immutabilité et wheel isolé; `LOG-0070`. | Relations/lifecycle complets, import croisé, bundles, policy de preuve de projet, validators/exécutions externes et parité historique ARET. | `SPLIT` |
