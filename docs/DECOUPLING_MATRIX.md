...
> **État de la précondition historique :** `MEM-WALL-001` est `OBSERVED_RESTORED` pour la disponibilité source/toolchain : la branche toolkit fournie a été construite et ses scripts ont été exécutés. Un runner Pack fermé persiste maintenant un asset/execution/evidence VERA `PENDING` pour `difftest 272/272` et la fixture Wine `win32_username 1/1`. Il interdit toujours toute affirmation de parité : l’admission/proof/gate n’est pas livrée, le corpus Wine historique échoue à `255/264` et le corpus sandboxé complet n’est pas terminé.
