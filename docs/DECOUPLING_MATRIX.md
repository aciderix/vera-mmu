...
|---|---|---|---|---|
| `C05` | Migration `019` ajoute des dépendances directes append-only de `work_item` avec refus de cycle et des gates immuables reliées à une evidence. M3.11 ajoute des exigences supplémentaires append-only : l’évaluation retourne `PASS` seulement si l’evidence principale et toutes les exigences ont une admission préexistante `ADMITTED`; elle ne modifie aucun work item ni knowledge. | `tests/test_work_graph_gates.py` : dépendance, refus de cycle, gate simple et multi-evidence `FAIL` tant qu’une admission manque puis `PASS` lorsque toutes sont admises; suite de 149 tests et 14 sous-tests, wheel isolé et migration 001→023; `LOG-0068`, `LOG-0070`, `LOG-0085`. | Lifecycle, Front, ordre roadmap, traversal, transition de statut, quorum/disjonction/pondération/expiration de gate, import V1 `brick`, exécution réelle ARET et parité fonctionnelle. | `SPLIT` |
