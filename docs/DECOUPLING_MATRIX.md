...
> **État au 2026-09-19 — deux couplages clos.** Ce registre suit **16 couplages** (`C01`–`C16`). `C01` et `C03` sont `DONE` : leurs tests de parité sont exécutés contre la vraie source ARET, son vrai DDL et sa vraie mémoire baseline. 12 restent `SPLIT` et 2 `IN_PROGRESS` (`C07`, `C08`). Il est pourtant activement tenu — 33 sections d’avancement observé couvrent M1, M2.1 à M2.14, M3.S1 et M4.1 à M4-C — mais aucune de ces observations n’avait jamais promu une ligne mère, faute du test de parité que la règle exige. `C01` puis `C03` sont les premières promotions, et elles viennent de tels tests, pas d’une lecture de code. **Aucune affirmation de parité ARET globale ne peut s’appuyer sur lui en l’état** ; seuls `C01` et `C03` portent désormais une parité mesurée, et elle ne couvre que l’adressage et la table `component`. Voir `continuity/ENGINEERING_LOG.md` — LOG-0280 et LOG-0283.
