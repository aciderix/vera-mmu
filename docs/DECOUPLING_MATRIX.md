...
> **État au 2026-09-19 — trois couplages clos.** Ce registre suit **16 couplages** (`C01`–`C16`). `C01`, `C03` et `C04` sont `DONE` : leurs tests de parité sont exécutés contre la vraie source ARET, son vrai DDL et sa vraie mémoire baseline. 11 restent `SPLIT` et 2 `IN_PROGRESS` (`C07`, `C08`). Il est pourtant activement tenu — 33 sections d’avancement observé couvrent M1, M2.1 à M2.14, M3.S1 et M4.1 à M4-C — mais aucune de ces observations n’avait jamais promu une ligne mère, faute du test de parité que la règle exige. `C01`, `C03` puis `C04` sont les premières promotions, et elles viennent de tels tests, pas d’une lecture de code — `C04` a d’ailleurs corrigé un défaut d’unicité que personne n’avait vu. **Aucune affirmation de parité ARET globale ne peut s’appuyer sur lui en l’état** ; seuls `C01`, `C03` et `C04` portent désormais une parité mesurée, et elle ne couvre que l’adressage, `component` et `function_symbol`. Voir `continuity/ENGINEERING_LOG.md` — LOG-0280 et LOG-0283.
...
> **État de la précondition historique :** `MEM-WALL-001` est `OBSERVED_RESTORED` pour la disponibilité source/toolchain : la branche toolkit fournie a été construite et ses scripts ont été exécutés. Un runner Pack fermé persiste asset/execution/evidence; un `difftest 272/272` réel a ensuite parcouru validator asset/execution, admission stricte, proof HMAC et gate dans un runtime temporaire. Il interdit toujours toute affirmation de parité : le corpus Wine historique échoue à `255/264`, le corpus sandboxé complet n’est pas terminé, et cette chaîne bornée ne couvre pas toutes les surfaces ARET.
...
| `IN_PROGRESS` | Une précondition est disponible ou une observation réelle existe, mais la capability, la normalisation, l’admission ou le test de parité complet restent absents. |
...
|---|---|---|---|---|---|---|---|
| `C01` | `ARET://` accepte seulement les ressources ARET fermées (`knowledge`, `component`, `function`, `brick`, `proof`, `relation`, `asset`, `pipeline`) et `front/current`. | `core/addressing.py:8–60`; baseline `inventory/mcp_server_static_surface.txt`. | `vera://<project>/<resource>/<id>` avec registre de types ; lecteur d’adresses `ARET://` V1 uniquement en compatibilité de lecture. | Introduire un parseur strict générique en M1 ; encapsuler le parseur ARET actuel dans `domains/aret/compat/addressing.py`; ne pas modifier les adresses existantes. | Fixtures de round-trip VERA et ARET, encodage d’identifiants, rejet de schéma/type/injection non canoniques ; `tests/test_relation_addressing_and_wal.py` comme référence de comportement. | I002, I011, I014, I015 | `DONE` — `tests/test_aret_c01_addressing_parity.py` exécute **les deux implémentations** et compare leurs verdicts : la référence est une copie octet pour octet de `core/addressing.py` (SHA-256 `ebd73592…`, ARET-MMU `b9511dcc`), versionnée sous `tests/fixtures/aret_v1/` avec son empreinte épinglée. Quatre dimensions mesurées : écriture `242 paires, 0 divergence` ; round-trip `160 adresses produites par ARET, 0 non relue à l’identique` ; corpus réel `22 adresses de la baseline (11 280 384 octets, SHA-256 85bdf19a…), 0 divergence` ; direction `0 entrée élargissant la surface V1, 80 resserrements`, tous vérifiés hors de l’image de `ARET.make_address`. I002/I011/I014 épinglés sur le module, I015 par l’absence de référence ARET dans le Core. Dix règles, mordant vérifié une par une. |
...
| `C03` | `component` est une table et un type métier de reverse engineering ; `knowledge.component_id` le référence directement. | `schema/001_initial.sql:20–26,49–70`; `core/repository.py` opérations `register_component`; `tests/test_repository.py`. | `entity_type` déclaratif + `entity`, avec le type `COMPONENT` fourni par le pack ARET. | Ajouter les tables universelles dans une migration VERA nouvelle ; importer les composants ARET comme entités sans réécrire la base V1. | Import de composant, unicité, liens de connaissance, intégrité référentielle, export/import bundle et absence de `component` dans le Core. | I001, I003, I011, I014, I015 | `DONE` — `tests/test_aret_c03_component_parity.py` bâtit la source en exécutant **le DDL réel d'ARET** (les six `schema/*.sql` versionnés avec leurs SHA-256 épinglés) puis la peuple des **vraies lignes** de la baseline. Les six dimensions sont mesurées : import `17 composants lus, projetés, autorisés et importés, IMPORTED_NO_PROMOTION` ; unicité `clé primaire de la table STRICT réelle, collision refusée` ; liens de connaissance `520 liens, toutes les cibles déclarées` ; intégrité référentielle `0 orphelin` ; bundle `export puis restauration, 17 entités identiques` ; absence de `component` dans le Core `vérifiée par scan`. `aret_v1_schema_manifest()` est en outre confronté au DDL qu'il prétend décrire — `18 tables, migrations 1 à 6` — ce qu'aucun test ne faisait. Douze règles, mordant vérifié. |
| `C04` | `function_symbol` impose un parent `component` et une unicité `(component,module,symbol)` ; ses adresses sont `ARET://function`. | `schema/001_initial.sql:28–37`; `core/repository.py` opérations `register_function`; baseline `hashes/schema_sha256.txt`. | `symbol` technique optionnellement lié à une entité ; type et contraintes déclarés par profil/pack. | Ajouter `symbol` universel ; créer un importeur V1 `function_symbol → symbol` en conservant les IDs source dans la provenance. Le Core 034 peut enregistrer un batch générique `SYMBOL`, sans interpréter la source legacy. | Import exact, unicité configurable, relations vers entité, lecteur V1, rollback de migration de données. | I001, I003, I011, I014, I015 | `DONE` — `tests/test_aret_c04_function_symbol_parity.py`. **L'unicité a mordu :** ARET garantit `UNIQUE(component_id, module, symbol)`, et la projection joignait les trois par `-`, admis *dans* les composantes. Trois familles de triplets distincts produisaient le même identifiant VERA — séparateur dans le module contre dans le symbole, module vide contre module nommé `root`, séparateur dans le composant contre dans le module. Défaut **latent** : aucune des neuf lignes réelles ne le déclenche, mais `module` vaut `''` par défaut dans le schéma ARET. La projection échappe désormais le séparateur ; sur le corpus réel les identifiants sont inchangés. Les quatre autres dimensions : import exact `9 symboles écrits après leurs 17 composants` ; relations vers entité `chaque parent projeté est une entité déclarée` ; lecteur V1 `9 lignes rendues à l'identique, pagination terminée` ; rollback `collision introduite après l'autorisation, page entière annulée`. Onze règles, mordant vérifié. |
...
| Sous-lot | Couplages | Surface VERA désormais livrée | Evidence | Dimensions toujours inconnues | État |
|---|---|---|---|---|---|
| Ledger Core générique 034 | `C04`, `C05`, `C16` | Migration 034 ajoute `resource_import_batch`/`resource_import_batch_record` append-only. `ImportBatchService.commit_resource_import_batch` accepte exclusivement `SYMBOL`/`WORK_ITEM`, fingerprint le payload canonique, crée la cible via le service Core validé, lie le record, audite le commit, refuse l’incompatibilité et rejoue exactement sans écriture. Les transactions imbriquées utilisent des savepoints. | `tests/test_resource_import_batches.py` : success, kind inconnu, parent absent, conflit sémantique, rollback, replay, fingerprint divergent, upgrade 033→034 et triggers append-only ; `tests/test_store.py` : savepoints ; suite initiale Core `326 passed, 14 subtests passed` ; conformité/préflight M4-B : `340 passed, 14 subtests passed` ; collision/autorisation M4-B : `349 passed, 14 subtests passed`, scans pack/Core et wheel isolée ; commits Core `77591e586d8dfa60bb0b49dd06f1c056d11658a0`/`8e0d56692c3f1a5b19d9e2ac1d40678f10c7c7fc` et commits pack `8d6e4fc2ec674ac3d2be8297ad3b3f9868239eaa`/`3f21200cc0ca31119e752b5a785dc54170fa15ce`. | Import/post-validation ARET, cardinalité réelle, liens component, Front/statut et parité. La conformité source, le préflight, la collision non-fusionnelle et l’autorisation explicite sont désormais prouvés sans write-path. | `SPLIT` ; `M4-B IN_PROGRESS` |

> Le ledger 034 est une capacité Core **nécessaire mais non suffisante**. Les autorisations M4-B, bien qu’explicites, restent zéro-write et ne confèrent aucun import par elles-mêmes. Aucun de ces contrats ne lit de source hors des lecteurs établis et aucun ne produit evidence, admission, proof/proof link ni promotion. Les états C04/C05/C16 et la parité ARET restent inchangés.
...
### 2.30. Avancement observé M4-B — intégration structurelle temporaire

| Couplage | Surface VERA observée | Evidence M4-B | Dimension toujours inconnue | État de la ligne mère |
|---|---|---|---|---|
| `C04` | Une page initiale de `function_symbol` est lue depuis le snapshot ARET V1 attesté, projetée vers `symbol`, vérifiée contre les parents `component` VERA, puis importée avec le ledger Core 034 et post-validée en lecture seule. | Source Git propre `7f7b4df6d4f3bb493dfa26868fcec5f5b95a7ac4`, snapshot SHA-256 `85bdf19a5683591a8e3d42571bd4f28285a72f1a96627f392aa0dd0bfdb01cf5`, 9 symbols, liens de ledger exacts, replay sans écriture, tests ciblés et suite complète `360 passed, 14 subtests passed`; `LOG-0172`. | Pagination/séries, conflits et courses réels, réconciliation exhaustive, conversion d’autres tables et parité fonctionnelle ARET. | `SPLIT` |
| `C05` | Une page initiale de `brick` est lue, projetée vers `work_item`, et son `component_id` legacy est résolu vers le parent déterministe `aret-component--<id>` avant collision/import/post-validation. | 13 work items, parents `aret-component--CORE`/`aret-component--LIFT`, états legacy conservés dans les métadonnées et état Core `PLANNED`, replay sans écriture ; test rouge→vert de la résolution des parents, intégration temporaire et `LOG-0172`. | Pagination/séries, cycle et hiérarchie complète, lifecycle/Front, conflits et courses réels, réconciliation exhaustive et parité ARET. | `SPLIT` |
| `C16` | L’intégration temporaire utilise des transactions et ledgers génériques, avec audits d’import, sans créer de preuve, admission, lien de preuve ou promotion. | `evidence=0`, `evidence_admission=0`, `knowledge_proof=0`; post-validations read-only et replays sans écriture ; scans Core/pack et roue isolée passent ; `LOG-0172`. | Import des données sémantiques, preuves/admissions V1, front/audit V1, relations complètes et parité de tous les invariants ARET. | `SPLIT` |

> Cet addendum ne transforme aucun couplage en `DONE`. Il enregistre une intégration structurelle limitée à la première page dans un store temporaire, sans publication distante, sans promotion et sans preuve de parité ARET.

### 2.31. Addendum M4-B — séries structurelles non fusionnelles observées

| Couplage | Surface VERA observée | Evidence | Dimensions toujours inconnues | État de la ligne mère |
|---|---|---|---|---|
| `C04` | La collision structurelle distingue une cible `symbol` initialement vide d’une série ARET V1 compatible : toutes les cibles existantes doivent être liées par le ledger au même snapshot, mapping `aret-v1-function-symbol-to-symbol-v1` et kind `SYMBOL`. Une seconde page est alors autorisée, recontrôlée à l’écriture et post-validée exactement. | Tests rouge→vert de seconde page et post-validation ; `17 passed` ciblés, `362 passed, 14 subtests passed` complets. Intégration temporaire de source attestée : 9 symbols en 3 pages de 3, replay sans écriture. Artefact `continuity/artifacts/m4b_multi_page_integration_2026-08-26.json`, SHA-256 `9e04c92ed404f36bcbffff6f2dc666fcfabdaabf6b4c7ee28ffddab80cd52c96`; `LOG-0173`. | Conflits/courses indépendants, réconciliation sémantique exhaustive et parité fonctionnelle ARET. | `SPLIT` |
| `C05` | Le même mécanisme s’applique aux `work_item` avec mapping `aret-v1-brick-to-work-item-v1`; l’état Core reste `PLANNED` et l’état legacy demeure dans les métadonnées. | Tests rouge→vert de seconde page et post-validation ; intégration temporaire de source attestée : 13 work items en 5 pages, replay sans écriture, 22 liens structurels totaux; `LOG-0173`. | Lifecycle/Front, hiérarchie/cycles, conflits/courses indépendants, réconciliation exhaustive et parité fonctionnelle ARET. | `SPLIT` |
