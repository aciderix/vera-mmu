...
> **État de la précondition historique :** `MEM-WALL-001` est `OBSERVED_RESTORED` pour la disponibilité source/toolchain : la branche toolkit fournie a été construite et ses scripts ont été exécutés. Il interdit toujours toute affirmation de parité, car ces runs externes ne sont pas encore des preuves VERA et `winediff` échoue à `255/264`.
