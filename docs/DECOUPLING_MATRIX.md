...
| `C06` | Migration `015` apporte un contrat fermé, immutable et versionné sur une capability existante : runner profile, policy réseau, timeout, schéma JSON et `yields_proof`. M3.7 valide localement un sous-ensemble fermé de paramètres (`object`, propriétés scalaires, `required`, `additionalProperties`) à la déclaration puis avant l’execution `NOOP` / `DENY_NETWORK`; aucune commande ni accès réseau n’est construit. | `tests/test_capability_contracts.py`, `tests/test_noop_execution_runner.py`; publications `LOG-0051`, `LOG-0055`; contrôles M3.S1 `LOG-0070` et M3.7 `LOG-0073`. | JSON Schema général, politique explicite `ALLOW`/`DENY`/`CONFIRM`, dry-run, artefacts, runners additionnels, catalogue/adaptateur ARET et parité de `PIPELINES`. | `SPLIT` |
