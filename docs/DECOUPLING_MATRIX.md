...
| `C07` | Migrations `016`–`018` fournissent evidence JSON hashée liée à une execution, admission immutable ne permettant `ADMITTED` que pour `PASS`, et preuve dérivée immutable reliée à knowledge/evidence/admission. Le secret HMAC optionnel demeure en mémoire; un digest seul est persisté. | `tests/test_evidence.py`, `tests/test_admission.py`, `tests/test_proofs.py`; publications `LOG-0058`, `LOG-0061`, `LOG-0067`; contrôle M3.S1 `LOG-0070`. | Framework de validators/oracles, HMAC gouverné par une policy de projet, capture d’artefacts, runner ARET, toolchain/oracles réels et validation de parité. `MEM-WALL-001` bloque toujours ces dimensions. | `BLOCKED` — `MEM-WALL-001` |
