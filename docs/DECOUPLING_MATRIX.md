...
|---|---|---|---|---|
| `C05` | Migration `019` ajoute des dépendances directes append-only de `work_item` avec refus de cycle et des gates immuables reliées à une evidence. L’évaluation ne renvoie `PASS` que lorsqu’une admission préexistante est `ADMITTED`; elle ne modifie aucun work item ni knowledge. | `tests/test_work_graph_gates.py` : dépendance, refus de cycle, gate `FAIL` sans admission puis `PASS` après admission; suite de 139 tests et 14 sous-tests, wheel isolé et migration 001→019; `LOG-0068`, `LOG-0070`. | Lifecycle, Front, ordre roadmap, traversal, transition de statut, gates multi-evidence, import V1 `brick`, exécution réelle ARET et parité fonctionnelle. | `SPLIT` |
