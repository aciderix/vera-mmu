...
| `C06` | Migration `015` apporte un contrat fermé, immutable et versionné sur une capability existante : runner profile, policy réseau, timeout, schéma JSON et `yields_proof`. La seule exécution livrée est `ExecutionService.run_noop` sous contrat exact `NOOP` / `DENY_NETWORK`; aucune commande ni accès réseau n’est construit. | `tests/test_capability_contracts.py`, `tests/test_executions.py`; publications `LOG-0051`, `LOG-0055`; contrôles M3.S1 `LOG-0070`. | Validation typée complète, politique explicite `ALLOW`/`DENY`/`CONFIRM`, dry-run, artefacts, runners additionnels, catalogue/adaptateur ARET et parité de `PIPELINES`. | `SPLIT` |
