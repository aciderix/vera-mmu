...
> **État au 2026-09-18 — un couplage clos.** Ce registre suit **16 couplages** (`C01`–`C16`). `C01` est `DONE` : son test de parité est exécuté contre la vraie source ARET et la vraie mémoire baseline. 13 restent `SPLIT` et 2 `IN_PROGRESS` (`C07`, `C08`). Il est pourtant activement tenu — 33 sections d’avancement observé couvrent M1, M2.1 à M2.14, M3.S1 et M4.1 à M4-C — mais aucune de ces observations n’avait jamais promu une ligne mère, faute du test de parité que la règle exige. `C01` est la première promotion, et elle vient d’un tel test, pas d’une lecture de code. **Aucune affirmation de parité ARET globale ne peut s’appuyer sur lui en l’état** ; seul `C01` porte désormais une parité mesurée, et elle ne couvre que l’adressage. Voir `continuity/ENGINEERING_LOG.md` — LOG-0280 et LOG-0283.
