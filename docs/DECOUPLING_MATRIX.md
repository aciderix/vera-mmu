# Registre de découplage ARET-MMU → VeriChronicle
...
|---|---|---|---|---|---|
| `ARET://…` | Schéma d’adressage et ressources ARET fermées | `mmu://<project>/<resource>/<id>` ; lecteur V1 en compatibilité | `core/addressing.py` | Round-trip, rejet des ressources inconnues, lecture V1 | TODO |
| `.aret-memory` / `ARET_MEMORY_DIR` | Répertoire et variables de store | `.mmu/` / configuration de profil | `core/config.py`, CLI | Profil minimal, override borné, doctor | TODO |
...
| Outils `aret_*` | Surface MCP écrite manuellement | API `mmu_*` stable et registry contrôlée | `server/`, `compiler/` | Schémas, aliases temporaires, conformance | TODO |
