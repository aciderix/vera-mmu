...
VALIDATOR_KINDS=frozenset({'EVIDENCE_HASH','EVIDENCE_FIELDS'})
...
def record_validation(c:sqlite3.Connection,identifier:str,validator_id:str,evidence_id:str,*,actor:str,required_kind:str|None=None)->ValidationResult:
 v=c.execute('SELECT kind,rule_json FROM validator WHERE id=?',(validator_id,)).fetchone();e=c.execute('SELECT content_json,content_hash FROM evidence WHERE id=?',(evidence_id,)).fetchone()
...
def _rule(kind:str,keys:tuple[str,...]|None)->dict[str,object]:
 if kind=='EVIDENCE_HASH':
  if keys is not None:raise ValidatorError('Règle interdite pour EVIDENCE_HASH.')
  return {}
...
def _validator(r:sqlite3.Row)->Validator:
 rule={} if str(r['kind'])=='EVIDENCE_HASH' else _decode_rule(str(r['rule_json']));return Validator(str(r['id']),str(r['kind']),tuple(rule.get('required_keys',[])),str(r['created_at']),str(r['created_by']))
