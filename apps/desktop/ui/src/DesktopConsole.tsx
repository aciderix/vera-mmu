...
export function DesktopConsole() {
  ...
  const createProfileRebindPreview = () => action("Preview de rebind Profile produit", async () => setProfileRebindPreview(await desktopApi.profileRebindPreview(profileName, profileDescription)));
  ...
  return <div className="app-shell">
    ...
    <main>
      ...
      <div className="content">
        ...
        <section className="panel"><div className="panel-top"><div><p className="eyebrow">Project Profile</p><h2>Rebind contrôlé de l’identité</h2></div><Evidence label="ÉCRITURE" state="CONFIRMED ONLY" /></div><p>Seuls le nom et la description peuvent être modifiés ici. VERA sauvegarde le profil, journalise l’opération, aligne l’identité SQLite et refuse tout preview périmé. Une reprise est signalée par Doctor après interruption.</p><div className="field-grid"><label>Nom du projet<input value={profileName} onChange={(event) => setProfileName(event.target.value)} /></label><label className="wide">Description<input value={profileDescription} onChange={(event) => setProfileDescription(event.target.value)} /></label></div><button className="secondary" onClick={createProfileRebindPreview} disabled={!initialized || !profileName || busy}>Générer le preview</button>{profileRebindPreview && <div className="confirmation"><label><input type="checkbox" checked={profileRebindConfirmed} onChange={(event) => setProfileRebindConfirmed(event.target.checked)} /> J’ai vérifié le changement d’identité et son effet sur la mémoire locale.</label><button className="primary" onClick={applyProfileRebind} disabled={!profileRebindConfirmed || busy}>Confirmer le rebind</button><details><summary>Inspecter le preview</summary><pre>{JSON.stringify(profileRebindPreview, null, 2)}</pre></details></div>}</section>
