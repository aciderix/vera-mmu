...
import { readFailing, readRemaining, readRow, readStatus, readVerdict } from "./outcome";
import { isSafeToRemove, readBlockers, readChanges, readLifecycle, readPolicyLines as readTransitionPolicies, readTaxonomySections } from "./structure";
...
export function DesktopConsole() {
  ...
  const [outcome, setOutcome] = useState<JsonObject | null>(null);
  const [taxonomyOptions, setTaxonomyOptions] = useState<JsonObject | null>(null);
  const [taxonomyEdits, setTaxonomyEdits] = useState<Record<string, string>>({});
  const [taxonomyPreview, setTaxonomyPreview] = useState<JsonObject | null>(null);
  const [taxonomyConfirmed, setTaxonomyConfirmed] = useState(false);
  const [workGraph, setWorkGraph] = useState<JsonObject | null>(null);
  const [workGraphModes, setWorkGraphModes] = useState<Record<string, string>>({});
  const [workGraphPreview, setWorkGraphPreview] = useState<JsonObject | null>(null);
  const [workGraphConfirmed, setWorkGraphConfirmed] = useState(false);
  ...
  const concludeJourney = () => action("Parcours conclu", async () => setOutcome(await desktopApi.journeyOutcome()));
  const readTaxonomy = () => action("Taxonomie relue", async () => {
    const options = await desktopApi.taxonomyOptions();
    setTaxonomyOptions(options);
    setTaxonomyEdits(Object.fromEntries(readTaxonomySections(options).map((item) => [item.section, item.declared.join(", ")])));
  });
  const createTaxonomyPreview = () => action("Édition de taxonomie planifiée", async () => {
    const list = (section: string) => {
      const raw = taxonomyEdits[section];
      // A section never read is never sent: an empty box the operator has not seen is not a removal.
      return raw === undefined ? null : raw.split(",").map((item) => item.trim()).filter((item) => item.length > 0);
    };
    setTaxonomyPreview(await desktopApi.taxonomyPreview(list("knowledge"), list("entities"), list("relations")));
    setTaxonomyConfirmed(false);
  });
  const applyTaxonomy = () => action("Taxonomie écrite", async () => {
    const hash = getHash(taxonomyPreview);
    if (!hash) throw new Error("Aucun preview de taxonomie disponible.");
    await desktopApi.taxonomyApply(hash, taxonomyConfirmed);
    setTaxonomyPreview(null);
    setTaxonomyConfirmed(false);
    await readTaxonomy();
  });
  const readWorkGraph = () => action("Work Graph relu", async () => setWorkGraph(await desktopApi.workGraphRead()));
  const createWorkGraphPreview = () => action("Policy de transition planifiée", async () => {
    setWorkGraphPreview(await desktopApi.workGraphPreview(workGraphModes.start_policy ?? null, workGraphModes.completion_policy ?? null));
    setWorkGraphConfirmed(false);
  });
  const applyWorkGraph = () => action("Policy de transition scellée", async () => {
    const hash = getHash(workGraphPreview);
    if (!hash) throw new Error("Aucun preview de Work Graph disponible.");
    await desktopApi.workGraphApply(hash, workGraphConfirmed);
    setWorkGraphPreview(null);
    setWorkGraphConfirmed(false);
    setWorkGraph(await desktopApi.workGraphRead());
  });
  ...
  return <div className="app-shell">
    ...
    <main>
      ...
      <div className="content">
        ...
        <section className="panel"><div className="panel-top"><div><p className="eyebrow">Taxonomie déclarée · étapes 5 à 7</p><h2>Retirer un type, c’est orpheliner ce qu’il porte</h2></div><Evidence label="ÉCRITURE" state="CONFIRMED ONLY" /></div><p>Chaque type affiche <b>ce que la mémoire enregistre déjà sous lui</b>. Le retirer rendrait ces enregistrements orphelins, et c’est le Core qui refuse — pas cet écran qui grise un bouton : une règle appliquée seulement par une interface cesse d’exister dès qu’autre chose écrit. Un décompte que le Core n’a pas donné s’affiche <code>?</code> et jamais <code>0</code> : zéro se lit « sans risque », et c’est la seule erreur qui orpheline vraiment.</p>{!access("prepare").open && <Locked reason={access("prepare").reason} />}<button className="secondary" onClick={readTaxonomy} disabled={!initialized || busy}>Relire la taxonomie</button>{readTaxonomySections(taxonomyOptions).length > 0 && <><ul className="journey">{readTaxonomySections(taxonomyOptions).flatMap((section) => section.declared.map((type) => <li key={`${section.section}-${type}`} className={`journey-step ${isSafeToRemove(section, type) ? "journey-completed" : "journey-blocked"}`}><span className="journey-label">{section.section} · {type}</span><span className="journey-state">{section.usage[type] === null ? "?" : section.usage[type]}</span></li>))}</ul><div className="field-grid">{readTaxonomySections(taxonomyOptions).map((section) => <label key={section.section} className="wide">{section.section}{section.required ? " (requise)" : ""} — types séparés par virgule<input value={taxonomyEdits[section.section] ?? ""} onChange={(event) => setTaxonomyEdits({ ...taxonomyEdits, [section.section]: event.target.value })} /></label>)}</div></>}<button className="secondary" onClick={createTaxonomyPreview} disabled={!initialized || busy || !taxonomyOptions}>Générer le preview</button>{taxonomyPreview && <div className="confirmation">{readBlockers(taxonomyPreview).length > 0 ? <><p className="panel-locked"><b>ÉDITION REFUSÉE</b> Le Core refuse ce retrait ; rien ne sera écrit.</p><ul className="journey">{readBlockers(taxonomyPreview).map((item) => <li key={item} className="journey-step journey-blocked"><small className="journey-reason">{item}</small></li>)}</ul></> : <><ul className="journey">{readChanges(taxonomyPreview).map((change) => <li key={change.section} className="journey-step"><span className="journey-label">{change.section}</span><span className="journey-state">+{change.added.length} / −{change.removed.length}</span><small className="journey-reason">{change.added.length > 0 ? `ajout : ${change.added.join(", ")}. ` : ""}{change.removed.length > 0 ? `retrait : ${change.removed.join(", ")}.` : ""}</small></li>)}</ul><label><input type="checkbox" checked={taxonomyConfirmed} onChange={(event) => setTaxonomyConfirmed(event.target.checked)} /> J’ai vérifié les types retirés et ce qu’ils portent.</label><button className="primary" onClick={applyTaxonomy} disabled={!taxonomyConfirmed || busy}>Confirmer la taxonomie</button></>}<details><summary>Inspecter le preview</summary><pre>{JSON.stringify(taxonomyPreview, null, 2)}</pre></details></div>}</section>
        <section className="panel"><div className="panel-top"><div><p className="eyebrow">Work Graph · étape 8</p><h2>Le cycle de vie est fixe ; ses deux policies se scellent une fois</h2></div><Evidence label="ÉCRITURE" state="IRREVERSIBLE" /></div><p>Les états et les transitions viennent du Core et ne se modifient pas : les afficher comme éditables serait proposer une édition qu’il refuserait. Ce qui se déclare, ce sont les deux policies de transition, parmi un <b>catalogue fermé</b> que le Core publie. Une déclaration est <b>irréversible</b> — VERA le dit avant, pas après.</p>{!access("prepare").open && <Locked reason={access("prepare").reason} />}<button className="secondary" onClick={readWorkGraph} disabled={!initialized || busy}>Relire le Work Graph</button>{readLifecycle(workGraph) && <div className="action-grid"><div className="metric"><b>{readLifecycle(workGraph)!.states.length}</b><span>états</span></div><div className="metric"><b>{readLifecycle(workGraph)!.transitions}</b><span>transitions</span></div><div className="metric"><b>{readLifecycle(workGraph)!.editable ? "OUI" : "NON"}</b><span>éditable</span></div></div>}{readTransitionPolicies(workGraph).length > 0 && <div className="field-grid">{readTransitionPolicies(workGraph).map((line) => <label key={line.id}>{line.id} · {line.status}<select value={workGraphModes[line.id] ?? line.mode ?? ""} onChange={(event) => setWorkGraphModes({ ...workGraphModes, [line.id]: event.target.value })}><option value="">— ne pas déclarer —</option>{line.availableModes.map((mode) => <option key={mode}>{mode}</option>)}</select></label>)}</div>}<button className="secondary" onClick={createWorkGraphPreview} disabled={!initialized || busy || Object.values(workGraphModes).every((mode) => !mode)}>Générer le preview</button>{workGraphPreview && <div className="confirmation">{!isConfirmable(workGraphPreview) ? <p className="panel-locked"><b>DÉCLARATION REFUSÉE</b> {String(workGraphPreview.blockers ?? "Ce preview n’est pas une déclaration relue.")}</p> : <><p className="panel-locked"><b>IRRÉVERSIBLE</b> {workGraphPreview.irreversible === true ? "Cette déclaration ne peut pas être reprise." : "Le Core n’a pas qualifié la réversibilité de cette déclaration."}</p><label><input type="checkbox" checked={workGraphConfirmed} onChange={(event) => setWorkGraphConfirmed(event.target.checked)} /> J’ai vérifié les modes déclarés et compris qu’ils ne se reprennent pas.</label><button className="primary" onClick={applyWorkGraph} disabled={!workGraphConfirmed || busy}>Confirmer les policies</button></>}<details><summary>Inspecter le preview</summary><pre>{JSON.stringify(workGraphPreview, null, 2)}</pre></details></div>}</section>
        ...
        <section className="panel doctor" id="conclude"><div className="panel-top"><div><p className="eyebrow">Conclusion du parcours · étapes 15 à 18</p><h2>Le Doctor est la sortie du parcours, pas un bouton de côté</h2></div><Evidence label="PARCOURS" state={outcome ? readStatus(outcome) : "WAITING"} /></div><p>Les quatre dernières étapes — valider, générer, installer, diagnostiquer — existaient et fonctionnaient ; <b>rien ne les reliait</b>. Un projet pouvait finir son parcours, s’entendre dire qu’il ne restait rien, et être cassé. VERA rend ici le verdict unique : la surface déclarative de l’étape 15, la santé de l’étape 18, et ce qui reste ouvert quand il reste quelque chose. Tant qu’une étape observable est ouverte, les deux lignes valent <code>NOT_REACHED</code> — un diagnostic rendu trop tôt ne dirait que « pas encore », et apprendrait à passer outre le seul contrôle qu’on ne doit jamais passer.</p><button className="secondary" onClick={concludeJourney} disabled={!project || busy}>Conclure le parcours</button>{outcome && <div className="confirmation"><p className={readStatus(outcome) === "COMPLETE" ? "metric" : "panel-locked"}><b>{readStatus(outcome)}</b> {readVerdict(outcome)}</p><ul className="journey">{([["validation", 15, "Valider"], ["doctor", 18, "Lancer Doctor"]] as const).map(([key, index, label]) => { const row = readRow(outcome, key); return <li key={key} className={`journey-step ${row.status === "VALID" || row.status === "PASS" ? "journey-completed" : row.status === "NOT_REACHED" ? "journey-step" : "journey-blocked"}`}><span className="journey-index">{String(index).padStart(2, "0")}</span><span className="journey-label">{label}</span><span className="journey-state">{row.status}</span><small className="journey-reason">{row.detail}</small></li>; })}</ul>{readFailing(outcome).length > 0 && <ul className="journey">{readFailing(outcome).map((check) => <li key={check.name} className="journey-step journey-blocked"><span className="journey-label">{check.name}</span><span className="journey-state">FAIL</span><small className="journey-reason">{check.detail} → {check.remediation}</small></li>)}</ul>}{readRemaining(outcome).length > 0 && <ul className="journey">{readRemaining(outcome).map((step) => <li key={step.id} className={`journey-step journey-${step.state.toLowerCase()}`}><span className="journey-index">{String(step.index).padStart(2, "0")}</span><span className="journey-label">{step.label}</span><span className="journey-state">{step.state}</span><small className="journey-reason">{step.reason}</small></li>)}</ul>}<details><summary>Inspecter la conclusion</summary><pre>{JSON.stringify(outcome, null, 2)}</pre></details></div>}</section>
