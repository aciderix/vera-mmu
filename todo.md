...
- [ ] §54 — Parité ARET : `C01` (adressage), `C02` (résolution du store), `C03` (`component`) et `C04` (`function_symbol`) et `C05` (`brick`) sont `DONE`, leurs tests exécutés contre le DDL réel d'ARET et sa mémoire baseline. `C04` a révélé et fait corriger un défaut d'unicité : la projection de symbole perdait la garantie `UNIQUE` d'ARET. Neuf couplages de données restent faisables ici ; `C07`/`C08` demandent Wine et MinGW.
