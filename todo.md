...
- [x] Rejouer la suite complète sur la matrice native Windows x64 — fait au run #51 sur `c014dbf` : `1034 passed, 196 subtests passed` et `78 passed`, chiffres identiques sur Linux x64 et Windows x64, zéro échec, les 107 tests de parité inclus. Le run #50 avait trouvé trois causes Windows, toutes dans les tests ; voir `LOG-0313` et `LOG-0314`.
...
- [x] §29–34 — Dashboard configurateur visuel : livré entier. B1 à B12 sont clos ; les dix-huit étapes sont toutes praticables depuis le Dashboard, et un test de parité ferme la chaîne Core → bridge → parent Rust → console.
...
- [ ] §54 — Parité ARET : `C01` (adressage), `C02` (résolution du store), `C03` (`component`), `C04` (`function_symbol`), `C05` (`brick`), `C06` (catalogue de capabilities), `C09`, `C10`, `C11` (serveur MCP), `C12` (playbook), `C13` (synchronisation Git), `C14` (bundle) et `C16` (noyau épistémique) sont `DONE` — 13 sur 16, leurs tests exécutés contre le DDL réel d'ARET, sa mémoire baseline et, pour `C13`, ses vraies fonctions chargées et lancées sur de vrais dépôts. `C04` a révélé et fait corriger un défaut d'unicité ; `C13` a trouvé un défaut de parsing dans ARET V1 — `automatic_sync` refuse une mémoire propre et `sync_memory_only` ne commite rien en annonçant un succès — et une règle morte dans VERA, corrigée. Un seul couplage de données reste faisable ici ; `C07`/`C08` demandent Wine et MinGW.
