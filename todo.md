...
- [ ] Obtenir un run vert de `desktop-packaging.yml` sur les deux runners : trois causes racines corrigées (`os.fsync` sur poignée lecture seule, séparateur natif dans le journal de migration, connexions SQLite non fermées par les fixtures), verdict Windows à mesurer au prochain run.
