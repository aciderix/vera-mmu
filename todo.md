...
- [ ] Gouverner les panneaux de la console par l’état dérivé du parcours : aujourd’hui le parcours est affiché, il ne ferme aucun panneau bloqué.
- [ ] Installer un lanceur de tests d’interface pour `apps/desktop` — aucun n’existe, `build` se limite à `tsc --noEmit && vite build` — en vérifiant la CI, qui consomme le verrou de dépendances en `--frozen-lockfile`.
