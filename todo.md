...
- [ ] §54 — Parité ARET : `C01` (adressage), `C02` (résolution du store), `C03` (`component`) et `C04` (`function_symbol`) `C05` (`brick`) `C16` (noyau épistémique), `C09`, `C10` et `C11` (serveur MCP) sont `DONE` — 9 sur 16, leurs tests exécutés contre le DDL réel d'ARET et sa mémoire baseline. `C04` a révélé et fait corriger un défaut d'unicité : la projection de symbole perdait la garantie `UNIQUE` d'ARET. Cinq couplages de données restent faisables ici ; `C07`/`C08` demandent Wine et MinGW.
