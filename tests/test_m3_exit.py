...
class M3ExitTests(unittest.TestCase):
 ...
 def test_fresh_store_runs_the_entire_closed_m3_chain(self):
  with self._open() as s:
   self.assertEqual(s.metadata()['store_format'],{'schema_version':32});self.assertEqual(set(s.migration_checksums),set(range(1,33)))
 ...
 def test_historical_store_upgrades_from_001_to_032(self):
  ...
  with self._open(schema) as upgraded:self.assertEqual(upgraded.metadata()['store_format'],{'schema_version':32});self.assertEqual(set(upgraded.migration_checksums),set(range(1,33)))
