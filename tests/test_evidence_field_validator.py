...
class EvidenceFieldValidatorTests(unittest.TestCase):
 ...
 def test_declared_required_keys_produce_local_pass_fail_without_admission(self):
  with self._open() as s:
   self.assertEqual(s.metadata()['store_format'],{'schema_version':32});self._evidence(s);v=ValidatorService(s);v.register('fields','EVIDENCE_FIELDS',required_keys=('claim','scope'))
