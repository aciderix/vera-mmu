...
class CliTests(unittest.TestCase):
    ...
    def test_documentation_projects_canonical_state_without_writing(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            runtime = Path(directory) / ".vera-mmu"
            runtime.mkdir()
            profile_path = runtime / "project.yaml"
            profile_path.write_text(PROFILE, encoding="utf-8")
            self.assertEqual(main(["init", str(profile_path)]), 0)
            output = StringIO()
            with redirect_stdout(output):
                status = main(["documentation", str(profile_path)])
            payload = json.loads(output.getvalue())
            self.assertEqual(status, 0)
            self.assertTrue(payload["ok"])
            self.assertEqual(set(payload["documentation"]["documents"]), {"MMU_SETUP.md", "TOOLS.md", "GATES.md", "POLICIES.md", "ARCHITECTURE.md", "MAINTENANCE.md"})
    ...
    def test_init_creates_and_reopens_the_profile_bound_store(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(first["metadata"]["store_format"], {"schema_version": 39})
