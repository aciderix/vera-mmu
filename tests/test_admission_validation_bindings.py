...
class AdmissionValidationBindingTests(unittest.TestCase):
 ...
 def test_strict_policy_requires_explicit_same_evidence_pass_binding_atomically(self):
  with self._open() as s:
   self.assertEqual(s.metadata()['store_format'],{'schema_version':32});self._evidence_and_validations(s);AdmissionPolicyService(s).declare('VALIDATED_PASS_EVIDENCE');service=AdmissionService(s);audits=len(s.audit_events())
 ...
 def test_existing_m3_23_store_upgrades_from_031_to_optional_binding_schema(self):
  ...
  with self._open(schema) as upgraded:self.assertEqual(upgraded.metadata()['store_format'],{'schema_version':32});self.assertEqual(upgraded.connection.execute('SELECT COUNT(*) FROM admission_validation_binding').fetchone()[0],0)
