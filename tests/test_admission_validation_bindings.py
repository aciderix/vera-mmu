...
class AdmissionValidationBindingTests(unittest.TestCase):
 ...
 def test_strict_policy_requires_explicit_same_evidence_pass_binding_atomically(self):
  with self._open() as s:
   self.assertEqual(s.metadata()['store_format'],{'schema_version':35});self._evidence_and_validations(s);AdmissionPolicyService(s).declare('VALIDATED_PASS_EVIDENCE');service=AdmissionService(s);audits=len(s.audit_events())
