...
| **M4 — ARET compatibility pack** | **In progress.** M4.1 provides an isolated, strict read-only parser for canonical `ARET://` V1 addresses; M4.2 declares the historical runtime names without resolving environment or paths. Neither performs lookup, import, migration, conversion or I/O. A profile, offline importer, operational runtime adapter, toolchain declarations and parity suite remain required and isolated from the Core. |
