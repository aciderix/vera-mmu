...
| **M3 — Capability / Evidence / Gates** | **In progress. M3.S1 and M3.S2 are validated slices:** M3.S2 integrates M3.7–M3.12: closed contracts, bounded local parameter validation, immutable `ALLOW`/`DENY`/`CONFIRM` policy, a `NOOP`/`DENY_NETWORK` runner, executions, hashed evidence, admission, project HMAC policy without secret persistence, derived proof, conjunctive multi-evidence admission gates, the local `EVIDENCE_HASH` validator and a derived work lifecycle. Deferred: business/external validators, additional runners, advanced graph/lifecycle and CLI/MCP surface. This does not close M3 globally or claim ARET parity. An execution event is never a proof. |
