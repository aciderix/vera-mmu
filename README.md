...
| **M3 — Capability / Evidence / Gates** | **In progress. M3.S1 and M3.7–M3.10 delivered:** closed contracts, bounded local parameter validation, immutable `ALLOW`/`DENY`/`CONFIRM` policy, a `NOOP`/`DENY_NETWORK` runner, executions, hashed evidence, admission, project HMAC policy without secret persistence, derived proof, direct single-evidence admission gates and the local `EVIDENCE_HASH` validator. Deferred: business/external validators, additional runners, advanced graph/gates and CLI/MCP surface. An execution event is never a proof. |
