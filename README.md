...
| **M4 — ARET compatibility pack** | **In progress.** M4.1 provides an isolated, strict read-only parser for canonical `ARET://` V1 addresses; it performs no lookup, import, migration or conversion. A profile, offline importer, toolchain declarations and parity suite remain required and isolated from the Core. |
