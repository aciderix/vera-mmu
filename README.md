...
La suite actuelle compte **1114 tests et 392 sous-tests** en local — six de plus qu’en CI, où les tests dépendant du dépôt toolkit se sautent — côté Core et **78 tests d’interface**, dont le parcours de sortie complet — `scan`, `init`, `validate`, `generate`, `doctor` — rejoué sur les six domaines, et une chaîne épistémique complète menée jusqu’à une promotion `PROVEN` signée. Ce décompte est attesté **sur les deux plateformes**, le 19 septembre 2026 sur les runners natifs de `desktop-packaging.yml` (run #64, commit `c79690b`) : `1108 passed, 9 skipped, 381 subtests passed` et `78 passed`, chiffres identiques sur Linux x64 et Windows x64, zéro échec. Il couvre **les seize couplages** du registre de découplage et leurs **cent quatre-vingt-dix tests de parité**, ainsi que la suite de conformité, les tests d’interface, le sidecar natif, l’archive CLI, l’AppImage et le `.deb` côté Linux, NSIS et MSI côté Windows. Aucun test n’est conditionné à une plateforme : la collecte est la même des deux côtés. La suite s’exécute en **74 s** sur quatre cœurs — `python -m pytest -q -n auto --dist loadfile`, avec l’extra `test` installé — contre 261 s en série, décomptes identiques. Les résultats et limites détaillés figurent dans les [records de conformance M8](docs/continuity/artifacts/m8_multi_domain_conformance_2026-08-27.md) et de [candidats M9](docs/continuity/artifacts/m9_release_candidate_pipeline_2026-08-27.md).
