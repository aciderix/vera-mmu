...
The `identity` command validates the profile and prints its deterministic SHA-256 identity. The `inspect` command resolves project roots, optional local VCS markers, runtime, SQLite location, and artifact directory without opening a store or running Git. The `init` command opens only the profile-bound runtime, applies the checksum-protected migration ledger, records the ProjectIdentity and prints the resulting metadata. The public Python Core additionally exposes `EntityService` for registered generic types and exact entity creation/read, `RelationService` for registered immutable edges between existing entities, `KnowledgeService` for append-only hash-verified assertions under safe initial statuses, and `KnowledgeSourceService` for immutable, line-bounded document references. `KnowledgeSourceService` treats sources solely as declared data: it does not open, fetch, verify or import documents. `PROVEN` is explicitly rejected until Evidence Store exists. The implementation remains deliberately bounded: it adds no traversal, lifecycle, search, supersession, evidence, policy or execution semantics before their explicit tests and contracts exist.
