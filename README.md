...
La suite actuelle compte **927 tests et 69 sous-tests** côté Core et **78 tests d’interface**, dont le parcours de sortie complet — `scan`, `init`, `validate`, `generate`, `doctor` — rejoué sur les six domaines, et une chaîne épistémique complète menée jusqu’à une promotion `PROVEN` signée. Ce décompte est mesuré **sur les deux plateformes**, le 18 septembre 2026 sur les runners natifs de `desktop-packaging.yml` (run #49, commit `9861450`) : `927 passed, 69 subtests passed` et `78 passed` côté interface, chiffres identiques sur Linux x64 et Windows x64 — suite de conformité, tests d’interface, sidecar natif, archive CLI, AppImage et `.deb` côté Linux, NSIS et MSI côté Windows. Aucun test n’est conditionné à une plateforme : la collecte est la même des deux côtés. Les résultats et limites détaillés figurent dans les [records de conformance M8](docs/continuity/artifacts/m8_multi_domain_conformance_2026-08-27.md) et de [candidats M9](docs/continuity/artifacts/m9_release_candidate_pipeline_2026-08-27.md).
