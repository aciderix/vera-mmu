...
This repository now contains the **M1 identity Core**, the **M2.1 SQLite substrate**, the **M2.2 generic Entity Registry**, the **M2.3 Relation Registry**, the **M2.4 Knowledge Registry**, the **M2.5 Knowledge Source Registry**, the **M2.6 Knowledge Supersession Registry**, the **M2.7 Asset Registry**, and the **M2.8 Knowledge-Asset Link Registry**: a safely normalized Project Profile, deterministic project identities, a `WorkspaceResolver`, a confined `RuntimeLocator`, strict canonical `vera://` addresses, an immutable migration ledger, a ProjectIdentity-bound SQLite store, technical audit records, registered generic entity types, exact entity creation/read, immutable typed edges between entities, append-only knowledge assertions with content hashes, immutable declared source slices, explicit direct supersession links, binary Core assets whose SHA-256 and declared size are rechecked before byte reading, and exact immutable associations between existing knowledge and assets. A supersession link never rewrites the predecessor’s content, hash, metadata, provenance, or status. A knowledge–asset link does not read an asset, modify either endpoint, or make that asset evidence. The asset registry stores small bytes in SQLite; it does not accept file paths or open external files. It does **not** yet provide document fetch or verification, import batches, status mutation or `SUPERSEDED`, version counters, lineage traversal/listing, `PROVEN` admission, evidence services, executions or validators, gates, knowledge search, generic-relation integration, symbols, bundles, a production MCP server, a generic capability runner, an ARET migration tool, or a dashboard.
