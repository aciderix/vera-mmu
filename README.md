...
La suite actuelle compte **1060 tests et 256 sous-tests** côté Core et **78 tests d’interface**, dont le parcours de sortie complet — `scan`, `init`, `validate`, `generate`, `doctor` — rejoué sur les six domaines, et une chaîne épistémique complète menée jusqu’à une promotion `PROVEN` signée. Ce décompte est attesté **sur les deux plateformes**, le 19 septembre 2026 sur les runners natifs de `desktop-packaging.yml` (run #53, commit `76ef275`) : `1045 + 211` et `78`, chiffres identiques sur Linux x64 et Windows x64, zéro échec. Il couvre les cent dix-huit tests de parité `C01` à `C05`, `C09` à `C11`, `C13`, `C14` et `C16` — les quinze tests de `C06` ajoutés depuis ne sont pour l’instant mesurés que sur Linux x64 — ainsi que la suite de conformité, les tests d’interface, le sidecar natif, l’archive CLI, l’AppImage et le `.deb` côté Linux, NSIS et MSI côté Windows. Aucun test n’est conditionné à une plateforme : la collecte est la même des deux côtés. Les résultats et limites détaillés figurent dans les [records de conformance M8](docs/continuity/artifacts/m8_multi_domain_conformance_2026-08-27.md) et de [candidats M9](docs/continuity/artifacts/m9_release_candidate_pipeline_2026-08-27.md).
