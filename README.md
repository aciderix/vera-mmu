...
La suite actuelle compte **1091 tests et 312 sous-tests** côté Core et **78 tests d’interface**, dont le parcours de sortie complet — `scan`, `init`, `validate`, `generate`, `doctor` — rejoué sur les six domaines, et une chaîne épistémique complète menée jusqu’à une promotion `PROVEN` signée. L’attestation **sur les deux plateformes** la plus récente, le 19 septembre 2026 sur les runners natifs de `desktop-packaging.yml` (run #59, commit `834ee13`), porte sur `1068 + 275` et `78`, chiffres identiques sur Linux x64 et Windows x64, zéro échec ; les vingt-trois tests de `C15` lui sont postérieurs et sont mesurés sur Linux en attendant leur attestation. Elle couvre l’intégralité des **cent quarante et un tests de parité** `C01` à `C06`, `C09` à `C14` et `C16`, ainsi que la suite de conformité, les tests d’interface, le sidecar natif, l’archive CLI, l’AppImage et le `.deb` côté Linux, NSIS et MSI côté Windows. Aucun test n’est conditionné à une plateforme : la collecte est la même des deux côtés. La suite s’exécute en **74 s** sur quatre cœurs — `python -m pytest -q -n auto --dist loadfile`, avec l’extra `test` installé — contre 261 s en série, décomptes identiques. Les résultats et limites détaillés figurent dans les [records de conformance M8](docs/continuity/artifacts/m8_multi_domain_conformance_2026-08-27.md) et de [candidats M9](docs/continuity/artifacts/m9_release_candidate_pipeline_2026-08-27.md).
