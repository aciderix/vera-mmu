...
La suite actuelle compte **1020 tests et 196 sous-tests** côté Core et **78 tests d’interface**, dont le parcours de sortie complet — `scan`, `init`, `validate`, `generate`, `doctor` — rejoué sur les six domaines, et une chaîne épistémique complète menée jusqu’à une promotion `PROVEN` signée. Le dernier décompte mesuré **sur les deux plateformes** est `927 + 78`, le 18 septembre 2026 sur les runners natifs de `desktop-packaging.yml` (run #49, commit `9861450`) : chiffres identiques sur Linux x64 et Windows x64. Ce run couvre la suite de conformité, les tests d’interface, le sidecar natif, l’archive CLI, l’AppImage et le `.deb` côté Linux, NSIS et MSI côté Windows. Les quatre-vingt-treize tests de parité `C01` à `C05`, `C09` à `C11` et `C16` ajoutés depuis ne sont attestés que sur Linux x64. Aucun test n’est conditionné à une plateforme : la collecte est la même des deux côtés. Les résultats et limites détaillés figurent dans les [records de conformance M8](docs/continuity/artifacts/m8_multi_domain_conformance_2026-08-27.md) et de [candidats M9](docs/continuity/artifacts/m9_release_candidate_pipeline_2026-08-27.md).
