...
La suite actuelle compte **1045 tests et 211 sous-tests** côté Core et **78 tests d’interface**, dont le parcours de sortie complet — `scan`, `init`, `validate`, `generate`, `doctor` — rejoué sur les six domaines, et une chaîne épistémique complète menée jusqu’à une promotion `PROVEN` signée. Le dernier décompte attesté **sur les deux plateformes** date du 19 septembre 2026, sur les runners natifs de `desktop-packaging.yml` (run #51, commit `c014dbf`), avec des chiffres identiques sur Linux x64 et Windows x64 et zéro échec. Ce run — `1034 + 196` et `78` des deux côtés — couvre pour la première fois les cent-sept tests de parité `C01` à `C05`, `C09` à `C11`, `C13` et `C16`, ainsi que la suite de conformité, les tests d’interface, le sidecar natif, l’archive CLI, l’AppImage et le `.deb` côté Linux, NSIS et MSI côté Windows. Les onze tests de `C14` ajoutés depuis ne sont pour l’instant mesurés que sur Linux x64. Aucun test n’est conditionné à une plateforme : la collecte est la même des deux côtés. Les résultats et limites détaillés figurent dans les [records de conformance M8](docs/continuity/artifacts/m8_multi_domain_conformance_2026-08-27.md) et de [candidats M9](docs/continuity/artifacts/m9_release_candidate_pipeline_2026-08-27.md).
