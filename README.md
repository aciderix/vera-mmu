...
| **M3 — Capability / Evidence / Gates** | **In progress. M3.S1 and M3.S2 are validated slices; M3.13 is delivered:** M3.13 adds an explicit `PASS_EVIDENCE`/`VALIDATED_PASS_EVIDENCE` admission policy, where strict admission requires an existing validator `PASS` and never triggers validation. Deferred: business/external validators, additional runners, advanced graph/lifecycle and CLI/MCP surface. This does not close M3 globally or claim ARET parity. An execution event is never a proof. |
