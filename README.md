...
| **M3 — Capability / Evidence / Gates** | **In progress. M3.S1 and M3.7–M3.12 delivered:** closed contracts, bounded local parameter validation, immutable `ALLOW`/`DENY`/`CONFIRM` policy, a `NOOP`/`DENY_NETWORK` runner, executions, hashed evidence, admission, project HMAC policy without secret persistence, derived proof, conjunctive multi-evidence admission gates, the local `EVIDENCE_HASH` validator and a derived work lifecycle. Deferred: business/external validators, additional runners, advanced graph/lifecycle and CLI/MCP surface. An execution event is never a proof. |
