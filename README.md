...
La suite actuelle compte **812 tests et 69 sous-tests**, dont le parcours de sortie complet — `scan`, `init`, `validate`, `generate`, `doctor` — rejoué sur les six domaines, et une chaîne épistémique complète menée jusqu’à une promotion `PROVEN` signée. Elle passe intégralement sur **Linux x64 et Windows x64**, mesuré le 15 septembre 2026 sur les runners natifs de `desktop-packaging.yml` (run #48) : suite de conformité, dix tests d’interface, sidecar natif, archive CLI, AppImage et `.deb` côté Linux, NSIS et MSI côté Windows : suite, sidecar, archive CLI, AppImage et `.deb` côté Linux, NSIS et MSI côté Windows. Les résultats et limites détaillés figurent dans les [records de conformance M8](docs/continuity/artifacts/m8_multi_domain_conformance_2026-08-27.md) et de [candidats M9](docs/continuity/artifacts/m9_release_candidate_pipeline_2026-08-27.md).
